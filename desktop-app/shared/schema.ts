...
export const automations = pgTable("automations", {
  ...
  orLess: boolean("or_less").default(false), // If true, triggers immediately for projects already within threshold (e.g., "30 days OR LESS")
  ...
  emailBlocks: jsonb("email_blocks"), // Block-based email builder content (DEPRECATED - use templateBody)
  templateBody: text("template_body"), // Simplified text format with {{variables}} and [[BUTTON:...]] markers
  htmlBody: text("html_body"), // Pre-rendered HTML version of templateBody
  includeHeroImage: boolean("include_hero_image").default(false),
  heroImageUrl: text("hero_image_url"),
  includeHeader: boolean("include_header").default(false),
  headerStyle: text("header_style"), // minimal, professional, bold, classic
...
export const dripCampaigns = pgTable("drip_campaigns", {
  ...
  projectTypeId: varchar("project_type_id").references(() => projectTypes.id), // New FK
  name: text("name").notNull(),
  targetStageId: varchar("target_stage_id").references(() => stages.id), // Legacy single stage (nullable for new campaigns)
  // Multi-stage targeting (new system)
  targetStageIds: json("target_stage_ids"), // Array of stage IDs: ["stage-id-1", "stage-id-2"]
  isGlobal: boolean("is_global").default(false), // If true, fires when project enters ANY stage
...
export const dripCampaignEmails = pgTable("drip_campaign_emails", {
  ...
  delayMinutes: integer("delay_minutes"), // Alternative: send X minutes after subscription start (takes priority over day-based)
  // Visual email builder
  emailBlocks: text("email_blocks"), // JSON string storing email builder blocks
  templateBody: text("template_body"), // Simplified text format with button markers
