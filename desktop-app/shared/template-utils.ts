 ...
 */
export function contentBlocksToHtml(
  blocks: ContentBlock[],
  context?: {
    ...
    .map((block) => {
      switch (block.type) {
        case "HEADER":
          ...
          return generateEmailHeader(
            block.content?.style || "professional",
            context.brandingData,
          );
        ...
        case "BUTTON":
          ...
          if (linkType === "CUSTOM") {
            ...
            href = context?.smartFileToken
              ? `${baseUrl}/smart-file/${context.smartFileToken}`
              : "{{smart_file_link}}";
            ...
            href = context?.photographerToken
              ? `${baseUrl}/booking/${context.photographerToken}`
              : "{{calendar_link}}";
          } else if (linkType === "TESTIMONIALS") {
            // Testimonials/reviews link
            href = "{{testimonials_link}}";
          }
  ...
  return blocks
    .map((block) => {
      switch (block.type) {
        ...
        case "SIGNATURE":
          const sig = [];
          if (brandingData?.photographerName)
            sig.push(brandingData.photographerName);
