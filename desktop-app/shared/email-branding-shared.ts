...
import { getAccessibleTextColor } from "./color-utils.js";

/**
 * Convert relative paths to absolute URLs for use in emails
 * Works on both client and server
 * Production-aware: Uses tpcportal.co in Railway production environment
 */
function toAbsoluteUrl(urlOrPath: string | undefined): string | undefined {
  ...
  // Server-side: check for production environment first
  if (typeof window === "undefined") {
    const isProduction =
      process.env.NODE_ENV === "production" ||
      !!process.env.RAILWAY_ENVIRONMENT_NAME;
    if (isProduction) {
      // Production: use app.tpcportal.co for asset URLs (logos, images, etc.)
      baseUrl = "https://app.tpcportal.co";
...
export function generateEmailFromBlocks(
  ...
  const brandingData: BrandingData = {
    ...
    email:
      photographer?.emailFromAddr || photographer?.googleEmail || undefined,
  ...
  for (const block of blocks) {
    ...
    switch (blockType) {
      case "HEADER":
        const headerStyle =
          block.style || block.content?.style || "professional";
        htmlParts.push(generateEmailHeader(headerStyle, brandingData));
        break;

      case "HEADING":
        const headingText = renderTemplate(
          block.content?.text || "",
          variables,
        );
        htmlParts.push(
          `<h2 style="font-size: 24px; font-weight: 600; margin: 20px 0 10px 0; color: #000000;">${headingText}</h2>`,
        );
      ...
      case "TEXT":
        ...
        htmlParts.push(
          `<p style="font-size: 16px; line-height: 1.6; margin: 10px 0; color: #333333;">${textContent.replace(/\n/g, "<br>")}</p>`,
        );
        textParts.push(textContent + "\n");
        break;

      case "BUTTON":
        const buttonText = renderTemplate(
          block.content?.text || "Click Here",
          variables,
        );
        const buttonColor =
          block.content?.color || brandingData.brandPrimary || "#007bff";

        // Resolve button URL based on linkType
        let buttonUrl = "#";
        ...
        if (linkType === "CUSTOM") {
          // Custom URL - use linkValue or legacy url field, apply template variables
          buttonUrl = renderTemplate(
            linkValue || block.content?.url || "#",
            variables,
          );
        } else if (linkType === "CALENDAR") {
          // Calendar booking link - prioritize variables, then construct from context
          if (variables?.calendar_link) {
            // Use pre-resolved calendar_link from variables (preferred)
            buttonUrl = variables.calendar_link;
          } else if (photographer?.portalSlug) {
            // Portal subdomain uses /book path (photographer identified by subdomain)
            const portalBase =
              baseUrl || `https://${photographer.portalSlug}.tpcportal.co`;
            buttonUrl = `${portalBase}/book`;
          } else if (photographer?.publicToken) {
            // App domain uses /booking/calendar/{token} path
            const appBase = baseUrl || "https://app.thephotocrm.com";
            buttonUrl = `${appBase}/booking/calendar/${photographer.publicToken}`;
          }
          // If still no URL, leave as '#' (no placeholder that could appear in email)
        } else if (linkType === "SMART_FILE") {
          // Smart file links - check variables first, then linkValue, then legacy url
          if (variables?.smart_file_link) {
            buttonUrl = variables.smart_file_link;
          } else if (linkValue) {
            buttonUrl = renderTemplate(linkValue, variables);
          } else if (block.content?.url) {
            buttonUrl = renderTemplate(block.content.url, variables);
          }
        } else if (linkType === "GALLERY") {
          // Gallery links - check variables first, then linkValue, then legacy url
          if (variables?.gallery_url) {
            buttonUrl = variables.gallery_url;
          } else if (linkValue) {
            buttonUrl = renderTemplate(linkValue, variables);
          } else if (block.content?.url) {
            buttonUrl = renderTemplate(block.content.url, variables);
          }
        } else if (linkType === "TESTIMONIALS") {
          // Testimonials/reviews link - use pre-resolved testimonials_link from variables
          if (variables?.testimonials_link) {
            buttonUrl = variables.testimonials_link;
          } else if (linkValue) {
            buttonUrl = renderTemplate(linkValue, variables);
          }
        } else {
          // Fallback: use url field directly (legacy support), apply template variables
          buttonUrl = renderTemplate(block.content?.url || "#", variables);
        }

        const buttonTextColor = getAccessibleTextColor(buttonColor);
        htmlParts.push(`
          <div style="margin: 20px 0; text-align: center;">
            <a href="${buttonUrl}" style="display: inline-block; padding: 12px 30px; background-color: ${buttonColor}; color: ${buttonTextColor}; text-decoration: none; border-radius: 5px; font-weight: 600; font-size: 16px;">${buttonText}</a>
          </div>
        `);
      ...
      case "SIGNATURE":
        ...
        if (brandingData.phone)
          textParts.push(`Phone: ${brandingData.phone}\n`);
        if (brandingData.email)
          textParts.push(`Email: ${brandingData.email}\n`);
        if (brandingData.website)
          textParts.push(`Web: ${brandingData.website}\n`);
        break;
