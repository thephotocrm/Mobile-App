...
function formatPaymentSchedule(data: ContractVariableData): string {
  ...
  const scheduleText = data.paymentSchedule.map((payment, index) => {
    ...
    const description = (payment as any).description || `Payment ${index + 1}`;
...
export function parseContractVariablesHtml(template: string, data: Record<string, string>): string {
  let parsed = template;
