 ...
 */
export function calculateScheduleFromEventDate(options: GenerateScheduleOptions): PaymentInstallment[] {
  const {
    totalCents,
    eventDate,
    startDate = new Date(),
  ...
  finalPaymentDate.setDate(finalPaymentDate.getDate() - finalPaymentDaysBeforeEvent);
  ...
  const totalDays = Math.floor((finalDate.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
  ...
  if (depositPercent && depositPercent > 0) {
    ...
    if (numRemainingPayments > 0) {
      ...
      for (let i = 1; i < paymentDates.length; i++) {
        ...
        installments.push({
          id: nanoid(),
          description: isLast ? `Final payment (due ${finalPaymentDaysBeforeEvent} days before event)` : `Payment ${i}`,
    ...
    paymentDates.forEach((date, index) => {
      ...
      installments.push({
        id: nanoid(),
        description: isLast ? `Final payment (due ${finalPaymentDaysBeforeEvent} days before event)` : `Payment ${index + 1}`,
 ...
 * Distribute payments evenly across the time period
 */
function distributePaymentsEvenly(startDate: Date, endDate: Date, numberOfPayments: number): Date[] {
...
export function applyPaymentToSchedule(
  ...
  const sortedSchedule = [...updatedSchedule].sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
  ...
  for (const installment of sortedSchedule) {
    ...
    if (installment.status === 'PENDING' || installment.status === 'PARTIAL') {
      ...
      if (remainingPayment >= amountDue) {
        ...
        // Partial payment
        installment.amountPaidCents = (installment.amountPaidCents || 0) + remainingPayment;
 ...
 */
export function getNextDueInstallment(schedule: PaymentInstallment[]): PaymentInstallment | null {
  const sorted = [...schedule].sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
  return sorted.find(i => i.status === 'PENDING' || i.status === 'PARTIAL') || null;
}
...
export function recalculateScheduleForNewTotal(
  ...
  if (preservePaidInstallments) {
    ...
    const unpaidInstallments = updatedSchedule.filter(i => i.status !== 'PAID');
    
    if (unpaidInstallments.length > 0) {
      const amountPerUnpaid = Math.floor(remainingTotal / unpaidInstallments.length);
      ...
      unpaidInstallments.forEach((installment, index) => {
        ...
        installment.percentOfTotal = Number(((amount / newTotalCents) * 100).toFixed(2));
        remainingToDistribute -= amount;
      });
    }
  } else {
    // Recalculate all installments proportionally
    const totalAmount = updatedSchedule.reduce((sum, i) => sum + i.amountCents, 0);
    ...
    updatedSchedule.forEach((installment, index) => {
      const isLast = index === updatedSchedule.length - 1;
      const amount = isLast ? remainingToDistribute : Math.round(installment.amountCents * ratio);
...
export function getAllowedInstallmentCounts(
...
): { allowedCounts: number[]; maxInstallments: number; daysUntilFinal: number } {
  // If no event date, allow all options with a note they're estimated
  if (!eventDate) {
    return {
      allowedCounts: Array.from({ length: maxPossibleInstallments }, (_, i) => i + 1),
  ...
  const daysUntilFinal = Math.floor(
    (finalPaymentDate.getTime() - todayNormalized.getTime()) / (1000 * 60 * 60 * 24)
  );
...
export function generateSyntheticScheduleFromPaymentData(
  ...
  const effectiveDepositCents = depositCents > 0 ? depositCents : amountPaidCents;
  ...
  if (effectiveDepositCents > 0) {
    ...
    schedule.push({
      ...
      percentOfTotal: Number(((effectiveDepositCents / totalCents) * 100).toFixed(2)),
      status: depositPaid ? 'PAID' : 'PENDING',
      amountPaidCents: depositPaid ? effectiveDepositCents : (amountPaidCents > 0 ? amountPaidCents : undefined)
    });
  }

  // Add balance payment if there's remaining amount
  const effectiveBalanceCents = balanceDueCents > 0 ? balanceDueCents : (totalCents - effectiveDepositCents);
  ...
  if (effectiveBalanceCents > 0) {
    ...
    const amountPaidTowardsBalance = Math.max(0, amountPaidCents - effectiveDepositCents);
    const balanceFullyPaid = amountPaidTowardsBalance >= effectiveBalanceCents;
    const balancePartiallyPaid = amountPaidTowardsBalance > 0 && !balanceFullyPaid;
    ...
    schedule.push({
      ...
      percentOfTotal: Number(((effectiveBalanceCents / totalCents) * 100).toFixed(2)),
      status: balanceFullyPaid ? 'PAID' : (balancePartiallyPaid ? 'PARTIAL' : 'PENDING'),
      amountPaidCents: amountPaidTowardsBalance > 0 ? amountPaidTowardsBalance : undefined
    });
  }
...
export function generateClientChoiceSchedule(
  ...
  return calculateScheduleFromEventDate({
    ...
    finalPaymentDaysBeforeEvent: 30,
    depositPercent: depositPercent && depositPercent > 0 ? depositPercent : undefined
  });
}
