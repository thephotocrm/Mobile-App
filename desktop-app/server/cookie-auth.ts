...
export function getAuthToken(req: Request): { token: string; source: string } | null {
  ...
  if (photographerToken) {
    try {
      const payload = jwt.verify(photographerToken, process.env.JWT_SECRET!) as any;
...
export function clearAuthCookies(res: Response, req?: Request) {
  ...
  res.clearCookie(COOKIE_NAMES.PHOTOGRAPHER, { path: '/', domain: '.thephotocrm.com' }); // Domain version
  res.clearCookie(COOKIE_NAMES.ADMIN, { path: '/' }); // Host-only version
  res.clearCookie(COOKIE_NAMES.ADMIN, { path: '/', domain: '.thephotocrm.com' }); // Domain version
  res.clearCookie(COOKIE_NAMES.LEGACY, { path: '/' }); // Host-only version
  res.clearCookie(COOKIE_NAMES.LEGACY, { path: '/', domain: '.thephotocrm.com' }); // Domain version
