...
export async function setupGmailWatch(
  ...
  try {
    ...
    if (!oauth2Client) {
      console.error(
        `Cannot set up Gmail watch: OAuth client not available for photographer ${photographerId}`,
      );
      return false;
    }
    ...
    if (!topicName) {
      console.error(
        "Gmail Pub/Sub topic not configured - set GMAIL_PUBSUB_TOPIC environment variable",
      );
      return false;
    }
    ...
    if (error.code === 403) {
      console.error(
        `Gmail watch setup failed: Insufficient permissions for photographer ${photographerId}`,
      );
      console.error(
        "Make sure Gmail API is enabled and OAuth consent includes Gmail scope",
      );
    } else if (error.code === 429) {
      console.error(
        `Gmail watch setup rate limited for photographer ${photographerId}`,
      );
      console.error("Too many watch requests - will retry later");
    } else if (error.code === 400) {
      console.error(
        `Gmail watch setup invalid request for photographer ${photographerId}:`,
        error.message,
      );
    } else {
      console.error(
        `Error setting up Gmail watch for photographer ${photographerId}:`,
        error,
      );
    }
    return false;
...
export async function stopGmailWatch(photographerId: string): Promise<boolean> {
  try {
    ...
    console.error(
      `Error stopping Gmail watch for photographer ${photographerId}:`,
      error,
    );
    return false;
  }
...
export async function renewExpiringWatches(): Promise<void> {
  try {
    ...
    console.log(
      `Found ${photographers.length} photographers with Gmail watches`,
    );
    ...
    for (const photographer of photographers) {
      ...
      if (!photographer.gmailWatchExpiration) {
        console.log(
          `Photographer ${photographer.id} has no expiration date, setting up watch`,
        );
      ...
      if (photographer.gmailWatchExpiration < renewThreshold) {
        console.log(
          `Renewing Gmail watch for photographer ${photographer.id} (expires ${photographer.gmailWatchExpiration.toISOString()})`,
        );
    ...
    console.log(
      `✅ Finished checking Gmail watches: ${renewed} renewed, ${failed} failed`,
    );
  } catch (error: any) {
