 ...
 */
async function ensureShortLinkUpdated(
  shortLink: any,
  ...
  const hasReplitUrl =
    shortLink.targetUrl.includes("replit.dev") ||
  ...
  if (hasReplitUrl) {
    try {
      ...
      console.log(
        `🔧 Self-healing stale short link ${shortLink.shortCode}: ${shortLink.targetUrl} → ${newTargetUrl}`,
      );
      ...
      console.error(
        `❌ Failed to self-heal short link ${shortLink.shortCode}:`,
        error,
      );
    }
  }
...
async function getParticipantEmailsForBCC(
  projectId: string,
): Promise<string[]> {
  try {
    ...
    console.error(
      `Error fetching participants for project ${projectId}:`,
      error,
    );
    return [];
  }
...
async function getGalleryVariables(
  projectId: string,
  ...
  try {
    ...
    if (stageEnteredAt) {
      ...
      expirationDate.setMonth(
        expirationDate.getMonth() + galleryExpirationMonths,
      );
    ...
    return {
      gallery_link: linkedGallery
        ? `${baseUrl}/client/galleries/${linkedGallery.id}`
        : "",
    ...
    console.error(
      `Error fetching gallery variables for project ${projectId}:`,
      error,
    );
    return {
      gallery_link: "",
...
async function reserveAutomationExecution(
  ...
  try {
    ...
    if (automationType === "COMMUNICATION" && stepId) {
      ...
      const eventDateKey = normalizeEventDateKey(
        eventDate,
    ...
    if (existingRecord) {
      ...
      if (existingRecord.status === "SUCCESS") {
        console.log(
          `🚫 Automation already completed successfully: ${automationType} for project ${projectId}`,
        );
      ...
      const executedAt = existingRecord.executedAt
        ? new Date(existingRecord.executedAt)
        : null;
      ...
      if (
        ...
        console.log(
          `🔄 Retrying stale PENDING automation (was reserved at ${executedAt.toISOString()}): ${automationType} for project ${projectId}`,
        );
      ...
      if (existingRecord.status === "PENDING") {
        console.log(
          `⏳ Automation reservation still fresh (within 1 hour), skipping: ${automationType} for project ${projectId}`,
        );
        return { canExecute: false, executionId: null };
      }
      ...
      if (existingRecord.status === "FAILED") {
        console.log(
          `🔄 Retrying failed automation: ${automationType} for project ${projectId}`,
        );
        await db
          .update(automationExecutions)
          .set({ executedAt: new Date(), status: "PENDING" })
    ...
    console.log(
      `🔒 Automation execution reserved: ${automationType} for project ${projectId}`,
    );
    ...
    if (error?.code === "23505") {
      console.log(
        `🚫 Automation execution already reserved/completed (race condition): ${automationType} for project ${projectId}`,
      );
      return { canExecute: false, executionId: null };
    } else {
...
async function updateExecutionStatus(
  ...
  try {
    ...
    console.log(
      `✅ Updated execution status to ${status} for execution ${executionId}`,
    );
  } catch (error) {
...
function normalizeEventDateKey(
  eventDate: Date,
...
export async function processAutomations(
  ...
  try {
    ...
    for (const automation of allAutomations) {
      try {
        console.log(
          `Processing automation: ${automation.name} (type: ${automation.automationType})`,
        );
        ...
        console.error(
          `Error processing automation ${automation.name} (${automation.automationType}):`,
          error,
        );
      }
    }
...
export async function checkImmediateCountdownStageChanges(
  ...
  try {
    console.log(
      `📅 Checking immediate countdown stage changes for project ${projectId}`,
    );
    ...
    if (!project || project.status !== "ACTIVE" || !project.enableAutomations) {
      console.log(
        `📅 Project ${projectId} not eligible for countdown automations`,
      );
      return;
    }
    ...
    console.log(
      `📅 Found ${orLessAutomations.length} OR LESS countdown stage change automations`,
    );
    ...
    const sortedAutomations = orLessAutomations.sort(
    ...
    for (const automation of sortedAutomations) {
      ...
      if (
        automation.stageCondition &&
        project.stageId !== automation.stageCondition
      ) {
        console.log(
          `📅 Automation ${automation.name} requires stage ${automation.stageCondition}, project is in ${project.stageId}, skipping`,
        );
        continue;
      }
      ...
      if (shouldTrigger) {
        console.log(
          `📅 Triggering immediate countdown stage change: ${automation.name} (${daysThreshold} days or less, actual: ${daysFromEvent})`,
        );
        ...
        await executeCountdownStageChange(
          projectForChange,
          automation,
          photographerId,
          daysFromEvent,
    ...
    console.error(
      `Error checking immediate countdown stage changes for project ${projectId}:`,
      error,
    );
  }
}

async function processCommunicationAutomation(
  automation: any,
  photographerId: string,
): Promise<void> {
  ...
  console.log(
    `Communication automation "${automation.name}" (${automation.id}) - found ${projectsInStage.length} projects in stage`,
  );
...
async function processEmailBuilderAutomation(
  automation: any,
  photographerId: string,
): Promise<void> {
  ...
  console.log(
    `Email builder automation "${automation.name}" (${automation.id}) - found ${projectsInStage.length} projects in stage`,
  );
  ...
  for (const project of projectsInStage) {
    try {
      console.log(
        `Processing email builder step for contact ${project.firstName} ${project.lastName} (${project.email})`,
      );

      // Check email opt-in
      if (!project.emailOptIn) {
        console.log(
          `📧 Email opt-in missing for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
        );
        continue;
      }
      ...
      if (!project.email) {
        console.log(
          `📧 No email address for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
        );
        continue;
      }
      ...
      if (!project.stageEnteredAt) {
        console.log(
          `❌ No stageEnteredAt date for contact ${project.firstName} ${project.lastName}, skipping`,
        );
        continue;
      }
      ...
      const shouldSendAt = new Date(
        stageEnteredAt.getTime() + delayMinutes * 60 * 1000,
      );
      const now = new Date();
      ...
      if (now < shouldSendAt) {
        console.log(
          `⏰ Too early to send for ${project.firstName} ${project.lastName}. Should send at: ${shouldSendAt}, now: ${now}`,
        );
        continue;
      }
      ...
      if (!reservation.canExecute) {
        console.log(
          `🔒 Automation already reserved/executed for ${project.firstName} ${project.lastName}, prevented duplicate (bulletproof reservation)`,
        );
        continue;
      }
      ...
      if (!useTemplateBody) {
        ...
        try {
          emailBlocks =
            typeof automation.emailBlocks === "string"
              ? JSON.parse(automation.emailBlocks)
              : automation.emailBlocks || [];
        } catch (error) {
          console.error(
            `❌ Failed to parse emailBlocks for automation ${automation.name}:`,
            error,
          );
          await updateExecutionStatus(reservation.executionId!, "FAILED");
          continue;
        ...
        for (const block of emailBlocks) {
          if (
            block.type === "BUTTON" &&
            block.content?.linkType === "SMART_FILE" &&
            block.content?.linkValue
          ) {
            ...
            if (!smartFileTemplateId) {
              console.error(
                `❌ Smart File linkValue is null or undefined in button block`,
              );
              continue;
            }
            ...
            if (existingProjectSmartFile.length > 0) {
              ...
              console.log(
                `📄 Using existing Smart File token: ${smartFileToken}`,
              );
            } else {
              ...
              if (!smartFileTemplate) {
                console.error(
                  `❌ Smart File template not found: ${smartFileTemplateId}`,
                );
                continue;
              }
              ...
              console.log(
                `📄 Created new Smart File with token: ${smartFileToken}`,
              );
            }
      ...
      const galleryVars = await getGalleryVariables(
        project.id,
      ...
      const variables = {
        ...
        smart_file_link: smartFileToken
          ? `${baseUrl}/smart-file/${smartFileToken}`
          : "",
        calendar_link: photographer?.portalSlug
          ? `${baseUrl}/book`
          : photographer?.publicToken
            ? `${baseUrl}/booking/calendar/${photographer.publicToken}`
            : "",
      ...
      if (useTemplateBody) {
        ...
        const { generateEmailHeader, generateEmailSignature } = await import(
          "@shared/email-branding-shared"
        );
        ...
        bodyHtml = bodyHtml.replace(
          /\[\[BUTTON:([^:]+):([^\]]+)\]\]/g,
          (_match: string, type: string, label: string) => {
            let href = "#";
            if (type === "CALENDAR") href = variables.calendar_link || "#";
            else if (type === "SMART_FILE")
              href = variables.smart_file_link || "#";
            else if (type === "TESTIMONIALS")
              href = variables.testimonials_link || "#";
            else if (type === "GALLERY")
              href = (galleryVars as any).gallery_link || "#";
        ...
        console.log(
          `📧 Using templateBody pattern for automation: ${automation.name}`,
        );
      } else {
        ...
        console.log(
          `📧 Using legacy emailBlocks pattern for automation: ${automation.name}`,
        );
      }
      ...
      const fromEmail =
        process.env.SENDGRID_FROM_EMAIL || "scoop@missionscoopable.com";
      const fromName =
      ...
      try {
        ...
        console.log(
          `✅ Email sent successfully to ${project.firstName} ${project.lastName}`,
        );
        await updateExecutionStatus(reservation.executionId!, "SUCCESS");
      } catch (error) {
        console.error(
          `❌ Failed to send email to ${project.firstName} ${project.lastName}:`,
          error,
        );
        await updateExecutionStatus(reservation.executionId!, "FAILED");
      }
      ...
      console.error(
        `❌ Error processing email builder for ${project.firstName} ${project.lastName}:`,
        error,
      );
    }
  }
...
async function processStageChangeAutomation(
  automation: any,
  photographerId: string,
): Promise<void> {
  ...
  if (businessTriggers.length === 0) {
    console.log(
      `No enabled business triggers found for automation ${automation.name}, skipping`,
    );
    return;
  }

  console.log(
    `Found ${businessTriggers.length} enabled business triggers for automation: ${businessTriggers.map((t) => t.triggerType).join(", ")}`,
  );
  ...
  console.log(
    `Found ${activeProjectRows.length} active projects to check for business triggers`,
  );
  ...
  for (const row of activeProjectRows) {
    ...
    for (const businessTrigger of businessTriggers) {
      ...
      const effectiveFrom = automation.effectiveFrom
        ? new Date(automation.effectiveFrom)
        : undefined;
      const shouldTrigger = await checkTriggerCondition(
        businessTrigger.triggerType,
        project,
        photographerId,
        effectiveFrom,
      );
      ...
      if (
        businessTrigger.minAmountCents &&
        !(await checkMinAmountConstraint(
          project.id,
          businessTrigger.minAmountCents,
        ))
      ) {
        console.log(
          `Min amount constraint not met for business trigger ${businessTrigger.triggerType}, skipping`,
        );
        continue;
      }

      if (
        businessTrigger.projectType &&
        project.projectType !== businessTrigger.projectType
      ) {
        console.log(
          `Project type constraint not met for business trigger ${businessTrigger.triggerType}, skipping`,
        );
        continue;
      }
      ...
      if (!reservation.canExecute) {
        console.log(
          `🔒 Stage change automation already reserved/executed for project ${project.id} (trigger: ${businessTrigger.triggerType}), prevented duplicate`,
        );
      ...
      try {
        ...
        console.log(
          `✅ Business trigger "${businessTrigger.triggerType}" matched for project ${project.id}, moving to stage ${automation.targetStageId}`,
        );
        ...
        console.error(
          `❌ Error executing stage change for project ${project.id}:`,
          error,
        );
...
async function processAutomationStep(
  ...
  console.log(
    `Processing step for ${project.firstName} ${project.lastName} (${project.email}), project: ${projectId}`,
  );
  ...
  if (anchorType === "STAGE_ENTRY") {
    if (!project.stageEnteredAt) {
      console.log(
        `❌ No stageEnteredAt date for project ${projectId}, skipping`,
      );
      return;
    }
  }
  ...
  if (anchorType === "BOOKING_START" || anchorType === "BOOKING_END") {
    ...
    console.log(
      `📅 Fetching bookings for project ${projectId} with anchor type ${anchorType}`,
    );
    ...
    if (projectBookings.length === 0) {
      console.log(
        `📅 No active booking found for project ${projectId}, skipping booking-relative automation`,
      );
      return;
    }

    console.log(
      `📅 Found ${projectBookings.length} booking(s) for project ${projectId}`,
    );

    // Use the first upcoming booking, or the most recent one if all are in the past
    const now = new Date();
    booking =
      projectBookings.find((b) => new Date(b.startAt) > now) ||
      projectBookings[projectBookings.length - 1];

    if (anchorType === "BOOKING_START") {
      anchorTime = new Date(booking.startAt);
      console.log(
        `📅 Using booking start time as anchor: ${anchorTime.toISOString()} (booking: ${booking.id})`,
      );
    } else {
      anchorTime = new Date(booking.endAt);
      console.log(
        `📅 Using booking end time as anchor: ${anchorTime.toISOString()} (booking: ${booking.id})`,
      );
    }
  } else {
  ...
  if (automation.effectiveFrom && project.stageEnteredAt) {
    ...
    if (stageEnteredAt < effectiveFrom) {
      console.log(
        `⏸️ Project entered stage before automation was created (entered: ${stageEnteredAt.toISOString()}, effective: ${effectiveFrom.toISOString()}), skipping`,
      );
      return;
    }
  }
  ...
  if (automation.eventDateCondition) {
    if (
      automation.eventDateCondition === "HAS_EVENT_DATE" &&
      !project.eventDate
    ) {
      console.log(
        `📅 ${project.firstName} ${project.lastName} does not have event date (required by automation), skipping`,
      );
      return;
    }
    if (
      automation.eventDateCondition === "NO_EVENT_DATE" &&
      project.eventDate
    ) {
      console.log(
        `📅 ${project.firstName} ${project.lastName} has event date (automation requires no date), skipping`,
      );
      return;
    }
  }
  ...
  if (
    ...
    const triggerInTz = getDateTimeInTimezone(
      stageEnteredAt,
      photographerTimezone,
    );
    ...
    console.log(
      `📅 Day-based scheduling: ${step.delayDays} day(s) @ ${step.sendAtHour}:${String(step.sendAtMinute || 0).padStart(2, "0")} in ${photographerTimezone}`,
    );
    console.log(
      `   Trigger: ${stageEnteredAt.toISOString()} → Target: ${shouldSendAt.toISOString()}`,
    );
  ...
  if (step.quietHoursStart && step.quietHoursEnd) {
    ...
    if (inQuietHours) {
      console.log(
        `🌙 In quiet hours for ${project.firstName} ${project.lastName}, current hour: ${currentHour}`,
      );
  ...
  if (actionType === "SMS" && !project.smsOptIn) {
    console.log(
      `📱 SMS opt-in missing for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
    );
    return;
  }
  ...
  if (actionType === "SMS" && !project.phone) {
    console.log(
      `📱 No phone number for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
    );
    return;
  }
  if (actionType === "SMART_FILE" && !project.email) {
    console.log(
      `📄 No email address for Smart File notification for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
    );
    return;
  }
  ...
  if (actionType === "SMART_FILE") {
    if (!step.smartFileTemplateId) {
      console.log(
        `📄 No Smart File template ID for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
      );
      return;
    }
    ...
    if (!smartFileTemplate) {
      console.log(
        `📄 Smart File template not found for ${project.firstName} ${project.lastName}, templateId: ${step.smartFileTemplateId}, skipping (no reservation)`,
      );
      return;
    }
  } else {
    ...
    if (actionType === "SMS") {
      if (!step.templateId && !step.customSmsContent) {
        console.log(
          `📝 No template ID or custom SMS content for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
        );
        return;
      }
      ...
      if (step.templateId) {
        ...
        if (!template) {
          console.log(
            `📝 Template not found for ${project.firstName} ${project.lastName}, templateId: ${step.templateId}, skipping (no reservation)`,
          );
          return;
        }
        ...
        if (template.channel !== actionType) {
          console.log(
            `❌ Template channel mismatch for automation ${automation.name}: template=${template.channel}, action=${actionType}, skipping (no reservation)`,
          );
          return;
        }
      }
      ...
      if (!step.templateId) {
        console.log(
          `📝 No template ID for ${project.firstName} ${project.lastName}, skipping (no reservation)`,
        );
        return;
      }
  ...
  console.log(
    `🎯 COMM AUTOMATION EXECUTING: actionType=${actionType}, stepId=${step.id}, projectId=${project.id}`,
  );

  // Use centralized portal URL resolver for other links
  const baseUrl = resolvePortalBaseUrl(
    photographer?.portalSlug,
    "processAutomationStep-legacy",
  );
  ...
  const isRailway =
    !!process.env.RAILWAY_PROJECT_ID || !!process.env.RAILWAY_ENVIRONMENT_NAME;
  ...
  if (photographer?.portalSlug) {
    ...
    console.warn(
      `⚠️ scheduler_link: No portalSlug or publicToken for photographer ${photographer?.id}`,
    );
  }
  ...
  const calculateDaysUntilEvent = (
    eventDateValue: any,
  ): { days: number; weeks: number; formatted: string } => {
  ...
  try {
    ...
    if (actionType === "SMART_FILE" && smartFileTemplate && project.email) {
      ...
      console.log(
        `📄 Creating Smart File for ${project.firstName} ${project.lastName} from template "${smartFileTemplate.name}"...`,
      );
      ...
      const sfBaseUrl = resolvePortalBaseUrl(
        photographer?.portalSlug,
        "processAutomationStep-smartFile",
      );
      ...
      console.log(
        `📧 Sending Smart File notification email to ${project.email}...`,
      );
      ...
      console.log(
        `📄 Smart File ${success ? "sent successfully" : "FAILED"} to ${project.firstName} ${project.lastName}`,
      );
      ...
      if (automation.smartFileTemplateId) {
        console.log(
          `📄 Automation includes Smart File template: ${automation.smartFileTemplateId}`,
        );
        ...
        if (sfTemplate) {
          console.log(
            `📄 Creating Smart File "${sfTemplate.name}" for email automation...`,
          );
          ...
          const token = generateToken();
          // Use centralized portal URL resolver for Smart File URL
          const sfBaseUrl2 = resolvePortalBaseUrl(
            photographer?.portalSlug,
            "processAutomationStep-emailSmartFile",
          );
          ...
          console.log(
            `⚠️ Smart File template not found: ${automation.smartFileTemplateId}`,
          );
        }
      }
      ...
      console.log(
        `📧 Sending email to ${project.firstName} ${project.lastName} (${project.email})...`,
      );
      ...
      if (participantEmails.length > 0) {
        console.log(
          `📧 Including ${participantEmails.length} participants in BCC`,
        );
      }
      ...
      console.log(
        `📧 DEBUG: Using verified sender: ${fromEmail}, reply-to: ${replyToEmail}`,
      );
      ...
      console.log(
        `📧 Email ${success ? "sent successfully" : "FAILED"} to ${project.firstName} ${project.lastName}`,
      );
      ...
      actionType === "SMS" &&
      project.phone &&
      (template || step.customSmsContent)
    ) {
      ...
      console.log(
        `📱🔍 SMS DEBUG - rendered body preview:`,
        body.substring(0, 200),
      );

      console.log(
        `📱 Sending SMS to ${project.firstName} ${project.lastName}: ${step.customSmsContent ? "custom message" : "template"}`,
      );
    ...
    console.error(
      `❌ Error sending communication message to ${project.firstName} ${project.lastName}:`,
      error,
    );
    // 🔒 BULLETPROOF ERROR HANDLING - Mark reservation as FAILED on any error to prevent PENDING state
    await updateExecutionStatus(reservation.executionId!, "FAILED");
...
async function checkTriggerCondition(
  ...
  console.log(
    `Checking trigger condition: ${triggerType} for project ${project.id}${effectiveFrom ? ` (effectiveFrom: ${effectiveFrom.toISOString()})` : ""}`,
  );
    ...
    if (smartFile.amountPaidCents && smartFile.amountPaidCents > 0) {
      ...
      if (effectiveFrom && smartFile.paidAt) {
        if (new Date(smartFile.paidAt) < effectiveFrom) {
          console.log(
            `⏸️ Deposit paid before automation was created (paid: ${smartFile.paidAt}, effective: ${effectiveFrom.toISOString()}), skipping`,
          );
          continue;
        }
      }
...
async function checkFullPaymentMade(
  projectId: string,
  effectiveFrom?: Date,
): Promise<boolean> {
  ...
  for (const smartFile of paidSmartFiles) {
    ...
    if (effectiveFrom && smartFile.paidAt) {
      if (new Date(smartFile.paidAt) < effectiveFrom) {
        console.log(
          `⏸️ Full payment made before automation was created (paid: ${smartFile.paidAt}, effective: ${effectiveFrom.toISOString()}), skipping`,
        );
        continue;
      }
    }
...
async function checkAnyPaymentMade(
  projectId: string,
  effectiveFrom?: Date,
): Promise<boolean> {
    ...
    if (sf.clientSignedAt || sf.photographerSignedAt) {
      ...
      if (effectiveFrom && signedAt) {
        if (new Date(signedAt) < effectiveFrom) {
          console.log(
            `⏸️ Contract signed before automation was created (signed: ${signedAt}, effective: ${effectiveFrom.toISOString()}), skipping`,
          );
          continue;
        }
      }
...
async function checkSmartFileAccepted(
  projectId: string,
  effectiveFrom?: Date,
): Promise<boolean> {
...
async function checkSmartFileSent(
  projectId: string,
  effectiveFrom?: Date,
): Promise<boolean> {
  ...
  for (const sf of sentSmartFiles) {
    ...
    if (effectiveFrom && sf.sentAt) {
      if (new Date(sf.sentAt) < effectiveFrom) {
        console.log(
          `⏸️ Smart File sent before automation was created (sent: ${sf.sentAt}, effective: ${effectiveFrom.toISOString()}), skipping`,
        );
        continue;
      }
    }
...
async function checkAppointmentBooked(project: any): Promise<boolean> {
  ...
  const confirmedBookings = await db
    ...
    .from(bookings)
    .where(
      and(eq(bookings.projectId, project.id), eq(bookings.status, "CONFIRMED")),
    );
  return confirmedBookings.length > 0;
}

async function checkGalleryShared(
  projectId: string,
  effectiveFrom?: Date,
): Promise<boolean> {
  ...
  const sharedGalleries = await db
    ...
    .from(galleries)
    .where(
      and(eq(galleries.projectId, projectId), isNotNull(galleries.sharedAt)),
    );
  ...
  for (const gallery of sharedGalleries) {
    ...
    if (effectiveFrom && gallery.sharedAt) {
      if (new Date(gallery.sharedAt) < effectiveFrom) {
        console.log(
          `⏸️ Gallery shared before automation was created (shared: ${gallery.sharedAt}, effective: ${effectiveFrom.toISOString()}), skipping`,
        );
        continue;
      }
    }
...
async function checkMinAmountConstraint(
  projectId: string,
  ...
  const estimatesWithPayments = await db
    ...
    .from(estimates)
    .where(
      and(eq(estimates.projectId, projectId), eq(estimates.status, "SIGNED")),
    );
...
async function processCountdownAutomation(
  automation: any,
  photographerId: string,
): Promise<void> {
  console.log(
    `Processing countdown automation: ${automation.name} (${automation.daysBefore} days before ${automation.eventType || "event"})`,
  );

  // Validate daysBefore is a non-negative integer (allow 0 for same-day triggers)
  if (
    automation.daysBefore == null ||
    automation.daysBefore < 0 ||
    !Number.isInteger(automation.daysBefore)
  ) {
    console.log(
      `❌ Invalid daysBefore value for automation ${automation.name}: ${automation.daysBefore}`,
    );
    return;
  }
  ...
  const hasEmailContent =
    ...
    (automation.useEmailBuilder &&
      (automation.emailBlocks || automation.templateBody));

  if (!automation.eventType) {
    console.log(
      `❌ Missing eventType for countdown automation ${automation.name}`,
    );
    return;
  }
  ...
  if (!isStageChangeCountdown && !hasEmailContent) {
    console.log(
      `❌ Missing required fields for communication countdown automation ${automation.name}: templateId=${automation.templateId}, useEmailBuilder=${automation.useEmailBuilder}`,
    );
    return;
  }
  ...
  console.log(
    `Found ${activeProjects.length} active projects to check for countdown automation (stage filter: ${automation.stageCondition || "none"})`,
  );
  ...
  for (const project of activeProjects) {
      ...
      case "BOOKING_END":
        ...
        if (projectBookings.length > 0) {
          ...
          if (automation.eventType === "BOOKING_START") {
            ...
            console.log(
              `📞 Using booking start time: ${eventDate.toISOString()} (booking: ${booking.id})`,
            );
          } else {
            eventDate = new Date(booking.endAt);
            console.log(
              `📞 Using booking end time: ${eventDate.toISOString()} (booking: ${booking.id})`,
            );
          }
        } else {
          console.log(
            `📞 No active booking found for project ${project.id}, skipping booking-relative automation`,
          );
        }
        break;
      case "EVENT_DATE":
    ...
    if (automation.eventDateCondition) {
      if (automation.eventDateCondition === "HAS_EVENT_DATE" && !eventDate) {
        console.log(
          `📅 Project ${project.id} does not have event date (required by automation), skipping`,
        );
        continue;
      }
      if (automation.eventDateCondition === "NO_EVENT_DATE" && eventDate) {
        console.log(
          `📅 Project ${project.id} has event date (automation requires no date), skipping`,
        );
        continue;
      }
    }
    ...
    if (!eventDate) {
      console.log(
        `⏰ No ${automation.eventType || "event date"} for project ${project.id}, skipping countdown automation`,
      );
      continue;
    }
    ...
    if (automation.effectiveFrom && project.stageEnteredAt) {
      ...
      if (stageEnteredAt < effectiveFrom) {
        console.log(
          `⏸️ Project ${project.id} entered stage before automation was created (entered: ${stageEnteredAt.toISOString()}, effective: ${effectiveFrom.toISOString()}), skipping`,
        );
        continue;
      }
    }
    ...
    if (shouldTrigger) {
      ...
      console.log(
        `🎯 Countdown trigger for project ${project.id}: ${daysOffset} days ${timingLabel}${orLessLabel} ${eventDateOnly.toLocaleDateString()} (${automation.eventType}, timezone: ${timezone}), days from event: ${daysFromEvent}`,
      );
...
async function executeCountdownStageChange(
  ...
  console.log(
    `🎯 Executing countdown stage change for project ${project.id}: moving to stage ${automation.targetStageId}`,
  );

  // Skip if project is already in the target stage
  if (project.stageId === automation.targetStageId) {
    console.log(
      `📌 Project ${project.id} is already in target stage ${automation.targetStageId}, skipping`,
    );
    return;
  }

  // Calculate event date key for tracking
  const eventDateParts = getDateInTimezone(
  ...
  if (!reservation.canExecute) {
    console.log(
      `🔒 Countdown stage change already executed for project ${project.id} (event: ${eventDateKey}), skipping`,
    );
    return;
  }
  ...
  try {
    ...
    if (reservation.executionId) {
      await updateAutomationExecutionStatus(
        reservation.executionId,
        "completed",
        null,
      );
    ...
    console.log(
      `✅ Successfully moved project ${project.id} to stage "${targetStageName}" (countdown stage change)`,
    );
  } catch (error) {
    console.error(
      `❌ Error executing countdown stage change for project ${project.id}:`,
      error,
    );
    if (reservation.executionId) {
      await updateAutomationExecutionStatus(
...
async function sendCountdownMessage(
  ...
  console.log(
    `Sending countdown message to ${project.firstName} ${project.lastName}`,
  );
  ...
  if (!hasEmailBlocks && !hasTemplate) {
    console.log(
      `📝 No email content (template or email blocks) for countdown automation ${automation.name}, skipping (no reservation)`,
    );
    return;
  }
  ...
  if (hasTemplate) {
    ...
    if (!templateRecord) {
      console.log(
        `📝 Template not found for countdown automation, templateId: ${automation.templateId}, skipping (no reservation)`,
      );
      return;
    }
    ...
    if (templateRecord.channel !== automation.channel) {
      console.log(
        `❌ Template channel mismatch for countdown automation ${automation.name}: template=${templateRecord.channel}, automation=${automation.channel}, skipping (no reservation)`,
      );
      return;
    }
  ...
  if (!reservation.canExecute) {
    console.log(
      `🔒 Countdown automation already reserved/executed for ${project.firstName} ${project.lastName} (event: ${eventDateKey}), prevented duplicate (bulletproof reservation)`,
    );
    return;
  }

  // Calculate days remaining using photographer's timezone (reuse existing variables)
  const eventDateInTz = new Date(
    eventDateParts.year,
    eventDateParts.month,
    eventDateParts.day,
  );
  const todayDateInTz = new Date(
    todayParts.year,
    todayParts.month,
    todayParts.day,
  );
  const daysRemaining = Math.ceil(
    (eventDateInTz.getTime() - todayDateInTz.getTime()) / (1000 * 60 * 60 * 24),
  );
  ...
  const baseUrl = resolvePortalBaseUrl(
    photographer?.portalSlug,
    "processCountdownAutomation",
  );
  const isRailwayCountdown =
    !!process.env.RAILWAY_PROJECT_ID || !!process.env.RAILWAY_ENVIRONMENT_NAME;
  const isProductionCountdown =
    process.env.NODE_ENV === "production" || isRailwayCountdown;
  ...
  if (photographer?.portalSlug) {
    ...
    console.log(
      `📅 countdown scheduler_link (token fallback): ${schedulingLink}`,
    );
  } else {
    console.warn(
      `⚠️ countdown scheduler_link: No portalSlug or publicToken for photographer ${photographer?.id}`,
    );
  }
  ...
  try {
    ...
    if (automation.channel === "EMAIL" && project.email) {
      ...
      if (hasEmailBlocks) {
        ...
        const { contentBlocksToHtml, renderTemplate: renderTemplateFn } =
          await import("@shared/template-utils");
        ...
        const blocksBaseUrl = resolvePortalBaseUrl(
          photographer?.portalSlug,
          "processCountdownAutomation-blocks",
        );
        const blocksHtml = contentBlocksToHtml(
        ...
        console.error(
          "No email content available (neither blocks nor template)",
        );
        await updateExecutionStatus(reservation.executionId!, "FAILED");
        return;
      }
      ...
      console.log(
        `📧 Sending countdown email to ${project.firstName} ${project.lastName} (${project.email})...`,
      );
      ...
      console.log(
        `📧 Countdown email ${success ? "sent successfully" : "FAILED"} to ${project.firstName} ${project.lastName}`,
      );
      ...
      console.log(
        `📱 Sending countdown SMS to ${project.firstName} ${project.lastName} (${project.phone})...`,
      );
      ...
      console.log(
        `📱 Countdown SMS ${result.success ? "sent successfully" : "FAILED"} to ${project.firstName} ${project.lastName}`,
      );
    ...
    console.error(
      `❌ Error sending countdown message to ${project.firstName} ${project.lastName}:`,
      error,
    );
    // 🔒 BULLETPROOF ERROR HANDLING - Mark reservation as FAILED on any error to prevent PENDING state
    await updateExecutionStatus(reservation.executionId!, "FAILED");
...
async function moveProjectToStage(
  projectId: string,
  ...
  try {
    ...
    console.log(
      `✅ Successfully moved project ${projectId} to stage ${targetStageId}`,
    );
  } catch (error) {
    console.error(
      `❌ Failed to move project ${projectId} to stage ${targetStageId}:`,
      error,
    );
  }
}

async function processQuestionnaireAssignment(
  project: any,
  automation: any,
  photographerId: string,
): Promise<void> {
  console.log(
    `Processing questionnaire assignment for ${project.firstName} ${project.lastName} (automation: ${automation.name})`,
  );

  if (!automation.questionnaireTemplateId) {
    console.log(
      `❌ No questionnaire template ID for automation ${automation.name}`,
    );
    return;
  }
  ...
  if (!project.stageEnteredAt) {
    console.log(
      `❌ No stageEnteredAt date for project ${project.id}, skipping questionnaire assignment`,
    );
    return;
  }
  ...
  try {
    const existingAssignments = await db
      ...
      .where(
        and(
          ...
          eq(
            projectQuestionnaires.templateId,
            automation.questionnaireTemplateId,
          ),
        ),
      );
    ...
    if (existingAssignments.length > 0) {
      console.log(
        `🚫 Questionnaire already assigned to ${project.firstName} ${project.lastName} for this template`,
      );
      return;
    }
    ...
    console.log(
      `📋 Successfully assigned questionnaire to ${project.firstName} ${project.lastName}`,
    );
  } catch (error) {
    // 🚫 BULLETPROOF ERROR HANDLING - Don't crash automation system on database schema issues
    if (error?.code === "42703") {
      console.log(
        `⚠️ Database schema issue for questionnaire assignment (${error.message}). Skipping questionnaire assignment for ${project.firstName} ${project.lastName} to prevent automation system crash.`,
      );
    } else {
      console.error(
        `❌ Failed to assign questionnaire to ${project.firstName} ${project.lastName}:`,
        error,
      );
    }
  }
 ...
 */
export async function processPhotographerAppointmentReminders(
  photographerId: string,
): Promise<void> {
  try {
    ...
    const windowStart = new Date(
      now.getTime() + Math.max(1, reminderMinutes - windowBuffer) * 60000,
    );
    ...
    console.log(
      `📧 Processing ${upcomingBookings.length} appointment reminder(s) for photographer ${photographerId}`,
    );
    ...
    for (const booking of upcomingBookings) {
      try {
        ...
        console.log(
          `✅ Sent appointment reminder for booking ${booking.id} to ${photographerUser.email}`,
        );
      } catch (error) {
        console.error(
          `❌ Failed to send appointment reminder for booking ${booking.id}:`,
          error,
        );
      }
    }
    ...
    console.error(
      `❌ Error processing appointment reminders for photographer ${photographerId}:`,
      error,
    );
  }
}

async function processNurtureAutomation(
  automation: any,
  photographerId: string,
): Promise<void> {
  ...
  console.log(
    `Found ${activeCampaigns.length} active drip campaigns for project type ${automation.projectType}`,
  );
...
async function processNurtureCampaign(
  campaign: any,
  photographerId: string,
): Promise<void> {
...
async function subscribeNewProjectsToCampaign(
  campaign: any,
  photographerId: string,
): Promise<void> {
  ...
  if (campaign.isGlobal) {
    ...
    campaign.targetStageIds &&
    Array.isArray(campaign.targetStageIds) &&
    ...
    console.log(
      `Campaign ${campaign.name} has no stage configuration, skipping`,
    );
    return;
  }
  ...
  console.log(
    `Found ${eligibleProjects.length} eligible projects for campaign ${campaign.name}`,
  );
  ...
  for (const sub of pausedSubscriptions) {
    console.log(
      `▶️ Resuming paused subscription for project ${sub.projectId} (${sub.firstName} ${sub.lastName}) - back in target stages`,
    );
    await db
  ...
  for (const project of eligibleProjects) {
    ...
    if (!project.email) {
      console.log(
        `Project ${project.id} has no email address, skipping subscription`,
      );
      continue;
    }
    ...
    if (
      firstEmail?.delayMinutes !== null &&
      firstEmail?.delayMinutes !== undefined
    ) {
    ...
    console.log(
      `✅ Subscribed project ${project.id} (${project.firstName} ${project.lastName}) to campaign ${campaign.name}`,
    );
  }
}

async function processExistingSubscriptions(
  campaign: any,
  photographerId: string,
): Promise<void> {
  const now = new Date();
  ...
  console.log(
    `Found ${readySubscriptionRows.length} subscriptions ready for next email in campaign ${campaign.name}`,
  );
...
async function processSubscriptionEmail(
  subscription: any,
  campaign: any,
  project: any,
  contact: any,
  ...
  console.log(
    `Processing subscription email for ${contact.firstName} ${contact.lastName}, next email index: ${subscription.nextEmailIndex}`,
  );
  ...
  if (!campaign.isGlobal) {
    ...
    if (!isInTargetStage) {
      console.log(
        `⏸️ Project ${project.id} has left target stages for campaign ${campaign.name}, pausing subscription`,
      );
      await db
        .update(dripCampaignSubscriptions)
        .set({ status: "PAUSED" })
  ...
  if (nextEmail.length === 0) {
    console.log(
      `No more emails in sequence for subscription ${subscription.id}, completing campaign`,
    );
  ...
  if (existingDelivery.length > 0) {
    console.log(
      `Email ${emailToSend.sequenceIndex} already delivered for subscription ${subscription.id}, skipping`,
    );
    return;
  }
  ...
  if (project.eventDate) {
    ...
    if (eventDate <= now) {
      console.log(
        `Event date has passed for project ${project.id}, completing drip campaign`,
      );
      await db
        .update(dripCampaignSubscriptions)
        .set({
  ...
  if (now.getTime() - subscriptionStartDate.getTime() > maxDurationMs) {
    console.log(
      `Max duration reached for subscription ${subscription.id}, completing campaign`,
    );
    await db
      .update(dripCampaignSubscriptions)
      .set({
  ...
  const baseUrl = resolvePortalBaseUrl(
    photographer?.portalSlug,
    "processDripCampaignEmails",
  );
  const isRailwayDrip =
    !!process.env.RAILWAY_PROJECT_ID || !!process.env.RAILWAY_ENVIRONMENT_NAME;
  const isProductionDrip =
    process.env.NODE_ENV === "production" || isRailwayDrip;
  ...
  if (photographer?.portalSlug) {
    ...
    console.warn(
      `⚠️ drip scheduler_link: No portalSlug or publicToken for photographer ${photographer?.id}`,
    );
  }
  ...
  if (emailToSend.useEmailBuilder && emailToSend.emailBlocks) {
    const { contentBlocksToHtml } = await import(
    ...
    try {
      const blocks =
        typeof emailToSend.emailBlocks === "string"
      ...
      const dripBlocksBaseUrl = resolvePortalBaseUrl(
        photographer?.portalSlug,
        "processDripCampaignEmails-blocks",
      );
      ...
      console.log(
        "📧 Email builder content (no wrapper), length:",
        rawBlocksHtml.length,
      );
      ...
      const headerHtml =
        shouldIncludeHeader && headerStyleToUse
          ? generateEmailHeader(headerStyleToUse, brandingData)
          : "";

      const signatureHtml =
        shouldIncludeSignature && signatureStyleToUse
...
<body style="margin: 0 !important; padding: 0 !important; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif !important; background-color: #ffffff !important;">
  <div style="max-width: 600px; margin: 0 auto;">
    ...
    const shouldIncludeHeader =
      emailToSend.includeHeader ||
      (!emailToSend.includeHeader && photographer?.emailHeaderStyle);
    const headerStyleToUse =
      emailToSend.headerStyle || photographer?.emailHeaderStyle;

    const shouldIncludeSignature =
      emailToSend.includeSignature ||
      (!emailToSend.includeSignature && photographer?.emailSignatureStyle);
    const signatureStyleToUse =
      emailToSend.signatureStyle || photographer?.emailSignatureStyle;
  ...
  try {
    // Send email
    console.log(
      `📧 Sending drip email to ${contact.firstName} ${contact.lastName} (${contact.email})...`,
    );
    ...
    const fromName =
      ...
      "Scoop Photography";
    const replyToEmail =
      photographer?.emailFromAddr || process.env.SENDGRID_REPLY_TO || fromEmail;
    ...
    if (success) {
      ...
      if (
        ...
        nextEmailAt = new Date(
          subscriptionStart.getTime() + emailDays * 24 * 60 * 60 * 1000,
        );
        ...
        const minScheduleTime = new Date(
          now.getTime() +
            campaign.emailFrequencyWeeks * 7 * 24 * 60 * 60 * 1000,
        );
      ...
      console.log(
        `✅ Sent drip email ${emailToSend.sequenceIndex} to ${contact.firstName} ${contact.lastName}. Next email scheduled for ${nextEmailAt}`,
      );
    } else {
      ...
      console.log(
        `❌ Failed to send drip email to ${contact.firstName} ${contact.lastName}`,
      );
    }
  } catch (error) {
    console.error(
      `❌ Error sending drip email to ${contact.firstName} ${contact.lastName}:`,
      error,
    );
 ...
 */
export async function onBookingUpdated(
  bookingId: string,
  projectId: string,
  oldStartAt?: Date,
  oldEndAt?: Date,
  ...
  console.log(
    `✅ Cleared pending booking-relative automations for project ${projectId}`,
  );
}
...
export async function onBookingCancelled(
  ...
  console.log(
    `📅 Booking cancelled/deleted: ${bookingId} for project ${projectId}`,
  );
  ...
  console.log(
    `✅ Cancelled pending booking-relative automations for project ${projectId}`,
  );
}
 ...
 */
async function cancelPendingBookingAutomations(
  projectId: string,
): Promise<void> {
  try {
    // Find all automation steps that use booking-relative anchors
    const bookingRelativeSteps = await db
      .select({ stepId: automationSteps.id })
      .from(automationSteps)
      .where(
        or(
    ...
    console.log(
      `🔄 Cancelled ${stepIds.length} booking-relative automation executions for project ${projectId}`,
    );
  } catch (error) {
    console.error(
      `❌ Error cancelling booking automations for project ${projectId}:`,
      error,
    );
  }
}
...
export async function processDripCampaigns(): Promise<void> {
  try {
    ...
    console.log(
      `📧 Processing ${dueSubscriptions.length} due drip campaign emails`,
    );
    ...
    for (const subscription of dueSubscriptions) {
      try {
        ...
        if (!currentEmail) {
          ...
          console.log(
            `✅ Drip campaign completed for subscription ${subscription.id}`,
          );
        ...
        if (!client || !client.email) {
          console.error(
            `❌ Client ${subscription.clientId} has no email, skipping`,
          );
          continue;
        }
        ...
        const renderedHtml = currentEmail.htmlBody
          ? renderTemplate(currentEmail.htmlBody, templateVars)
          : undefined;
        const renderedText = currentEmail.textBody
          ? renderTemplate(currentEmail.textBody, templateVars)
          : undefined;
        ...
        const calculateNextEmailAt = async (nextEmail: typeof currentEmail) => {
          ...
          if (
            ...
            const campaign = await storage.getDripCampaign(
              subscription.campaignId,
            );
        ...
        const advanceToNextEmail = async () => {
          ...
          if (nextEmail) {
            ...
            console.log(
              `  → Next drip email (index ${nextEmailIndex}) scheduled for ${nextEmailAt.toISOString()}`,
            );
          } else {
            await storage.updateDripCampaignSubscription(subscription.id, {
              status: "COMPLETED",
        ...
        try {
          ...
          if (claimError.code === "23505") {
            console.log(
              `⏭️ Email already claimed by another worker for subscription ${subscription.id}, skipping (no-op)`,
            );
            continue;
          }
          throw claimError;
        ...
        // Send the email
        console.log(
          `📤 Sending drip email "${currentEmail.subject}" to ${client.email}`,
        );
        ...
        if (emailResult.success) {
          ...
          console.error(
            `❌ Failed to send drip email to ${client.email}: ${emailResult.error}`,
          );
        }
        ...
        console.error(
          `❌ Error processing drip subscription ${subscription.id}:`,
          subscriptionError,
        );
      }
    }
