...
export class GoogleCalendarService {
  ...
  /**
   * List all calendars the photographer has access to
   * Used for calendar selection UI in settings
   */
  async listCalendars(photographerId: string): Promise<{
    success: boolean;
    calendars?: Array<{
      id: string;
      summary: string;
  ...
  /**
   * Get busy times from specified calendars for a date range
   * Used for blocking slots that conflict with Google Calendar events
   */
  async getBusyTimes(
    photographerId: string,
    calendarIds: string[],
    ...
    try {
      ...
      for (const [calendarId, calendarData] of Object.entries(calendarsData)) {
  ...
  /**
   * Get actual calendar events with titles and details for a date range
   * Used for displaying Google Calendar events on the CRM calendar
   */
  ...
  /**
   * Check for conflicts between existing CRM bookings and Google Calendar events
   * Used during sync setup to detect scheduling conflicts
   */
  async detectBookingConflicts(
    ...
    try {
      ...
      for (const booking of futureBookings) {
        ...
        for (const busy of busyResult.busyTimes) {
          // Check for overlap: (startA < endB) && (endA > startB)
          if (bookingStart < busy.end && bookingEnd > busy.start) {
            conflicts.push({
              booking: {
                id: booking.id,
                title: booking.title,
