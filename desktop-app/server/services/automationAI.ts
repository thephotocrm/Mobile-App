...
const StageIntentSchema = z.object({
  stageId: z.string(),
  intent: z.enum(["INQUIRY", "QUALIFICATION", "PROPOSAL", "BOOKING", "PRE_EVENT", "POST_EVENT", "NURTURE"]),
 ...
 */
export async function classifyStageIntents(stages: Stage[]): Promise<StageClassification[]> {
  if (stages.length === 0) {
    return [];
  }
  ...
  if (unknownStages.length > 0) {
    const aiClassifications = await classifyUnknownStagesWithAI(unknownStages, stages.length);
 ...
 */
function getKnownStageIntent(stageName: string): { intent: StageIntent; confidence: number; reasoning: string } | null {
  const normalized = stageName.toLowerCase().trim();

  // INQUIRY patterns
  if (/^(new\s+)?(inquiry|lead|contact|interested|prospect)$/i.test(normalized) ||
      /new\s+(inquiry|lead)/i.test(normalized)) {
    return { intent: "INQUIRY", confidence: 0.95, reasoning: "Stage name indicates initial lead/inquiry stage" };
  }

  // QUALIFICATION patterns
  if (/consultation|discovery|call|meeting|scheduled|qualification|intro/i.test(normalized)) {
    return { intent: "QUALIFICATION", confidence: 0.92, reasoning: "Stage name indicates consultation/discovery stage" };
  }

  // PROPOSAL patterns
  if (/proposal|quote|estimate|sent|pricing|offer/i.test(normalized) &&
      !/booked|paid|accepted/i.test(normalized)) {
    return { intent: "PROPOSAL", confidence: 0.93, reasoning: "Stage name indicates proposal/quote stage" };
  }

  // BOOKING patterns
  if (/booked|retainer|deposit|paid|confirmed|signed|contract/i.test(normalized) &&
      !/proposal/i.test(normalized)) {
    return { intent: "BOOKING", confidence: 0.94, reasoning: "Stage name indicates booked/confirmed stage" };
  }

  // PRE_EVENT patterns
  if (/pre[- ]?(event|wedding|shoot)|planning|prep|timeline|before/i.test(normalized)) {
    return { intent: "PRE_EVENT", confidence: 0.91, reasoning: "Stage name indicates pre-event preparation stage" };
  }

  // POST_EVENT patterns
  if (/gallery|deliver|editing|edit|sneak|post[- ]?(event|wedding)|completed/i.test(normalized) &&
      !/review|referral|nurture/i.test(normalized)) {
    return { intent: "POST_EVENT", confidence: 0.90, reasoning: "Stage name indicates post-event/delivery stage" };
  }

  // NURTURE patterns
  if (/review|referral|alumni|past|nurture|follow[- ]?up|archive/i.test(normalized)) {
    return { intent: "NURTURE", confidence: 0.88, reasoning: "Stage name indicates nurture/completed client stage" };
  }
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: `Classify these pipeline stages:\n${JSON.stringify(stageData, null, 2)}` }
      ],
      response_format: zodResponseFormat(AllStagesClassificationSchema, "stage_classifications"),
      temperature: 0.2,
    });

    const content = response.choices[0].message.content;
    if (!content) {
      return unknownStages.map(s => positionBasedClassification(s, totalStages));
    }

    const parsed = JSON.parse(content) as z.infer<typeof AllStagesClassificationSchema>;
    if (!parsed || !parsed.classifications) {
      return unknownStages.map(s => positionBasedClassification(s, totalStages));
    }

    return parsed.classifications.map((c: z.infer<typeof StageIntentSchema>) => {
    ...
    console.error("AI classification failed, using position-based fallback:", error);
    return unknownStages.map(s => positionBasedClassification(s, totalStages));
  }
}
 ...
 */
function positionBasedClassification(stage: Stage, totalStages: number): StageClassification {
...
export async function generateAutomationsForStage(
  ...
  const templates = getTemplatesForIntent(classification.intent, businessName, ctx);
...
function convertTemplatesToPreviews(
  ...
  return templates.map(template => {
    ...
    const conflict = detectConflict(template, classification.stageId, existingAutomations);
    ...
    if (template.automationType === "STAGE_CHANGE" && template.targetStageName) {
      ...
      let targetStage = allStages.find(s =>
      ...
      if (!targetStage) {
        const synonymMap: Record<string, string[]> = {
          ...
          "consultation": ["consultation", "discovery", "call", "meeting", "consult"],
          "proposal": ["proposal", "quote", "estimate", "pricing", "sent"],
          "completed": ["completed", "finished", "done", "delivered", "complete"],
          "pre-event": ["pre-event", "pre-wedding", "planning", "preparation", "prep"],
          "post-event": ["post-event", "post-wedding", "editing", "delivery", "post"],
      ...
      if (!targetStageId) {
        console.warn(`Could not find target stage for "${template.targetStageName}" among stages: ${allStages.map(s => s.name).join(", ")}`);
      }
    }

    // Get email content
    const emailSubject = step?.template?.subject?.replace(/\{\{business_name\}\}/g, businessName) || null;
    const emailBody = step?.template?.htmlBody?.replace(/\{\{business_name\}\}/g, businessName) || null;
    const smsContent = step?.customSmsContent?.replace(/\{\{business_name\}\}/g, businessName) || null;
...
function detectConflict(
  ...
  for (const existing of existingAutomations) {
    ...
    if (template.automationType === "COMMUNICATION" && existing.automationType === "COMMUNICATION") {
      if (existing.channel === template.channel) {
        return { hasConflict: true, conflictId: existing.id };
      }
    }

    if (template.automationType === "STAGE_CHANGE" && existing.automationType === "STAGE_CHANGE") {
...
function getInquiryTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  return [
    {
...
<p>{{photographer_name}}</p>`,
    ...
    // Pipeline automation: Move to Consultation when they book an appointment
    {
      ...
      description: "Auto-move to Consultation stage when client books an appointment",
...
function getQualificationTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  return [
    {
...
function getProposalTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  return [
    {
...
function getBookingTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  return [
    {
...
function getPreEventTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  const isWeddingOrEvent = ctx.eventName.includes('wedding') || ctx.eventName.includes('event');
...
function getPostEventTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  return [
    {
...
function getNurtureTemplates(businessName: string, ctx: ProjectTypeContext): AutomationTemplate[] {
  return [
    {
      name: "Review request",
