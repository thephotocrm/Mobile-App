...
async function getCredentials() {
  ...
  if (process.env.REPLIT_CONNECTORS_HOSTNAME) {
    ...
    if (!xReplitToken) {
      console.warn(
        "REPLIT_CONNECTORS_HOSTNAME found but no X_REPLIT_TOKEN, falling back to env vars",
      );
    } else {
      try {
        ...
        console.warn(
          "Failed to fetch Twilio from Replit connector, falling back to env vars:",
          error,
        );
      }
    }
...
function sanitizePhoneNumber(phoneNumber: string): string {
  ...
  throw new Error(
    `Unable to format phone number: ${phoneNumber} (cleaned: ${cleaned})`,
  );
}
...
export async function sendSms(
  ...
  try {
    ...
    if (process.env.REPLIT_DEV_DOMAIN) {
      ...
      // Production: always use app.thephotocrm.com (the main CRM domain)
      // This avoids issues with wildcard domains like *.tpcportal.co
      statusCallbackUrl = "https://app.thephotocrm.com/webhooks/twilio/status";
    }
