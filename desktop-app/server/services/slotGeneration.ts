...
const googleBusyCache = new Map<
  string,
  {
    busyTimes: Array<{ start: Date; end: Date; calendarId: string }>;
    expiresAt: number;
  }
>();
...
function getCacheTTL(date: string): number {
  ...
  const daysAhead = Math.floor(
    (targetDate.getTime() - now.getTime()) / (24 * 3600 * 1000),
  );
...
export class SlotGenerationService {
  ...
  async generateSlotsForDateRange(
    ...
    const templates =
      await storage.getDailyAvailabilityTemplatesByPhotographer(photographerId);
  ...
  private async generateSlotsForDate(
    ...
    if (override) {
      ...
      const templateBreaks = await storage.getDailyAvailabilityBreaksByTemplate(
        template.id,
      );
    ...
    const availabilitySlots: InsertAvailabilitySlot[] = timeSlots.map(
      (slot) => {
   ...
   */
  async regenerateSlotsForTemplate(
    templateId: string,
  ...
  async getSlotsForDate(
    ...
    let calendarMetadata = photographer?.googleCalendarMetadata as Record<
      string,
      { name: string; color: string }
    > | null;
    if (
      ...
      if (
        ...
        (!calendarMetadata || Object.keys(calendarMetadata).length === 0)
      ) {
    ...
    const allBreaks =
      await storage.getDailyAvailabilityBreaksByPhotographer(photographerId);
    ...
    if (
      ...
      if (calendarIds.length > 0) {
        ...
        if (cached && cached.expiresAt > Date.now()) {
          ...
          const startOfDay = createDateInTimezone(
            dateString,
            "00:00",
            timezone,
          );
          ...
          if (busyResult.success && busyResult.busyTimes) {
            ...
            console.error(
              `Google Calendar API error for photographer ${photographerId}:`,
              busyResult.error,
            );
            googleCalendarError =
              busyResult.error || "Failed to fetch Google Calendar busy times";
          }
        }
      }
    }
    ...
    const allSlots = timeSlots.map((slot) => {
      ...
      const uniqueBlockingCalendarIds = Array.from(
        new Set(blockingCalendarIds),
      );
      ...
      return {
        ...
        blockedByCalendars:
          uniqueBlockingCalendarIds.length > 0
            ? uniqueBlockingCalendarIds
            : undefined,
      };
    });
  ...
  async getSlotsForDateRange(
    ...
    const allBookings = await storage.getBookingsByPhotographerInRange(
      photographerId,
      startDateStr,
      endDateStr,
    );
    ...
    while (currentDate <= endDate) {
      ...
      if (
        !earliestStartTime ||
        availabilityConfig.startTime! < earliestStartTime
      ) {
      ...
      const dateBusyTimes = googleBusyTimes.filter(
    ...
    return {
      ...
      googleCalendarEvents:
        googleCalendarEvents.length > 0 ? googleCalendarEvents : undefined,
      earliestStartTime,
      latestEndTime,
    };
  }
