...
import { setAuthCookie, clearAuthCookies, clearAuthCookie, getAuthToken } from './cookie-auth';
import { validatePortalSlug, normalizeSlug, generateSlugFromBusinessName } from "./utils/slug-validation";
...
import { slotGenerationService, invalidateGoogleCalendarCache } from "./services/slotGeneration";
import { getGoogleRedirectUri, getPhotographerAppUrl } from "./utils/oauthRedirect";
import { insertUserSchema, insertPhotographerSchema, insertContactSchema, insertStageSchema, insertProjectTypeSchema,
         ...
         emailLogs, smsLogs, projectActivityLog, projectSmartFiles, photographerEarnings, projects, automationBusinessTriggers, paymentTransactions,
         ...
         users, insertPaymentTransactionSchema } from "@shared/schema";
...
import { processAutomations, checkImmediateCountdownStageChanges } from "./services/automation";
...
const ALLOWED_PLACEHOLDERS = [
  'first_name', 'last_name', 'email', 'phone', 'business_name', 'event_date',
  'PHOTOGRAPHER_NAME', 'BUSINESS_NAME', 'SCHEDULING_LINK', // Legacy placeholders
  'CLIENT_FIRST_NAME', 'CLIENT_LAST_NAME', 'CLIENT_EMAIL', 'PROJECT_NAME'
];
...
function sanitizePlaceholders(content: string): string {
  ...
  return content.replace(placeholderRegex, (match, placeholder) => {
    ...
    console.warn(`⚠️ Removed invalid placeholder from AI content: {{${trimmed}}}`);
    return '';
  });
}
...
async function generateUniquePortalSlug(businessName: string): Promise<string> {
  ...
  const { generateSafeSlug, generateRandomSlug, validatePortalSlug } = await import('./utils/slug-validation');
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    // Check uniqueness
    const existingCandidate = await storage.getPhotographerByPortalSlug(candidateSlug);
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      for (const contact of contacts) {
        ...
        try {
          ...
          await notifyNewMessage({
            ...
            projectId: latestProject?.id,
            messagePreview: messageBody.length > 50 ? messageBody.substring(0, 50) + '...' : messageBody,
            channel: 'sms'
          });
        } catch (notifyError) {
          console.error('Failed to create inbound SMS notification:', notifyError);
        }
      }
  ...
  app.get("/s/:shortCode", async (req, res) => {
    try {
      ...
      const hasStaleUrl = targetUrl.includes('replit.dev') || 
      ...
      if (hasStaleUrl) {
        try {
          ...
          const photographer = await storage.getPhotographer(shortLink.photographerId);
          const { resolvePortalBaseUrl } = await import('./utils/portal-url');
          const correctBaseUrl = resolvePortalBaseUrl(photographer?.portalSlug, 'shortLinkRedirect');
          ...
          console.log(`🔧 Self-healing short link on click ${shortCode}: ${targetUrl} → ${newTargetUrl}`);
          ...
          console.error(`❌ Failed to self-heal short link ${shortCode}:`, healError);
    ...
    try {
      ...
      if (role === "PHOTOGRAPHER") {
        if (isBetaMode) {
          ...
          console.log(`🎉 BETA MODE: Creating photographer with unlimited access for ${normalizedEmail}`);
        ...
        console.log(`[REGISTRATION] Created default project types and stages for photographer ${photographerId}`);
    ...
    try {
      ...
      if (emailRateData) {
        if (now - emailRateData.windowStart > LOGIN_RATE_LIMIT_WINDOW) {
          ...
          console.log(`🚫 Login rate limit exceeded for email: ${normalizedEmail}`);
      ...
      if (ipRateData) {
        ...
        } else if (ipRateData.count >= LOGIN_RATE_LIMIT_MAX * 2) { // Higher limit for IP (10 attempts)
          console.log(`🚫 Login rate limit exceeded for IP: ${clientIp}`);
          return res.status(429).json({ message: "Too many login attempts from this location. Please try again in 15 minutes." });
        } else {
          ipRateData.count++;
        }
      } else {
    ...
    try {
      ...
      if (!user) {
        ...
        if (existingUser) {
          ...
          if (isBetaMode) {
            console.log(`🎉 BETA MODE: Granted unlimited access to Google OAuth user ${normalizedEmail}`);
          }

          // Create default project types and stages for new photographer
          await storage.seedDefaultProjectTypes(photographer.id);
          console.log(`[GOOGLE OAUTH] Created default project types and stages for photographer ${photographer.id}`);
          ...
          console.log(`✅ Created default scheduling availability for Google OAuth user`);
          ...
          console.log(`📝 New photographer signup via Google OAuth: ${normalizedEmail} (${photographer.businessName}) - photographer: ${photographer.id}, user: ${user.id}`);
        }
      } else {
  ...
  // Project Types
  app.get("/api/project-types", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      ...
      if (projectTypes.length === 0) {
        console.log(`[PROJECT TYPES API] No project types found for photographer ${req.user!.photographerId}, seeding defaults...`);
        projectTypes = await storage.seedDefaultProjectTypes(req.user!.photographerId!);
        console.log(`[PROJECT TYPES API] Seeded ${projectTypes.length} default project types`);
      }
  ...
  app.post("/api/project-types", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      // Auto-generate slug from name if not provided
      const slug = req.body.slug || req.body.name?.toUpperCase().replace(/[^A-Z0-9]/g, '_') || '';
      ...
      console.log(`[PROJECT TYPES API] Created project type "${projectType.name}" with ${defaultStages.length} default stages`);
  ...
  app.patch("/api/project-types/:id", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { id } = req.params;
      const projectType = await storage.getProjectType(id);
      if (!projectType || projectType.photographerId !== req.user!.photographerId!) {
  ...
  app.get("/api/project-types/:id/usage", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { id } = req.params;
      const projectType = await storage.getProjectType(id);
      if (!projectType || projectType.photographerId !== req.user!.photographerId!) {
  ...
  app.delete("/api/project-types/:id", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { id } = req.params;
      const projectType = await storage.getProjectType(id);
      if (!projectType || projectType.photographerId !== req.user!.photographerId!) {
      ...
      if (projectType.isDefault) {
        return res.status(400).json({ message: "Cannot delete the default project type" });
      }
      
      // Check usage before deletion
      const usage = await storage.getProjectTypeUsageStats(id);
      if (usage.projectCount > 0 || usage.stageCount > 0 || usage.automationCount > 0) {
  ...
  app.post("/api/project-types/:id/archive", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { id } = req.params;
      const projectType = await storage.getProjectType(id);
      if (!projectType || projectType.photographerId !== req.user!.photographerId!) {
  ...
  app.post("/api/project-types/seed", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const seeded = await storage.seedDefaultProjectTypes(req.user!.photographerId!);
  ...
  app.post("/api/project-types/reorder", authenticateToken, requirePhotographer, async (req, res) => {
    try {
  ...
  app.get("/api/stages", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      console.log(`[STAGES API] Getting stages for photographer: ${req.user!.photographerId}`);
  ...
  app.patch("/api/stages/:id", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const updated = await storage.updateStage(id, { name, color, orderIndex });
      res.json(updated);
    } catch (error) {
  ...
  app.get("/api/stages/:id/usage", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { id } = req.params;
      const stage = await storage.getStage(id);
      if (!stage || stage.photographerId !== req.user!.photographerId!) {
  ...
  app.delete("/api/stages/:id", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const allStages = await storage.getStagesByPhotographer(req.user!.photographerId!);
      if (allStages.length <= 1) {
        return res.status(400).json({ message: "Cannot delete the last stage. You need at least one stage." });
      }
      
      // Get usage stats
      const stats = await storage.getStageUsageStats(id);
      const hasUsage = stats.projectCount > 0 || stats.automationCount > 0 || stats.campaignCount > 0;
      
      // If has usage and no force flag or no reassignment target, return info about what needs attention
      if (hasUsage && !force) {
        return res.status(409).json({
          message: "Stage has dependencies that need to be handled before deletion",
          stats,
      ...
      if (stats.projectCount > 0 && reassignToStageId) {
        ...
        if (!reassignStage || reassignStage.photographerId !== req.user!.photographerId!) {
          return res.status(400).json({ message: "Invalid reassignment stage" });
        }
        // Reassign projects
  ...
  // Reorder stages
  app.post("/api/stages/reorder", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { stageOrders } = req.body;
      if (!Array.isArray(stageOrders) || stageOrders.length === 0) {
        return res.status(400).json({ message: "stageOrders must be a non-empty array" });
      }

      // Validate each item has id and orderIndex
      for (const item of stageOrders) {
        if (!item.id || typeof item.orderIndex !== 'number') {
          return res.status(400).json({ message: "Each stageOrder must have id and orderIndex" });
        }
      }
      ...
      for (const { id } of stageOrders) {
        const stage = await storage.getStage(id);
        if (!stage || stage.photographerId !== req.user!.photographerId!) {
          return res.status(403).json({ message: "Cannot reorder stages that don't belong to you" });
        }
      }
  ...
  // Global Search
  app.get("/api/search", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const query = req.query.q as string;
      const limit = Math.min(parseInt(req.query.limit as string) || 10, 50);

      console.log('[SEARCH] Query:', query, 'PhotographerId:', photographerId);

      if (!query || query.trim().length < 2) {
        return res.json({ contacts: [], projects: [], smartFiles: [] });
      }

      const results = await storage.globalSearch(photographerId, query.trim(), limit);
  ...
  app.get("/api/contacts/recent", authenticateToken, requirePhotographer, async (req, res) => {
  ...
  app.get("/api/projects/recent", authenticateToken, requirePhotographer, async (req, res) => {
  ...
  // Notifications
  app.get("/api/notifications", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      ...
      const notifications = await storage.getNotifications(photographerId, { read, limit, offset });
  ...
  app.get("/api/notifications/unread-count", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
  ...
  app.post("/api/notifications/:id/read", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { id } = req.params;
      const photographerId = req.user!.photographerId!;
  ...
  app.post("/api/notifications/mark-all-read", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
  ...
  app.delete("/api/notifications/:id", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { id } = req.params;
      const photographerId = req.user!.photographerId!;
...
${photographer?.businessName || 'Your Photography Team'}`;
  ...
  app.get("/api/projects", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      console.log('📦 DEPLOYMENT CHECK: URL-FIX-V2-DEC14 | NODE_ENV:', process.env.NODE_ENV, '| RAILWAY:', process.env.RAILWAY_ENVIRONMENT_NAME || 'not-set');
    ...
    try {
      ...
      try {
        ...
        for (const campaign of activeCampaigns) {
          ...
          const now = new Date();
          const campaignEmails = await storage.getDripCampaignEmails(campaign.id);
          ...
          if (campaignEmails.length > 0) {
            ...
            if (nextEmail.delayMinutes !== null && nextEmail.delayMinutes !== undefined) {
              ...
              console.log(`  → Using delay-based timing: ${nextEmail.delayMinutes} minutes from now`);
              ...
              console.log(`  → Using schedule-based timing: Day ${nextEmail.daysAfterStart} at ${nextEmail.sendAtHour ?? 'any'}:00`);
            } else {
  ...
  app.put("/api/projects/:id", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      const allowedFields = ['title', 'eventDate', 'venue', 'leadSource', 'smsOptIn', 'emailOptIn', 'notes', 'status', 'projectType'];
      ...
      if (updateData.eventDate) {
        ...
        ).catch(err => console.error('Error checking immediate countdown stage changes:', err));
      }
  ...
  app.patch("/api/projects/:id/archive", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { archive } = req.body;
      ...
      res.json({
        message: archive ? "Project archived successfully" : "Project restored successfully",
        project
      });
    } catch (error) {
  ...
  app.get("/api/projects/:id/payment-summary", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { id } = req.params;
      const photographerId = req.user!.photographerId!;
      ...
      const projectSmartFiles = await storage.getProjectSmartFilesByProject(id);

      // Build comprehensive payment summary
      const smartFileSummaries = await Promise.all(projectSmartFiles.map(async (psf) => {
        const smartFile = await storage.getSmartFile(psf.smartFileId);
        if (!smartFile) return null;

        // Get transactions for this smart file
        const transactions = await storage.getPaymentTransactionsByProjectSmartFile(psf.id);
        ...
        const completedTransactions = transactions.filter(t => t.status === 'SUCCEEDED' || t.status === 'COMPLETED');
        const totalPaidCents = completedTransactions.reduce((sum, t) => sum + (t.amountCents || 0), 0);
        const totalRemainingCents = Math.max(0, totalCents - totalPaidCents);

        // Find next unpaid installment
        const nextInstallment = schedule.find((inst: any) =>
        ...
        return {
          ...
          totalPaidCents,
          totalRemainingCents,
          percentComplete: totalCents > 0 ? Math.round((totalPaidCents / totalCents) * 100) : 0,
      ...
      const projectTotalCents = validSummaries.reduce((sum, sf) => sum + (sf?.totalCents || 0), 0);
      const projectTotalPaidCents = validSummaries.reduce((sum, sf) => sum + (sf?.totalPaidCents || 0), 0);
      const projectTotalRemainingCents = validSummaries.reduce((sum, sf) => sum + (sf?.totalRemainingCents || 0), 0);
      ...
      res.json({
        ...
        summary: {
          ...
          percentComplete: projectTotalCents > 0 ? Math.round((projectTotalPaidCents / projectTotalCents) * 100) : 0,
          isFullyPaid: projectTotalRemainingCents === 0 && projectTotalCents > 0
        },
  ...
  app.patch("/api/projects/:projectId/participants/:participantId", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { role } = req.body;
      const project = await storage.getProject(req.params.projectId);
  ...
  // Photographer Tags
  app.get("/api/tags", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const tags = await storage.getPhotographerTags(req.user!.photographerId!);
      res.json(tags);
    } catch (error) {
  ...
  app.post("/api/tags", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { name, color } = req.body;
      if (!name || typeof name !== 'string' || !name.trim()) {
        return res.status(400).json({ message: "Tag name is required" });
      }
      const tag = await storage.createPhotographerTag(req.user!.photographerId!, name.trim(), color);
  ...
  app.delete("/api/tags/:tagId", authenticateToken, requirePhotographer, async (req, res) => {
    try {
  ...
  // Update project tags
  app.put("/api/projects/:id/tags", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const project = await storage.getProject(req.params.id);
      if (!project) {
        return res.status(404).json({ message: "Project not found" });
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f9fafb;">
  ...
  app.post("/api/projects/:projectId/smart-files/:projectSmartFileId/send", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      ...
      const { expirationDays, expiresAt: expiresAtInput, expirationMode } = req.body;
      ...
      if (expiresAtInput) {
        ...
        const minExpiration = new Date(sentAt.getTime() + 24 * 60 * 60 * 1000);
        ...
        expiresAt = new Date(sentAt.getTime() + expirationDays * 24 * 60 * 60 * 1000);
      }
      ...
      const updatedProjectSmartFile = await storage.updateProjectSmartFile(projectSmartFileId, {
        ...
        expirationMode: expirationMode || (expiresAt ? 'UNLESS_PAYMENT' : null) // Default to UNLESS_PAYMENT if expiration is set
      });
  ...
  app.patch("/api/projects/:projectId/smart-files/:projectSmartFileId/extend", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { projectId, projectSmartFileId } = req.params;
      const { expirationDays, expiresAt: expiresAtInput, expirationMode } = req.body;
      ...
      console.log(`🔄 [EXTEND EXPIRATION] Extended projectSmartFile ${projectSmartFileId} expiration to ${expiresAt}`);
  ...
  app.post("/api/projects/:projectId/smart-files/:projectSmartFileId/mark-paid", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { projectId, projectSmartFileId } = req.params;
      const { installmentId, amountCents, paymentMethod, note, paidAt } = req.body;
      const photographerId = req.user!.photographerId!;

      // Validate required fields
      if (!installmentId || typeof amountCents !== 'number' || amountCents <= 0 || !paymentMethod || !paidAt) {
        return res.status(400).json({ message: "Missing required fields: installmentId, amountCents, paymentMethod, paidAt" });
      }
      ...
      const currentSchedule = (projectSmartFile.paymentSchedule as any[]) || [];
      const installmentIndex = currentSchedule.findIndex((inst: any) => inst.id === installmentId);

      if (installmentIndex === -1) {
        return res.status(404).json({ message: "Installment not found in payment schedule" });
      }
      ...
      if (installment.status === 'PAID') {
        return res.status(400).json({ message: "This installment is already fully paid" });
      }
      ...
      const newStatus = newPaidCents >= installment.amountCents ? 'PAID' : 'PARTIAL';
      ...
      const totalPaidCents = (projectSmartFile.amountPaidCents || 0) + amountCents;
      const totalCents = projectSmartFile.totalCents || 0;
      const allPaid = updatedSchedule.every((inst: any) => inst.status === 'PAID');
      ...
      const updatedSmartFile = await storage.updateProjectSmartFile(projectSmartFileId, {
      ...
      const transaction = await storage.createPaymentTransaction({
        ...
        paymentType: 'MANUAL',
        installmentId,
        installmentDescription: installment.description || installment.label || `Payment ${installmentIndex + 1}`,
        status: 'COMPLETED',
        notes: note || null,
        metadata: JSON.stringify({
      ...
      try {
        ...
        await handleBusinessEvent(photographerId, 'ANY_PAYMENT_MADE', project);
        ...
        if (allPaid) {
          await handleBusinessEvent(photographerId, 'FULL_PAYMENT_MADE', project);
        }
  ...
  app.post("/api/projects/:projectId/send-smart-file", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const { projectId } = req.params;
      const { templateId, expirationDays, expiresAt: expiresAtInput, expirationMode } = req.body;
      ...
      if (!contact.email) {
        return res.status(400).json({ message: "Contact email required to send Smart File" });
      }
  ...
  app.get("/api/public/smart-files/:token", async (req, res) => {
    try {
      ...
      const checkExpiration = async (): Promise<{ isExpired: boolean; reason?: string }> => {
        ...
        switch (mode) {
          ...
          case 'UNLESS_BOOKING':
            ...
            const bookings = await storage.getBookingsByProject(projectSmartFile.projectId);
      ...
      if (projectSmartFile.status === 'SENT' && !projectSmartFile.viewedAt) {
        ...
        try {
          ...
          if (contact && smartFile) {
            const { notifySmartFileViewed } = await import('./services/notifications');
          ...
          console.error('Failed to create proposal viewed notification:', notifyError);
        }
      }
  ...
  app.patch("/api/public/smart-files/:token/accept", async (req, res) => {
    try {
      const { token } = req.params;
      const { selectedPackages, selectedAddOns, subtotalCents, totalCents, depositCents, paymentSchedule, paymentScheduleMode, preview } = req.body;
      ...
      const updated = await storage.updateProjectSmartFile(projectSmartFile.id, updateData);
      ...
      try {
        ...
        const smartFile = await storage.getSmartFile(projectSmartFile.smartFileId);
        if (contact && smartFile) {
          const { notifySmartFileAccepted } = await import('./services/notifications');
        ...
        console.error('Failed to create proposal accepted notification:', notifyError);
      }
  ...
  app.patch("/api/public/smart-files/:token/sign", async (req, res) => {
    try {
      const { token } = req.params;
      const { clientSignatureUrl, contractHtml, contractTermsAccepted, preview } = req.body;
      ...
      try {
        ...
        if (client) {
          const { notifyContractSigned } = await import('./services/notifications');
        ...
        console.error('Failed to create contract signed notification:', notifyError);
      }
  ...
  app.patch("/api/public/smart-files/:token/payment-schedule", async (req, res) => {
    try {
      const { token } = req.params;
      const { paymentSchedule, selectedInstallments } = req.body;

      if (!paymentSchedule || !Array.isArray(paymentSchedule)) {
        return res.status(400).json({ message: "Payment schedule array is required" });
      }
      ...
      if (!['ACCEPTED', 'DEPOSIT_PAID', 'PAID'].includes(projectSmartFile.status)) {
        return res.status(400).json({ message: "Smart File must be accepted before selecting payment schedule" });
      }
  ...
  app.post("/api/public/smart-files/:token/create-payment-intent", async (req, res) => {
    try {
      const { token } = req.params;
      const { paymentType = 'DEPOSIT', tipCents = 0, amountCents, preview } = req.body; // DEPOSIT, FULL, BALANCE, or custom amountCents
      ...
      const hasContractPage = smartFile.pages?.some((p: any) => p.pageType === 'CONTRACT');
      ...
      if (totalCents === 0) {
        const invoicePage = smartFile.pages?.find((p: any) => p.pageType === 'INVOICE');
      ...
      const hasProposalPages = smartFile.pages?.some((p: any) =>
      ...
      if (typeof amountCents === 'number' && amountCents > 0) {
        ...
        if (!hasProposalPages) {
          const maxAllowed = projectSmartFile.status === 'DEPOSIT_PAID'
            ? balanceDueCents
            : totalCents;
      ...
      console.log(`💳 Creating new PaymentIntent for token ${token}, amount: ${paymentAmountCents} cents, idempotencyKey: ${idempotencyKey}`);
      ...
      console.log(`✅ PaymentIntent created successfully, clientSecret starts with: ${clientSecret?.substring(0, 30)}...`);
  ...
  app.get("/api/public/smart-files/:token/payment-history", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      const transactions = await storage.getPaymentTransactionsByProjectSmartFile(projectSmartFile.id);
  ...
  app.get("/api/public/smart-files/:token/payment-schedule", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      const { getNextDueInstallment, getTotalPaid, getTotalRemaining, generateSyntheticScheduleFromPaymentData } = await import('../shared/paymentScheduleUtils');
      ...
      const completedTransactions = transactions.filter(t => t.status === 'SUCCEEDED' || t.status === 'COMPLETED');
      const actualPaidCents = completedTransactions.reduce((sum, t) => sum + (t.amountCents || 0), 0);
      const totalCents = projectSmartFile.totalCents || 0;
      ...
      const totalPaidCents = schedule.length > 0 ? getTotalPaid(schedule) : actualPaidCents;
      const totalRemainingCents = schedule.length > 0 ? getTotalRemaining(schedule) : actualRemainingCents;
      ...
      res.json({
        ...
        summary: {
          ...
          paidCount,
          totalCount,
          percentComplete: totalCount > 0 ? Math.round((paidCount / totalCount) * 100) : 0,
          status: projectSmartFile.status,
          isFullyPaid: projectSmartFile.status === 'PAID' || totalRemainingCents === 0
        },
        transactions,
  ...
  app.post("/api/public/smart-files/:token/create-installment-payment", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      if (installmentId) {
        ...
        const { getNextDueInstallment } = await import('../shared/paymentScheduleUtils');
      ...
      const amountDueCents = installmentToPay.amountCents - (installmentToPay.amountPaidCents || 0);
      ...
      const installmentIndex = schedule.findIndex((i: any) => i.id === installmentToPay.id);
      const isFirstPayment = installmentIndex === 0;
      const isLastPayment = installmentIndex === schedule.length - 1;
      const paymentType = isFirstPayment ? 'DEPOSIT' : (isLastPayment ? 'BALANCE' : 'INSTALLMENT');

      // Create Stripe PaymentIntent with idempotency key to prevent duplicate charges
      const idempotencyKey = `sf_${projectSmartFile.id}_inst_${installmentToPay.id}_${paymentAmountCents}_${Math.floor(Date.now() / 60000)}`;
      console.log(`💳 Creating installment PaymentIntent for token ${token}, installment ${installmentToPay.id}, amount: ${paymentAmountCents} cents, idempotencyKey: ${idempotencyKey}`);
      const clientSecret = await createConnectPaymentIntent({
        amountCents: paymentAmountCents,
        connectedAccountId: photographer.stripeConnectAccountId,
        platformFeeCents,
  ...
  app.post("/api/public/smart-files/:token/confirm-payment", async (req, res) => {
    try {
      ...
      const existingTransaction = await storage.getPaymentTransactionByStripePaymentIntentId(paymentIntentId);
      if (existingTransaction) {
        console.log(`Payment already processed for paymentIntentId: ${paymentIntentId}, returning existing transaction`);
        const existingSmartFile = await storage.getProjectSmartFileByToken(token);
        return res.json({
          success: true,
      ...
      if (projectSmartFile.paymentSchedule && Array.isArray(projectSmartFile.paymentSchedule)) {
        ...
        newBalanceDue = updatedPaymentSchedule.reduce((sum, installment) => {
          const amountDue = installment.amountCents - (installment.amountPaidCents || 0);
      ...
      res.json({
        ...
        paymentMethod: paymentMethodDetails.brand && paymentMethodDetails.last4
          ? { brand: paymentMethodDetails.brand, last4: paymentMethodDetails.last4 }
          : null
      });
    } catch (error) {
  ...
  app.post("/api/public/smart-files/:token/create-setup-intent", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      if (!photographer?.stripeConnectAccountId) {
        return res.status(400).json({ message: "Photographer has not connected Stripe" });
      }
  ...
  app.post("/api/public/smart-files/:token/save-payment-method", async (req, res) => {
    try {
      const { token } = req.params;
      const { setupIntentId, setAsDefault = true, enableAutopay = false, nickname } = req.body;
      ...
      if (setupIntent.status !== 'succeeded') {
        return res.status(400).json({ message: "Setup intent has not succeeded" });
      }
      ...
      const existingMethod = await storage.getSavedPaymentMethodByStripeId(paymentMethodId);
      ...
      if (setAsDefault) {
        const existingMethods = await storage.getSavedPaymentMethodsByContact(projectSmartFile.clientId);
  ...
  app.get("/api/public/smart-files/:token/saved-payment-methods", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      const savedMethods = await storage.getSavedPaymentMethodsByContact(projectSmartFile.clientId);

      res.json({ 
        paymentMethods: savedMethods,
        autopayEnabled: projectSmartFile.autopayEnabled || false,
        autopayPaymentMethodId: projectSmartFile.autopayPaymentMethodId || null
      });
    } catch (error) {
      console.error("Get saved payment methods error:", error);
      res.status(500).json({ message: "Failed to get saved payment methods" });
    }
  });

  // DELETE /api/public/smart-files/:token/payment-methods/:methodId - Delete a saved payment method (PUBLIC ROUTE)
  app.delete("/api/public/smart-files/:token/payment-methods/:methodId", async (req, res) => {
    try {
      ...
      if (projectSmartFile.autopayPaymentMethodId === paymentMethod.stripePaymentMethodId) {
        await storage.updateProjectSmartFile(projectSmartFile.id, {
  ...
  app.post("/api/public/smart-files/:token/set-default-payment-method", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      await storage.setDefaultPaymentMethod(projectSmartFile.clientId, paymentMethodId);
  ...
  app.post("/api/public/smart-files/:token/toggle-autopay", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      if (enabled) {
        ...
        if (!methodId) {
          return res.status(400).json({ message: "A saved payment method is required to enable autopay" });
        }

        // Verify the payment method exists and belongs to this client
        const methods = await storage.getSavedPaymentMethodsByContact(projectSmartFile.clientId);
        const validMethod = methods.find(m => m.stripePaymentMethodId === methodId || m.id === methodId);
  ...
  app.post("/api/public/smart-files/:token/booking", async (req, res) => {
    try {
      ...
      const bookingDate = createDateInTimezone(dateStr, time, photographerTimezone);
  ...
  app.get("/api/automations/count", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
  ...
  app.post("/api/automations/generate-email-content", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      const availableVariables = [
        ...
        '{{phone}}', '{{business_name}}', '{{event_date}}'
      ];
...
Respond with JSON in this format:
{
  ...
  "body": "Email body with variables like {{first_name}} and optionally a button like [[BUTTON:CALENDAR:Schedule a Call]]"
      ...
      if (!result.subject && !result.body) {
        ...
        return res.status(500).json({ message: "AI did not generate valid content. Please try again." });
      }
      ...
      res.status(500).json({ message: error.message || "Failed to generate email content" });
    }
  });
  ...
  app.post("/api/automations/chat", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { message, conversationHistory, currentState, projectType } = req.body;
      const photographerId = req.user!.photographerId!;
      ...
      console.log(`📨 Chat message received: "${message.substring(0, 50)}..." (photographer: ${photographerId}, projectType: ${projectType})`);
      ...
      if (newState.status === "complete" && newState.collectedInfo) {
        ...
        for (let i = 0; i < automationsToCreate.length; i++) {
          ...
          if (automationType === 'COUNTDOWN') {
            ...
            // Sanitize AI-generated content for security
            const sanitizedSmsContent = collectedInfo.actionType === 'SMS' ? sanitizeAIContent(collectedInfo.content) : null;
            const sanitizedEmailContent = sanitizeAIContent(collectedInfo.content || '');
            const sanitizedSubject = sanitizeAIContent(collectedInfo.subject || '');
            ...
            if (collectedInfo.actionType === 'EMAIL') {
              ...
              automationData = {
                ...
                emailSubject: sanitizedSubject || `Reminder: ${collectedInfo.daysBefore} days until your event`,
            ...
            const parsedAutomationData = validateAutomationSchema.parse(automationData);
            const automation = await storage.createAutomation(parsedAutomationData);
          ...
          if (collectedInfo.actionType === 'SMART_FILE') {
            if (!collectedInfo.smartFileTemplateId) {
              console.log(`⏭️ Skipping SMART_FILE automation ${i + 1} (no template selected)`);
              continue;
            }
            // Verify the template exists and belongs to this photographer
            const smartFileTemplates = await storage.getSmartFilesByPhotographer(photographerId);
            const templateExists = smartFileTemplates.some(t => t.id === collectedInfo.smartFileTemplateId);
            if (!templateExists) {
              console.log(`⏭️ Skipping SMART_FILE automation ${i + 1} (template ${collectedInfo.smartFileTemplateId} not found)`);
              continue;
            }
          }
          ...
          // Sanitize AI-generated content for security
          const sanitizedCommSmsContent = collectedInfo.actionType === 'SMS' ? sanitizeAIContent(collectedInfo.content) : null;
          const sanitizedCommEmailContent = sanitizeAIContent(collectedInfo.content || '');
          const sanitizedCommSubject = sanitizeAIContent(collectedInfo.subject || '');
          ...
          if (collectedInfo.actionType === 'EMAIL') {
            ...
            const emailAutomationData = validateAutomationSchema.parse({
              ...
              enabled: true,
              useEmailBuilder: true,
              emailSubject: sanitizedCommSubject || 'Message from your photographer',
              templateBody: templateBody,
              htmlBody: htmlBody,
              includeHeader: true,
              headerStyle: 'professional',
            ...
            const automation = await storage.createAutomation(emailAutomationData);
            ...
            createdAutomations.push(automation);
            console.log(`✅ Created EMAIL automation: ${automation.name} (using email builder)`);
            continue;
          }
  ...
  app.delete("/api/automations/project-type/:projectType", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      const allAutomations = await storage.getAutomationsByPhotographer(photographerId, projectType);

      if (allAutomations.length === 0) {
        return res.json({ message: "No automations found for this project type", deletedCount: 0 });
      }
      ...
      console.log(`Deleted ${deletedCount} automations for project type ${projectType} (photographer: ${photographerId})`);
      res.json({ message: `Successfully deleted ${deletedCount} automations`, deletedCount });
    } catch (error) {
  ...
  app.delete("/api/automations/stage/:stageId", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { stageId } = req.params;
      const photographerId = req.user!.photographerId!;
      
      // Get all automations for this photographer and stage
      const allAutomations = await storage.getAutomationsByPhotographer(photographerId);
      const stageAutomations = allAutomations.filter(a => a.stageId === stageId);
      
      if (stageAutomations.length === 0) {
        return res.json({ message: "No automations found for this stage", deletedCount: 0 });
      }
  ...
  app.post("/api/automations/:id/test", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      const hasEmailContent = automation.templateBody || automation.emailBlocks;
      const isCountdownWithEmailBuilder = automation.automationType === 'COUNTDOWN' && automation.useEmailBuilder && hasEmailContent;
      const isCommunicationWithEmailBuilder = automation.automationType === 'COMMUNICATION' && automation.useEmailBuilder && hasEmailContent;
        ...
        try {
          ...
          if (!photographerEmail) {
            ...
            if (automation.templateBody) {
              ...
              const bodyHtml = automation.htmlBody || automation.templateBody
                .split('\n\n')
                .filter((p: string) => p.trim())
                .map((p: string) => `<p style="margin: 0 0 16px 0; line-height: 1.6;">${p.replace(/\n/g, '<br>')}</p>`)
                .join('');
              ...
              if (automation.includeHeader) {
                const { generateEmailHeader } = await import('../shared/email-branding-shared.js');
                const headerHtml = generateEmailHeader(photographer, automation.headerStyle || 'professional');
                htmlBody = headerHtml + htmlBody;
              }
              if (automation.includeSignature !== false) {
                const { generateEmailSignature } = await import('../shared/email-branding-shared.js');
                const signatureHtml = generateEmailSignature(photographer, automation.signatureStyle || 'professional');
  ...
  app.post("/api/automations/generate-all-stages", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      ...
      const allAutomations = await storage.getAutomationsByPhotographer(photographerId);
      const existingAutomations = allAutomations.filter(a => a.projectType === projectType);

      // 3. Get photographer info for personalization
      const photographer = await storage.getPhotographer(photographerId);
      const businessName = photographer?.businessName || photographer?.fullName || "our studio";

      // 4. Classify all stages with AI
      const { classifyStageIntents, generateAutomationsForStage } = await import("./services/automationAI");
      const classifications = await classifyStageIntents(stages);

      // 5. Generate automations for each stage based on intent
      const stageResults = await Promise.all(classifications.map(async (classification) => {
        const existingForStage = existingAutomations.filter(a => a.stageId === classification.stageId);
      ...
      res.json({
        stages: stageResults,
        totalSuggested: stageResults.reduce((sum, s) => sum + s.suggestedAutomations.length, 0),
        totalExisting: existingAutomations.length
      });
    } catch (error: any) {
      console.error('Generate all stages error:', error);
      res.status(500).json({ message: error.message || "Failed to generate automations" });
    }
  });

  // Bulk create automations from AI-generated previews
  app.post("/api/automations/bulk-create", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const { automations, projectType = "WEDDING" } = req.body;

      if (!automations || !Array.isArray(automations) || automations.length === 0) {
        return res.status(400).json({ message: "Automations array is required" });
      }
      ...
      for (const auto of automations) {
        try {
          ...
          if (auto.automationType === 'STAGE_CHANGE') {
            ...
            if (!auto.triggerType) {
              console.warn(`STAGE_CHANGE automation "${auto.name}" missing triggerType, skipping`);
            ...
            if (!resolvedTargetStageId && auto.targetStageName) {
              const targetStage = allStages.find(s =>
                s.name?.toLowerCase().includes(auto.targetStageName.toLowerCase())
              );
              resolvedTargetStageId = targetStage?.id || null;
            }

            if (!resolvedTargetStageId) {
              console.warn(`STAGE_CHANGE automation "${auto.name}" has no valid targetStageId, skipping`);
              failed.push({ name: auto.name, error: 'Could not resolve target stage' });
              continue;
            }
            ...
            const automation = await storage.createAutomation({
              photographerId,
              name: auto.name,
              description: auto.description || `Auto-move to next stage when ${auto.triggerType.replace(/_/g, ' ').toLowerCase()}`,
              automationType: 'STAGE_CHANGE',
            ...
            console.log(`Created STAGE_CHANGE automation "${auto.name}" with targetStageId=${resolvedTargetStageId}`);
            created.push({ id: automation.id, name: auto.name, stageId: auto.stageId });
            continue;
          }
          ...
          if (auto.channel === 'EMAIL') {
            ...
            const automation = await storage.createAutomation({
              photographerId,
              name: auto.name,
              description: auto.description || 'Email sent when client enters this stage',
              automationType: 'COMMUNICATION',
              stageId: auto.stageId,
              channel: 'EMAIL',
              projectType: projectType,
            ...
            const automation = await storage.createAutomation({
              photographerId,
              name: auto.name,
              description: auto.description || 'SMS sent when client enters this stage',
              automationType: 'COMMUNICATION',
              stageId: auto.stageId,
              channel: 'SMS',
              projectType: projectType,
            ...
            const automation = await storage.createAutomation({
              photographerId,
              name: auto.name,
              description: auto.description || 'Automation triggered when client enters this stage',
          ...
          failed.push({ name: auto.name, error: error.message || 'Unknown error' });
        }
      }
      ...
      res.status(500).json({ message: error.message || "Failed to create automations" });
    }
  });
  ...
  app.post("/api/drip-campaigns", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      if (!isGlobal) {
        if (targetStageIds && Array.isArray(targetStageIds) && targetStageIds.length > 0) {
          // Multi-stage: verify all stage IDs are valid
          const invalidStages = targetStageIds.filter((id: string) => !allStages.find(s => s.id === id));
          if (invalidStages.length > 0) {
            return res.status(400).json({ message: "Invalid target stages", invalidStages });
          }
          ...
          const stage = allStages.find(s => s.id === campaignData.targetStageId);
          if (!stage) {
            return res.status(400).json({ message: "Invalid target stage" });
          }
        } else {
          return res.status(400).json({ message: "Either targetStageIds, targetStageId, or isGlobal must be specified" });
        }
      } else {
  ...
  app.post("/api/drip-campaigns/:id/pause", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const campaign = await storage.getDripCampaign(req.params.id);
      if (!campaign || campaign.photographerId !== req.user!.photographerId!) {
        return res.status(404).json({ message: "Drip campaign not found" });
  ...
  app.patch("/api/drip-campaigns/:campaignId/emails/:emailId", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { campaignId, emailId } = req.params;
      const { subject, htmlBody, textBody, emailBlocks, templateBody, sendAtHour, daysAfterStart, delayMinutes, useEmailBuilder, includeHeader, headerStyle, includeSignature, signatureStyle } = req.body;
      
      // Validate delayMinutes if provided
      if (delayMinutes !== null && delayMinutes !== undefined && delayMinutes < 1) {
        return res.status(400).json({ message: "Delay must be at least 1 minute" });
      }
  ...
  app.post("/api/drip-campaigns/:campaignId/emails", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { campaignId } = req.params;
      const { subject, htmlBody, textBody, templateBody, daysAfterStart, sendAtHour, delayMinutes, emailBlocks, useEmailBuilder } = req.body;
      ...
      const email = await storage.createDripCampaignEmail({
        ...
        weeksAfterStart: Math.floor((daysAfterStart ?? (nextSequenceIndex * 7)) / 7),
  ...
  app.delete("/api/drip-campaigns/:campaignId/emails/:emailId", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { campaignId, emailId } = req.params;
      ...
      const remainingEmails = emails.filter(e => e.id !== emailId).sort((a, b) => a.sequenceIndex - b.sequenceIndex);
      for (let i = 0; i < remainingEmails.length; i++) {
        if (remainingEmails[i].sequenceIndex !== i) {
          await storage.updateDripCampaignEmail(remainingEmails[i].id, { sequenceIndex: i });
        }
      }
  ...
  app.post("/api/drip-campaigns/:campaignId/emails/reorder", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { campaignId } = req.params;
      ...
      for (const id of emailIds) {
        if (!emailIdSet.has(id)) {
          return res.status(400).json({ message: `Email ${id} not found in this campaign` });
        }
      }

      // Update sequence indices
      for (let i = 0; i < emailIds.length; i++) {
        await storage.updateDripCampaignEmail(emailIds[i], { sequenceIndex: i });
      }
  ...
  app.post("/api/openai/generate-drip-email", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const { campaignName, projectType, prompt, emailNumber, totalEmails } = req.body;
...
REMEMBER: The body MUST have real email content (3-4 sentences minimum), not just a button. Return valid JSON.`
      ...
      const bodyWithoutButtons = (result.body || '').replace(/\[\[BUTTON:[^\]]+\]\]/g, '').trim();
      if (bodyWithoutButtons.length < 50) {
        console.error("AI returned insufficient body content:", result.body);
        throw new Error("AI generated insufficient email content. Please try again.");
      }
  ...
  function extractWizardIntent(message: string, currentInfo: any = {}) {
    ...
    const campaignPatterns = [
      { pattern: /follow[- ]?up|nurture|inquiry|inquir/, type: 'follow-up sequence for new inquiries' },
      { pattern: /welcome|new lead|first contact/, type: 'welcome sequence for new leads' },
      { pattern: /re[- ]?engage|past client|dormant|cold/, type: 're-engagement for past clients' },
      { pattern: /post[- ]?booking|after book|booked client/, type: 'post-booking follow-up' },
    ...
    const audiencePatterns = [
      { pattern: /new lead|inquir|just reach|potential/, audience: 'new leads who inquired' },
    ...
    const goalPatterns = [
      { pattern: /book.*call|consultation|discovery|schedule.*call|call.*book|get.*book|book.*them|schedule.*zoom|zoom.*call|phone.*call|meeting|appointment|book.*consult|consult.*book/, goal: 'book consultation calls' },
      { pattern: /pricing|proposal|quote|package|invest/, goal: 'share pricing/proposal' },
      { pattern: /trust|relationship|connect|warm|nurture|educate/, goal: 'build trust and connection' },
      { pattern: /qualify|screen|vet|filter/, goal: 'qualify leads' }
    ];
    for (const { pattern, goal } of goalPatterns) {
      const matched = pattern.test(text);
      if (matched) {
        console.log(`[Intent] Goal pattern matched: "${goal}" from pattern ${pattern}`);
      }
    ...
    extracted.isDelegating = currentInfo.isDelegating || delegationPatterns.some(p => p.test(text));
    
    // Check if we have enough info to generate
    const hasEssentials = extracted.campaignType && extracted.targetAudience && extracted.goals;
    extracted.readyToGenerate = hasEssentials && (extracted.emailCount || extracted.isDelegating);
    
    return extracted;
  }
  
  // In-memory conversation storage (production would use Redis/DB)
  const wizardConversations: Map<string, {
  ...
  app.post("/api/ai-campaign-wizard/start", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const photographer = await storage.getPhotographer(photographerId);
      ...
      const stagesList = stageNames.length > 0 
...
• Something else?`;
      ...
      res.status(500).json({ message: "Failed to start wizard", error: error.message });
    }
  });

  // Handle AI Campaign Wizard message - Split architecture: GPT-4.1 for conversation, GPT-4.1-mini for extraction
  app.post("/api/ai-campaign-wizard/message", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      async function callWithRetry<T>(
        ...
        for (let i = 0; i < retries; i++) {
          try {
            ...
            console.log(`[AI Wizard] Attempt ${i + 1} failed:`, error.message);
      ...
      console.log("[AI Wizard] Step 1: Extracting slots with GPT-4.1-mini...");
      
      // Get available stage names for extraction context
      const availableStageNames = conversation.availableStages?.map(s => s.name) || 
        ['New Inquiry', 'Consultation / Discovery', 'Proposal Sent', 'Booked / Paid', 'Completed'];
...
{"campaignType": string|null, "targetAudience": string|null, "goals": string|null, "emailCount": number|null, "targetStages": string[]|null}`;
      ...
      try {
        ...
        console.log("[AI Wizard] Extracted slots:", JSON.stringify(extractedSlots));
      } catch (extractError: any) {
        console.error("[AI Wizard] Slot extraction failed, continuing with current slots:", extractError.message);
      }

      // Validate and coerce emailCount to a number within bounds (3-10)
      let validatedEmailCount: number | undefined;
      if (extractedSlots.emailCount !== null && extractedSlots.emailCount !== undefined) {
      ...
      if (extractedSlots.targetStages) {
        if (Array.isArray(extractedSlots.targetStages)) {
          ...
          normalizedTargetStages = extractedSlots.targetStages.split(',').map((s: string) => s.trim());
        }
      }
      ...
      const mergedSlots: any = {
        ...currentSlots,
        ...(extractedSlots.campaignType && { campaignType: extractedSlots.campaignType }),
        ...(extractedSlots.targetAudience && { targetAudience: extractedSlots.targetAudience }),
        ...(extractedSlots.goals && { goals: extractedSlots.goals }),
        ...(validatedEmailCount && { emailCount: validatedEmailCount }),
        ...(normalizedTargetStages && normalizedTargetStages.length > 0 && { targetStages: normalizedTargetStages })
      };

      // STEP 2: Compute readiness deterministically in backend (NOT AI)
      const hasTargetStages = Array.isArray(mergedSlots.targetStages) && mergedSlots.targetStages.length > 0;
      ...
      console.log("[AI Wizard] Ready to generate (computed):", isReadyToGenerate);
      ...
      if (isReadyToGenerate) {
        ...
        const stageDisplay = Array.isArray(mergedSlots.targetStages) 
          ? mergedSlots.targetStages.some((s: string) => s.toUpperCase() === 'ALL')
            ? 'All Stages'
...
Click "Start Building Campaign" when you're ready, and I'll generate your complete email sequence!`;
        ...
        if (!mergedSlots.targetStages) missingSlots.push("which pipeline stage(s) should trigger this campaign");
...
Keep it conversational and under 100 words. Don't use JSON. Don't list all options unless helpful.`;
        ...
        try {
          ...
          console.error("[AI Wizard] Conversation generation failed, using fallback:", convError.message);
  ...
  app.post("/api/ai-campaign-wizard/generate", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      if (!conversation.readyToGenerate) {
        return res.status(400).json({ message: "Conversation is not ready to generate. Continue the conversation first." });
      }
      ...
      const stageNames = Array.isArray(info.targetStages) ? info.targetStages : [];
      
      // Check if user wants all stages (normalize "ALL", "all", "all stages" etc)
      const isAllStages = stageNames.some((s: string) => /^all(\s+stages?)?$/i.test(s.trim()));
      ...
      if (!isAllStages && stageNames.length > 0) {
        ...
        targetStageIds = stageNames
          .map((name: string) => {
            ...
            const stage = availableStages.find(s => s.name.toLowerCase() === normalizedName);
            return stage?.id;
          })
          .filter(Boolean) as string[];
        ...
        if (targetStageIds.length === 0) {
          targetStageIds = null;
          console.log("[AI Wizard Generate] Warning: No stage matches found, defaulting to all stages");
        }
      }
      console.log("[AI Wizard Generate] Target stages mapped:", stageNames, "->", targetStageIds);
...
Space emails: day 0, 2, 5, 7, 10.`;
      ...
      console.log("[AI Wizard Generate] Full GPT-5 response:", JSON.stringify(campaignCompletion, null, 2));
      ...
      console.log("[AI Wizard Generate] Finish reason:", campaignCompletion.choices[0]?.finish_reason);
      
      if (!campaignContent) {
        throw new Error("GPT-5 returned empty content. Full response: " + JSON.stringify(campaignCompletion));
      }

      const campaignData = JSON.parse(campaignContent);
      console.log("[AI Wizard Generate] Parsed campaign data:", JSON.stringify(campaignData, null, 2));
      ...
      console.log("[AI Wizard Generate] Created campaign with targetStageIds:", targetStageIds);
      ...
      if (campaignData.emails && Array.isArray(campaignData.emails)) {
        for (let i = 0; i < campaignData.emails.length; i++) {
          ...
          const templateBody = (emailData.blocks || []).map((block: any) => {
            if (block.type === "TEXT") {
              return typeof block.content === 'string' ? block.content : (block.content?.text || '');
            }
            ...
            if (block.type === "BUTTON") {
              ...
              const btnText = (btnContent.text || 'Click Here').replace(/[\[\]:]/g, '');
              const linkType = btnContent.linkType || 'CALENDAR';
              const linkValue = btnContent.linkValue || btnContent.url || '';
          ...
          const htmlBody = (emailData.blocks || []).map((block: any) => {
            if (block.type === "TEXT") {
              const text = typeof block.content === 'string' ? block.content : (block.content?.text || '');
              return `<p>${text}</p>`;
            }
      ...
      res.status(500).json({ message: "Failed to generate campaign", error: error.message });
    }
  });
    ...
    try {
      ...
      if (!booking.googleCalendarEventId) {
        try {
          ...
          console.error('Failed to create Google Calendar event for booking:', calError);
  ...
  app.put("/api/bookings/:id", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      if (timeChanged && updatedBooking.projectId) {
        ...
        await onBookingUpdated(updatedBooking.id, updatedBooking.projectId, booking.startAt, booking.endAt);
      }
...
${photographer.businessName}`
  ...
  app.get("/api/availability/slots-range", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      if (daysDiff > 31 || daysDiff < 0) {
        return res.status(400).json({ message: "Date range must be between 0 and 31 days" });
      }

      // Use optimized batch method
      const result = await slotGenerationService.getSlotsForDateRange(photographerId, start, end);

      // Fetch bookings for the date range (for calendar display with categories)
      const bookingsInRange = await storage.getBookingsByPhotographerInRange(photographerId, startDate, endDate);

      // Fetch payment due dates for the date range
      const paymentsDue = await storage.getPaymentsDueInRange(photographerId, startDate, endDate);
  ...
  app.get("/api/availability/slots/:date", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      const result = await slotGenerationService.getSlotsForDate(photographerId, new Date(date));
  ...
  app.post("/api/availability/overrides/bulk", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const { startDate, endDate, dates: individualDates, reason, startTime, endTime } = req.body;

      // Validate time formats if provided (for custom hours)
      if (startTime || endTime) {
        if (!startTime || !endTime) {
          return res.status(400).json({ message: "Both startTime and endTime are required for custom hours" });
        }
      ...
      if (individualDates && Array.isArray(individualDates) && individualDates.length > 0) {
        ...
        if (individualDates.length > 90) {
          return res.status(400).json({ message: "Cannot block more than 90 days at once" });
        }
        ...
        return res.status(400).json({ message: "Either 'dates' array or 'startDate' and 'endDate' are required" });
      }

      // Check for existing bookings in the date range
      const allBookings = await storage.getBookingsByPhotographer(photographerId);
      const conflictingBookings = allBookings.filter(booking => {
        ...
        time: new Date(booking.startAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
      ...
      for (const date of dates) {
        ...
        const existing = await storage.getDailyAvailabilityOverrideByDate(photographerId, date);
        if (existing) {
        ...
        await slotGenerationService.regenerateSlotsForDate(photographerId, new Date(date));
      }
  ...
  app.get("/api/calendar/status", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      let isAuthenticated = await googleCalendarService.isAuthenticated(photographerId);
      ...
      if (!isAuthenticated) {
        ...
        if (credentials?.refreshToken) {
          ...
          const refreshed = await googleCalendarService.refreshTokens(photographerId);
          if (refreshed) {
  ...
  app.get("/api/calendar/list", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const result = await googleCalendarService.listCalendars(photographerId);
  ...
  app.get("/api/calendar/sync-settings", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const photographer = await storage.getPhotographer(photographerId);
      ...
      res.json({
        ...
        calendarWriteTo: photographer.googleCalendarWriteTo || photographer.googleCalendarId || null
      });
    } catch (error) {
  ...
  app.post("/api/calendar/check-conflicts", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const { calendarIds } = req.body;

      if (!calendarIds || !Array.isArray(calendarIds) || calendarIds.length === 0) {
        return res.status(400).json({ message: "calendarIds array is required" });
      }

      const result = await googleCalendarService.detectBookingConflicts(photographerId, calendarIds);
  ...
  app.post("/api/calendar/sync-settings", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const { syncEnabled, calendarsToCheck, calendarWriteTo } = req.body;

      // Validate input
      if (syncEnabled && (!calendarsToCheck || !Array.isArray(calendarsToCheck) || calendarsToCheck.length === 0)) {
        return res.status(400).json({ message: "At least one calendar must be selected when enabling sync" });
      }
      ...
      await storage.updatePhotographer(photographerId, {
        ...
        googleCalendarMetadata: Object.keys(calendarMetadata).length > 0 ? calendarMetadata : null,
      ...
      res.json({
        message: syncEnabled ? "Calendar sync enabled" : "Calendar sync disabled",
  ...
  app.get("/api/calendar/busy/:date", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographerId = req.user!.photographerId!;
      const { date } = req.params;
  ...
  app.post("/api/projects/:projectId/native-gallery", authenticateToken, requirePhotographer, requireActiveSubscription, requireGalleryPlan, async (req, res) => {
    try {
      const { projectId } = req.params;
      const photographerId = req.user!.photographerId!;
      ...
      const existingGalleries = await storage.getGalleriesByPhotographer(photographerId);
      const linkedGallery = existingGalleries.find(g => g.projectId === projectId);
      if (linkedGallery) {
        return res.status(400).json({
  ...
  app.get("/api/galleries/:id/view", async (req, res) => {
    try {
      ...
      if (authResult) {
        try {
          const decoded = jwt.verify(authResult.token, process.env.JWT_SECRET || 'fallback-secret') as any;
              ...
              if (gallery.projectId) {
                ...
                if (project) {
                  ...
                  if (project.clientId === contact.id) {
                    ...
                    const participants = await storage.getProjectParticipants(gallery.projectId);
                    const isParticipant = participants.some(p => p.clientId === contact.id);
                    if (isParticipant) {
                      isAuthorizedClient = true;
                    }
      ...
      if (!gallery.isPublic && !isAuthorizedClient) {
        ...
        const photographer = await storage.getPhotographer(gallery.photographerId);
        return res.status(403).json({ 
  ...
  app.get("/api/client-portal/projects", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      ...
      const formattedProjects = await Promise.all(sortedProjects.map(async (p) => {
        ...
        const sharedGalleries = projectGalleries.filter(g => g.status === 'SHARED');
  ...
  app.get("/api/client-portal/payment-methods", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      const user = req.user!;
      ...
      // Get saved payment methods for this client
      const savedMethods = await storage.getSavedPaymentMethodsByContact(userRecord.clientId);
  ...
  app.delete("/api/client-portal/payment-methods/:methodId", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      const user = req.user!;
  ...
  app.post("/api/client-portal/set-default-payment-method", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      const user = req.user!;
      const { paymentMethodId } = req.body;
      ...
      // Set as default
      await storage.setDefaultPaymentMethod(userRecord.clientId, paymentMethodId);
  ...
  app.get("/api/client-portal/projects/:id/payment-summary", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      const { id } = req.params;
      const user = req.user!;
      ...
      if (!hasAccess) {
        return res.status(403).json({ message: "Access denied - not a participant in this project" });
      }
      ...
      const smartFileSummaries = await Promise.all(projectSmartFiles.map(async (psf) => {
        ...
        // Parse payment schedule from project smart file
        let schedule = psf.paymentSchedule && Array.isArray(psf.paymentSchedule)
          ? psf.paymentSchedule
          : [];
        ...
        // Generate synthetic schedule if no explicit schedule but payment data exists
        if (schedule.length === 0 && totalCents > 0) {
          const { generateSyntheticScheduleFromPaymentData } = await import('../shared/paymentScheduleUtils');
          const eventDate = project.eventDate ? new Date(project.eventDate) : null;
          schedule = generateSyntheticScheduleFromPaymentData(
            totalCents,
            psf.depositCents || 0,
  ...
  app.get("/api/client-portal/questionnaires/:id", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      ...
      const questionnaire = await storage.getProjectQuestionnaire(questionnaireId);
      if (!questionnaire) {
      ...
      const template = await storage.getQuestionnaireTemplate(questionnaire.templateId);
      const questions = await storage.getQuestionnaireQuestionsByTemplate(questionnaire.templateId);
  ...
  app.put("/api/client-portal/questionnaires/:id", authenticateToken, enforceSubdomainTenantAuth, async (req, res) => {
    try {
      const questionnaireId = req.params.id;
      const user = req.user!;
      ...
      const updated = await storage.updateProjectQuestionnaire(questionnaireId, {
        answers,
  ...
  const { WIDGET_EMBED_SCRIPT, FORM_EMBED_SCRIPT, BOOKING_EMBED_SCRIPT } = await import('./widget-scripts');
  ...
  app.post("/api/public/booking/calendar/:publicToken/book/:date/:slotId", async (req, res) => {
    try {
      ...
      const { slots: availableSlots } = await slotGenerationService.getSlotsForDate(photographer.id, new Date(date));
      ...
      const startAt = createDateInTimezone(date, startTime, photographerTimezone);
...
${photographer.businessName}
  ...
  app.post("/api/public/booking/by-slug/:slug/book/:date/:slotId", async (req, res) => {
    try {
  ...
  app.post("/api/short-links", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      const { resolvePortalBaseUrl } = await import('./utils/portal-url');
      const currentEnvBaseUrl = resolvePortalBaseUrl(photographer.portalSlug, 'scheduler-short-link');
      ...
      console.log(`🔧 Canonicalizing target URL: ${targetUrl} → ${canonicalTargetUrl}`);
      ...
      let existingLink = await storage.getShortLinkByPhotographerAndTarget(photographerId, canonicalTargetUrl);
      
      // Also check by pathname pattern in case legacy records have different domains
      if (!existingLink) {
        existingLink = await storage.getShortLinkByPhotographerAndPathname(photographerId, pathname);
        ...
        if (existingLink && existingLink.targetUrl !== canonicalTargetUrl) {
          console.log(`🔧 Updating legacy short link ${existingLink.shortCode} domain: ${existingLink.targetUrl} → ${canonicalTargetUrl}`);
          await storage.updateShortLink(existingLink.id, { targetUrl: canonicalTargetUrl });
          existingLink.targetUrl = canonicalTargetUrl;
        }
      }
      
      if (existingLink) {
        console.log(`♻️ Reusing existing short link ${existingLink.shortCode} for ${canonicalTargetUrl}`);
        return res.json({
          ...existingLink,
      ...
      console.log(`🆕 Created new short link ${shortCode} for ${canonicalTargetUrl}`);
  ...
  app.get("/s/:code", async (req, res) => {
    try {
      ...
      if (!originalTargetUrl || typeof originalTargetUrl !== 'string' || originalTargetUrl.trim() === '') {
        console.error(`❌ Short link ${code} has invalid targetUrl: ${originalTargetUrl}`);
      ...
      const hasReplitUrl = originalTargetUrl.includes('replit.dev') || 
        ...
        try {
          ...
          if (photographer?.portalSlug) {
            ...
            try {
              ...
              console.log(`🔧 Self-healing stale short link ${code}: ${originalTargetUrl} → ${newTargetUrl}`);
              
              // Update the database for future redirects
              await storage.updateShortLink(shortLink.id, { targetUrl: newTargetUrl });
              
              // Use the corrected URL for this redirect
              targetUrl = newTargetUrl;
            } catch (parseError) {
              // URL parsing failed - log and continue with original URL
              console.log(`⚠️ Could not parse URL for self-heal: ${originalTargetUrl}, using original`);
              // targetUrl remains originalTargetUrl
            }
          } else {
            console.log(`⚠️ Cannot self-heal short link ${code}: photographer has no portal slug, using original`);
            // targetUrl remains originalTargetUrl
          }
        } catch (healError) {
          console.error(`❌ Failed to self-heal short link ${code}:`, healError);
          // targetUrl remains originalTargetUrl
        }
      }
  ...
  app.post("/api/stripe-connect/disconnect", authenticateToken, requirePhotographer, async (req, res) => {
    try {
      const photographer = await storage.getPhotographer(req.user!.photographerId!);
      if (!photographer) {
        return res.status(404).json({ message: "Photographer not found" });
      ...
      if (!photographer.stripeConnectAccountId) {
        return res.status(400).json({ message: "No Stripe Connect account to disconnect" });
      }
      ...
      console.log(`Disconnected Stripe account ${oldAccountId} for photographer ${photographer.id}`);

      res.json({
        success: true,
        message: "Stripe Connect account disconnected successfully. You can now connect a new account."
      });
  ...
  app.get("/api/stripe-connect/account-status", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      ...
      let account;
      try {
        account = await stripe.accounts.retrieve(photographer.stripeConnectAccountId);
      } catch (stripeError: any) {
        console.log('Stripe account retrieve error:', stripeError.code, stripeError.message);
        ...
        if (isTestLiveMismatch) {
          console.log(`Stripe account ${photographer.stripeConnectAccountId} not accessible - test/live mode mismatch detected`);
          return res.json({
            hasAccount: true,
            accountId: photographer.stripeConnectAccountId,
            status: 'invalid',
            ...
            reconnectReason: 'Your Stripe account was connected in a different mode (test vs live). Please disconnect and reconnect your Stripe account.'
          });
        }
  ...
  app.post("/api/stripe-connect/dashboard-link", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographer = await storage.getPhotographer(req.user!.photographerId!);
      if (!photographer) {
        return res.status(404).json({ message: "Photographer not found" });
      ...
      if (!photographer.stripeConnectAccountId) {
        return res.status(400).json({ 
          message: "No Stripe Connect account found. Please connect your Stripe account first." 
        });
      }

      // Generate a login link for the Express Dashboard
      const loginLink = await stripe.accounts.createLoginLink(photographer.stripeConnectAccountId);
      ...
      if (error.code === 'account_invalid' || error.code === 'resource_missing') {
        return res.status(400).json({ 
          message: "Your Stripe account is not accessible. Please reconnect your Stripe account.",
          requiresReconnect: true
        });
      }
  ...
  app.get("/api/stripe-connect/outstanding", authenticateToken, requirePhotographer, requireActiveSubscription, async (req, res) => {
    try {
      const photographer = await storage.getPhotographer(req.user!.photographerId!);
      if (!photographer) {
        return res.status(404).json({ message: "Photographer not found" });
      ...
      for (const project of projects) {
        ...
        const projectSmartFiles = await storage.getProjectSmartFilesByProject(project.id);
        
        for (const psf of projectSmartFiles) {
          // Skip if fully paid or not in accepted status
          if (psf.status !== 'ACCEPTED' && psf.status !== 'SIGNED' && psf.status !== 'PENDING') {
            continue;
          }
          ...
          const transactions = await storage.getPaymentTransactionsByProjectSmartFile(psf.id);
          const completedTransactions = transactions.filter(t => t.status === 'SUCCEEDED' || t.status === 'COMPLETED');
          const paidCents = completedTransactions.reduce((sum, t) => sum + (t.amountCents || 0), 0);
          const totalCents = psf.totalCents || 0;
          ...
          // Get client info
          const client = project.clientId ? await storage.getContact(project.clientId) : null;
          ...
          outstandingInvoices.push({
            ...
            paidCents,
            remainingCents,
            percentComplete: totalCents > 0 ? Math.round((paidCents / totalCents) * 100) : 0,
            schedule,
            nextInstallment,
  ...
  app.post("/api/chatbot", async (req, res) => {
    try {
      const { message, context = "general", photographerName, history = [], navigationContext } = req.body;
      ...
      const response = await getChatbotResponse(message, context, photographerName, history, photographerId, navigationContext);
