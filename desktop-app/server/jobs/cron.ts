...
import { processAutomations, processPaymentReminders, processPhotographerAppointmentReminders, processDripCampaigns } from '../services/automation';
...
async function processExpirationReminders() {
  ...
  console.log(`Found ${expiringSmartFiles.length} proposals expiring within 3 days`);
  ...
  for (const psf of expiringSmartFiles) {
    try {
      ...
      if (!emailResult.success) {
        console.error(`Failed to send expiration reminder for ${psf.id}: ${emailResult.error}`);
      ...
      console.log(`✅ Sent expiration reminder for proposal ${psf.id} to ${contact.email}`);
