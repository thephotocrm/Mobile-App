...
export async function generateContractPDF(htmlContent: string): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      for (const line of lines) {
        ...
        if (isFirstLine || isHeading(trimmedLine)) {
          ...
          if (signatureIndex < signatures.length) {
            try {
              const sigBuffer = Buffer.from(signatures[signatureIndex], 'base64');
              const imgX = doc.page.margins.left;
              const imgWidth = Math.min(200, doc.page.width - doc.page.margins.left - doc.page.margins.right);
      ...
      doc.moveDown(0.5);
      doc.fontSize(8).text('This PDF is a simplified rendering. The authoritative legal record is the HTML snapshot stored at the time of signing.', { align: 'center' });
...
function convertHtmlToPlainText(html: string): string {
  ...
  text = text.replace(/<img[^>]+src=["']data:image\/[^;]+;base64,[^"']+["'][^>]*>/gi, '\n[SIGNATURE_IMAGE]\n');
...
function isHeading(line: string): boolean {
  ...
  return headingPatterns.some(pattern => pattern.test(line)) && line.length < 50;
}
