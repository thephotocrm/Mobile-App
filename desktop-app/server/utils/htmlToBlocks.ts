...
export function convertHtmlToBlocks(htmlBody: string): EmailBlock[] {
  ...
  const contentMatch = htmlBody.match(
    /<div class="content-section"[^>]*>(.*?)<\/div>\s*(?:<div|<\/div|<\/body)/s,
  );
  if (!contentMatch) {
    // Fallback: try to find content between content-section tags without strict ending
    const fallbackMatch = htmlBody.match(
      /<div class="content-section"[^>]*>([\s\S]*?)(?=<div style="background: #f8f9fa|<\/body>)/,
    );
    ...
    if (tag && tag.match(/^h[1-6]$/)) {
      ...
      if (cleanedContent) {
        ...
        if (
          ...
          innerContent.includes("⚠️") ||
          innerContent.includes("<strong>")
        ) {
          // Extract the tip content
          const tipMatch =
            innerContent.match(/💡\s*<\/div>\s*<div[^>]*>(.*?)<\/div>/s) ||
            innerContent.match(
              /⚠️\s*Important<\/div>\s*<div[^>]*>(.*?)<\/div>/s,
            );
          if (tipMatch) {
      ...
      if (
        innerContent.includes("cta-button") ||
        innerContent.includes("Start Conversation")
      ) {
        // It's a CTA button
...
function addButtonBlock(blocks: EmailBlock[], text: string, url: string): void {
  ...
  if (url.includes("mailto:")) {
    ...
    url.includes("/smart-files/") ||
    url.includes("{{smartFileUrl}}")
  ) {
