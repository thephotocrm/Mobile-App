...
async function setupDemoAutomations() {
  try {
    console.log(
      "🚀 Setting up wedding inquiry automations for demo@lazyphotog.test...\n",
    );
    ...
    const oldComms = existingAutomations.filter(
      (a) =>
        a.stageId === inquiryStage.id && a.automationType === "COMMUNICATION",
    );
    ...
    const existingTemplates =
      await storage.getTemplatesByPhotographer(photographerId);
...
{{photographer_name}}</p>`,
    ...
    const a1 = await storage.createAutomation({
      ...
      description:
        "Instant email response - sets warmth, professionalism, and instant credibility",
    ...
    const a2 = await storage.createAutomation({
      ...
      description:
        "Instant SMS - confirms legitimacy and keeps you top-of-mind on mobile",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "SMS",
      projectType: "WEDDING",
    ...
    await storage.createAutomationStep({
      ...
      actionType: "SMS",
      customSmsContent:
        "Hey {{first_name}}! This is {{business_name}} — thanks for reaching out about your wedding 🎉 Excited to hear what you're planning! You can grab a quick time to chat here: {{scheduler_link}}",
      enabled: true,
    });
...
<p>– {{photographer_name}}</p>`,
    ...
    const a3 = await storage.createAutomation({
      ...
      description:
        "6-hour follow-up - visual proof + emotional tone while lead is still warm",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "EMAIL",
      projectType: "WEDDING",
...
<p>– {{photographer_name}}</p>`,
    ...
    const a4 = await storage.createAutomation({
      ...
      description:
        "24-hour reminder - keeps conversation alive without being pushy",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "EMAIL",
      projectType: "WEDDING",
...
<p>– {{photographer_name}}</p>`,
    ...
    const a5 = await storage.createAutomation({
      ...
      description:
        "48-hour social proof - validates you through others, builds trust",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "EMAIL",
      projectType: "WEDDING",
...
<p>– {{photographer_name}}</p>`,
    ...
    const a6 = await storage.createAutomation({
      ...
      description:
        "4-day hold offer - creates gentle urgency and micro-commitment",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "EMAIL",
      projectType: "WEDDING",
...
<p>– {{photographer_name}}</p>`,
    ...
    const a7 = await storage.createAutomation({
      ...
      description:
        "7-day story gallery - re-engages cold leads through emotion and visualization",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "EMAIL",
      projectType: "WEDDING",
...
<p>– {{photographer_name}}</p>`,
    ...
    const a8 = await storage.createAutomation({
      ...
      description:
        "14-day final check - last touchpoint before closing the lead",
      automationType: "COMMUNICATION",
      stageId: inquiryStage.id,
      channel: "EMAIL",
      projectType: "WEDDING",
