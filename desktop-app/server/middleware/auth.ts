...
export async function requireActiveSubscription(
  ...
  try {
    ...
    if (
      ...
      !activeStatuses.includes(photographer.subscriptionStatus)
    ) {
      return res.status(402).json({
        message: "Active subscription required",
        subscriptionStatus: photographer.subscriptionStatus,
        trialEnded: photographer.trialEndsAt
          ? new Date(photographer.trialEndsAt) < new Date()
          : false,
      });
    }
...
export async function requireGalleryPlan(
  req: Request,
  res: Response,
  next: NextFunction,
) {
