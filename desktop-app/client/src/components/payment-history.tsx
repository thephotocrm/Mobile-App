      ...
      <CardContent>
        <div className="space-y-3">
          {data.transactions.map((transaction) => {
            ...
            const baseAmount = transaction.amountCents - (transaction.tipCents || 0);
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <p className="font-medium text-sm" data-testid={`text-transaction-type-${transaction.id}`}>
                        {paymentType === 'DEPOSIT' ? 'Deposit Payment' : 
                         paymentType === 'BALANCE' ? 'Balance Payment' :
                         paymentType === 'FULL' ? 'Full Payment' : 'Payment'}
                      </p>
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-sm" data-testid={`text-transaction-amount-${transaction.id}`}>
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
                    <span className="capitalize">{transaction.paymentMethod.toLowerCase()}</span>
                    <span>•</span>
