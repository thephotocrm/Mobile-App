...
interface ButtonRichTextEditorProps {
  ...
  onEditButton?: (data: { marker: string; text: string; linkType: string; customUrl?: string }) => void;
}
  ...
  const processNode = (node: Node) => {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (element.tagName === 'BR') {
        ...
        const marker = decodeURIComponent(element.getAttribute('data-button-marker') || '');
        result += marker;
...
export function ButtonRichTextEditor({
  ...
  useLayoutEffect(() => {
    ...
    const shouldUpdate = prevValueRef.current === null || prevValueRef.current !== value;
  ...
  useEffect(() => {
    const checkAndConvert = () => {
      ...
      const hasPlainTextMarkers = /\[\[BUTTON:[A-Z_]+:[^\]]+\]\]/.test(editorText);
      ...
      if (hasPlainTextMarkers && !hasChips) {
        console.log('[ButtonRichTextEditor] Forcing chip conversion - detected plain text markers');
        isUpdatingFromProp.current = true;
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    if (target.classList.contains('chip-delete') || target.getAttribute('data-action') === 'delete') {
      e.preventDefault();
      e.stopPropagation();
    ...
    if (chip && onEditButton) {
      ...
      const marker = decodeURIComponent(chip.getAttribute('data-button-marker') || '');
      const text = decodeURIComponent(chip.getAttribute('data-button-text') || '');
      const linkType = chip.getAttribute('data-link-type') || 'CALENDAR';
      const customUrl = decodeURIComponent(chip.getAttribute('data-custom-url') || '');
      ...
      if (range.collapsed) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (e.key === 'Backspace' && offset > 0) {
            ...
            if (prevNode && (prevNode as HTMLElement).classList?.contains('button-chip')) {
              e.preventDefault();
          ...
          if (e.key === 'Delete' && offset < children.length) {
            ...
            if (nextNode && (nextNode as HTMLElement).classList?.contains('button-chip')) {
              e.preventDefault();
  ...
  const insertButton = useCallback((linkType: string, buttonText: string, customUrl?: string) => {
    if (!editorRef.current) return;

    // Build the marker string
    const marker = linkType === 'CUSTOM' && customUrl
  ...
  const updateButton = useCallback((oldMarker: string, newLinkType: string, newText: string, newCustomUrl?: string) => {
    if (!editorRef.current) return;

    // Find the chip with the matching marker
    const encodedOldMarker = encodeURIComponent(oldMarker);
    const chip = editorRef.current.querySelector(`[data-button-marker="${CSS.escape(encodedOldMarker)}"]`) as HTMLElement;
    if (!chip) return;

    // Build new marker
    const newMarker = newLinkType === 'CUSTOM' && newCustomUrl
    ...
    chip.setAttribute('data-custom-url', encodeURIComponent(newCustomUrl || ''));
...
export type ButtonRichTextEditorRef = {
  insertButton: (linkType: string, buttonText: string, customUrl?: string) => void;
