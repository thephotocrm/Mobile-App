...
} from "./payment-schedule-timeline";
...
function getSmartFileStatusColor(
  status: string,
...
function SmartFilePaymentCard({
  ...
  const timelineSchedule: TimelineInstallment[] = smartFile.schedule.map(
    (inst) => ({
      id: inst.id,
      label: inst.label,
  ...
  return (
    <Card
      ...
      data-testid={`smartfile-payment-card-${smartFile.smartFileId}`}
    >
      {/* Smart File Header */}
      <CardHeader className="pb-2 pt-4 px-4">
        <div className="flex items-center gap-2">
          ...
          <Badge
            variant={getSmartFileStatusColor(smartFile.status)}
            className="text-xs"
          >
            {smartFile.status}
          </Badge>
        ...
        {smartFile.schedule.length > 0 ? (
          <PaymentScheduleTimeline
            ...
            onPayClick={(installment) =>
              onPayInstallment(installment as unknown as PaymentInstallment)
            }
          />
        ) : (
          <div className="text-center py-6 text-gray-500">
            ...
            <Button variant="outline" onClick={onViewClick} className="mt-3">
              View Details
...
function TransactionHistorySection({
  ...
  const allTransactions = smartFiles
    .flatMap((sf) =>
  ...
  return (
    <Card
      className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700"
      data-testid="card-transaction-history"
    >
      <CardHeader>
...
export function ClientPortalPaymentsSection({
  projectId,
  onNavigateToSmartFile,
}: ClientPortalPaymentsSectionProps) {
      ...
      <Card className="bg-white border-gray-200">
        <CardContent className="pt-6">
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 mt-2">
              Your invoices and contracts will appear here.
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="space-y-4" data-testid="client-portal-payments-section">
      <div className="space-y-3">
        <h3 className="text-base font-semibold tracking-wide text-gray-900 dark:text-gray-100">
          Invoices & Contracts
        </h3>
        {data.smartFiles.map((smartFile) => (
          <SmartFilePaymentCard
            ...
            onPayInstallment={(installment) => {
              ...
              onNavigateToSmartFile(
                `${smartFile.token}?payInstallment=${installment.id}`,
              );
            }}
          />
        ))}
