...
function StripePaymentForm({
  ...
  onSaveCardChange: (enabled: boolean) => void;
  enableAutopay: boolean;
  onEnableAutopayChange: (enabled: boolean) => void;
}) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-start space-x-3 ml-6 p-3 bg-muted/50 rounded-lg">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                onEnableAutopayChange(checked as boolean)
              }
              data-testid="checkbox-enable-autopay"
            />
            <div className="grid gap-1.5 leading-none">
              <Label
                htmlFor="enable-autopay"
                className="text-sm font-medium cursor-pointer"
              >
                Enable autopay
              </Label>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground pt-4 border-t">
        ...
        <span>
          We use the same SSL encryption technology that banks use to protect
          your sensitive data.
        </span>
      </div>
...
export function EmbeddedPaymentForm(
  ...
  const {
    ...
    queryFn: async () => {
      ...
      console.log(
        "✅ Got payment intent, clientSecret starts with:",
        data.clientSecret?.substring(0, 30),
      );
      return data;
    },
    enabled:
      canCreatePayment && !selectedSavedMethodId && !props.existingClientSecret,
    retry: false,
    staleTime: 0,
  ...
  const effectiveClientSecret =
    props.existingClientSecret || paymentIntentData?.clientSecret;

  // Determine if we have fresh data (fetched in this session or provided externally)
  const hasFreshData =
    effectiveClientSecret &&
    (props.existingClientSecret || (dataUpdatedAt > 0 && !isFetching));
  ...
  if (
    !canCreatePayment ||
    (isLoading && !selectedSavedMethodId && !props.existingClientSecret) ||
    ...
    <div className="space-y-6">
            ...
            <>
              ...
              <div className="flex items-start space-x-3 p-3 bg-muted/50 rounded-lg">
                <Checkbox
                  ...
                  checked={enableAutopay}
                  onCheckedChange={(checked) =>
                    setEnableAutopay(checked as boolean)
                  }
                  data-testid="checkbox-enable-autopay-saved"
                />
                <div className="grid gap-1.5 leading-none">
                  <Label
                    htmlFor="enable-autopay-saved"
                    className="text-sm font-medium cursor-pointer"
                  >
      ...
      {!selectedSavedMethodId && (
        <>
          ...
          {(isLoading && !props.existingClientSecret) ||
          isFetching ||
          !hasFreshData ||
          !stripePromise ? (
            <div className="flex items-center justify-center py-8">
