...
interface PaymentPlanCardProps {
  token: string;
  onPaymentClick?: (installment: PaymentInstallment, clientSecret: string) => void;
  compact?: boolean;
}
  ...
  return (
    <div className="space-y-4" data-testid="installment-timeline">
      {schedule.map((installment, index) => {
        ...
        const remainingCents = installment.amountCents - (installment.amountPaidCents || 0);
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="font-medium text-sm" data-testid={`installment-description-${index}`}>
                    {installment.description}
                  </p>
                  <div className="flex items-center gap-2 mt-1">
                    <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                    <span className="text-xs text-muted-foreground" data-testid={`installment-date-${index}`}>
                ...
                <div className="text-right">
                  <p className={`font-semibold ${isPaid ? 'text-green-600 dark:text-green-400' : ''}`} data-testid={`installment-amount-${index}`}>
                    {formatPrice(installment.amountCents)}
                  </p>
                  {isPartial && (
                    <p className="text-xs text-muted-foreground" data-testid={`installment-remaining-${index}`}>
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-lg font-bold" data-testid="text-progress-percent">{percent}%</span>
      </div>
    </div>
  );
}

export function PaymentPlanCard({ token, onPaymentClick, compact = false }: PaymentPlanCardProps) {
  ...
  const createInstallmentPayment = useMutation({
    mutationFn: async (installmentId: string) => {
      const response = await apiRequest(`/api/public/smart-files/${token}/create-installment-payment`, {
        method: 'POST',
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-green-600 dark:text-green-400" data-testid="text-total-paid">
            ...
            <p className="text-xs text-muted-foreground" data-testid="text-payment-count">
        ...
        {summary.isFullyPaid && (
          <div className="p-4 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
            <div className="flex items-center gap-2 text-green-800 dark:text-green-400">
              ...
              <p className="font-medium" data-testid="text-fully-paid">All payments complete!</p>
            </div>
          </div>
        )}
          ...
          <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="font-semibold" data-testid="text-next-payment-amount">
                  {formatPrice(nextInstallment.amountCents - (nextInstallment.amountPaidCents || 0))}
                </p>
                <p className="text-xs text-muted-foreground" data-testid="text-next-payment-date">
              ...
              {onPaymentClick && (
                ...
                >
                  <CreditCard className="w-4 h-4 mr-2" />
                  {createInstallmentPayment.isPending ? 'Loading...' : 'Pay Now'}
                </Button>
              )}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {transactions.map((tx) => (
                ...
                >
                  ...
                  <span className="font-medium">{formatPrice(tx.amountCents)}</span>
                </div>
              ))}
            </div>
          </div>
...
export function PaymentPlanCompact({ 
  ...
  return <PaymentPlanCard token={token} onPaymentClick={onPaymentClick} compact />;
}
