...
export function DelayTimingEditor({ 
  ...
  const isBeforeTiming = value.delayMinutes < 0 || value.delayHours < 0 || value.delayDays < 0;
  ...
  const handleDelayMinutesChange = (inputValue: string) => {
    ...
    const signedMinutes = isBeforeTiming ? -Math.abs(minutes) : Math.abs(minutes);
    onChange({ 
      ...value, 
  ...
  const getTimingDescription = () => {
    if (anchorType === 'STAGE_ENTRY') {
      ...
      return isBeforeTiming ? 'Time before booking starts' : 'Time after booking starts';
    } else {
      return isBeforeTiming ? 'Time before booking ends' : 'Time after booking ends';
    }
  };
