  ...
  const createCampaignMutation = useMutation({
    ...
    onSuccess: (campaign) => {
      toast({
        ...
        description: "Now let's add your emails.",
      });
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="flex items-center justify-center gap-2 py-4">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center">
              ...
              {s < 2 && (
                <div
                  className={`w-16 h-0.5 mx-2 ${step > s ? "bg-green-500" : "bg-muted"}`}
                />
              )}
            </div>
          ))}
        ...
        {step === 2 && (
          <div className="space-y-4 py-4">
            ...
            {/* Stage Chips */}
            <ScrollArea
              className={`${isGlobal ? "opacity-50 pointer-events-none" : ""}`}
            >
            ...
            {!isGlobal && selectedStageIds.length > 0 && (
              <p className="text-xs text-muted-foreground">
                {selectedStageIds.length} stage
                {selectedStageIds.length !== 1 ? "s" : ""} selected
              </p>
            )}
          </div>
        )}
