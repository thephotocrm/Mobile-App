...
export default function PublicSmartFile() {
  ...
  // PAY page state
  const [selectedTip, setSelectedTip] = useState<number | "custom" | null>(
    null,
  );
  const [customTipAmount, setCustomTipAmount] = useState<string>("");
  const [payPageAutopayEnabled, setPayPageAutopayEnabled] = useState(false);
  const [payPageClientSecret, setPayPageClientSecret] = useState<string | null>(
    null,
  );
  ...
  const [payPageResult, setPayPageResult] = useState<PaymentResult | null>(
    null,
  );
  ...
  const [pendingClientSignature, setPendingClientSignature] = useState<
    string | null
  >(null);
  const [acceptedTerms, setAcceptedTerms] = useState(false);
  const [expiredData, setExpiredData] = useState<ExpiredSmartFileData | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const hasValidInstallmentCount =
      savedConfig?.selectedInstallments &&
  ...
  useEffect(() => {
    ...
    const hasValidSchedule =
      savedSchedule && Array.isArray(savedSchedule) && savedSchedule.length > 0;
    ...
    data?.projectSmartFile?.paymentSchedule,
    data?.projectSmartFile?.paymentScheduleConfig,
    selectionsRehydrated,
  ]);
  ...
  useEffect(() => {
    ...
    const savedSchedule = data?.projectSmartFile?.paymentSchedule as
      | PaymentInstallment[]
      | undefined;
    const hasValidSavedSchedule =
      savedSchedule &&
      Array.isArray(savedSchedule) &&
  ...
  const { data: existingBookingData, isLoading: isLoadingBooking } = useQuery<{
    booking: any;
    photographer: any;
  } | null>({
  ...
  useEffect(() => {
    if (data && freshPackages && freshAddOns) {
      ...
      const sortedPages = [...mergedPages].sort(
  ...
  useEffect(() => {
    if (data && freshPackages && freshAddOns) {
      ...
      console.log(
        "📄 [PAGE NAVIGATION] Page changed to index:",
        currentPageIndex,
        "pageType:",
        currentPage?.pageType,
      );
      console.log(
        "📄 [PAGE NAVIGATION] Total pages:",
        sortedPages.length,
        "Page types:",
        sortedPages.map((p) => p.pageType),
      );
    }
  ...
  useEffect(() => {
    if (
      data?.projectSmartFile.status === "ACCEPTED" &&
      payPageNeedsAcceptance
    ) {
      console.log(
        "💰 [PAYMENT INTENT] Smart file is now ACCEPTED - clearing payPageNeedsAcceptance flag",
      );
    ...
    return data.smartFile.pages.map((page) => {
      ...
      if (page.pageType === "PACKAGE") {
        ...
        if (page.content.packages) {
          ...
          const mergedPackages = page.content.packages.map(
            (snapshotPkg: any) => {
              const freshPkg = freshPackages.find(
                (fp) => fp.id === snapshotPkg.id,
              );
          ...
          const packages = page.content.packageIds
            .map((id: string) =>
              freshPackages.find((pkg: any) => pkg.id === id),
            )
      ...
      if (page.pageType === "ADDON" && page.content.addOnIds) {
        const addOns = page.content.addOnIds
          .map((id: string) =>
            freshAddOns.find((addon: any) => addon.id === id),
          )
  ...
  const paymentPage = data?.smartFile.pages?.find(
    (p: SmartFilePage) => p.pageType === "PAYMENT",
  );
  ...
  const acceptMutation = useMutation({
    mutationFn: async (acceptanceData: any) => {
      ...
      await apiRequest(
        "PATCH",
        `/api/public/smart-files/${params?.token}/accept`,
        acceptanceData,
      );
    ...
    onSuccess: () => {
      ...
      console.log(
        "✅ [ACCEPT MUTATION] Success - cleared payment errors and reset fetch guard",
      );
    ...
    onError: (error: any) => {
      ...
      toast({
        title: "Error",
        description:
          error.message || "Failed to accept Smart File. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const resetSelectionsMutation = useMutation({
    mutationFn: async () => {
      await apiRequest(
        "POST",
        `/api/public/smart-files/${params?.token}/reset-selections`,
        {},
      );
      return { reset: true };
    },
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "You can now edit your selections. Please re-sign the contract when done.",
      });
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description:
          error.message || "Failed to reset selections. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const saveSignatureMutation = useMutation({
    mutationFn: async (signatureData: {
      ...
      await apiRequest(
        "PATCH",
        `/api/public/smart-files/${params?.token}/sign`,
        {
          ...signatureData,
          contractHtml,
        },
  ...
  const createCheckoutMutation = useMutation({
    mutationFn: async (paymentType: "DEPOSIT" | "FULL" | "BALANCE") => {
      const response = await apiRequest(
        "POST",
        `/api/public/smart-files/${params?.token}/create-checkout`,
        {
          paymentType,
        },
      );
  ...
  const savePaymentScheduleMutation = useMutation({
    mutationFn: async (scheduleData: {
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/public/smart-files/${params?.token}/payment-schedule`,
        scheduleData,
      );
      return response.json();
    },
  ...
  const submitFormMutation = useMutation({
    mutationFn: async (formData: { answers: Record<string, any> }) => {
      await apiRequest(
        "PATCH",
        `/api/public/smart-files/${params?.token}/form-answers`,
        formData,
      );
    },
    onSuccess: () => {
  ...
  const bookingMutation = useMutation({
    mutationFn: async (bookingData: {
      ...
      if (existingBookingData?.booking) {
        throw new Error(
          "An appointment has already been booked for this Smart File",
        );
      }

      const response = await apiRequest(
        "POST",
        `/api/public/smart-files/${params?.token}/booking`,
        {
  ...
  useEffect(() => {
    ...
    console.log(
      "✅ [AUTO-ACCEPT] On PAY page - checking if auto-accept should trigger",
    );

    const isAccepted = ["ACCEPTED", "DEPOSIT_PAID", "PAID"].includes(
      data.projectSmartFile.status,
    );
    const contractPage = sortedPages.find((p) => p.pageType === "CONTRACT");
    const requiresClientSignature =
      contractPage && contractPage.content.requireClientSignature !== false;
    const clientHasSigned = !!data.projectSmartFile.clientSignatureUrl;
    const canPay = !requiresClientSignature || clientHasSigned;

    console.log(
      "✅ [AUTO-ACCEPT] Status check - isAccepted:",
      isAccepted,
      "canPay:",
      canPay,
    ...
    console.log(
      "✅ [AUTO-ACCEPT] Selection check - hasPackagePages:",
      hasPackagePages,
  ...
  const { subtotal, depositAmount, total, depositPercent } = useMemo(() => {
    ...
    const hasPackageSelections =
      selectedPackages.size > 0 || selectedAddOns.size > 0;
    console.log(
      "🔢 [TOTAL CALC] hasPackageSelections:",
    ...
    if (hasPackageSelections) {
      ...
      console.log(
        "🔢 [TOTAL CALC] Proposal mode - total from packages/addons:",
        subtotalCents,
      );
    } else {
      ...
      const mergedPages = getMergedPages();
      console.log(
        "🔢 [TOTAL CALC] Invoice-only mode - mergedPages count:",
        mergedPages.length,
      );
      const invoicePage = mergedPages.find(
        (p: any) => p.pageType === "INVOICE",
      );
      console.log("🔢 [TOTAL CALC] Invoice page found:", !!invoicePage);
      if (invoicePage) {
        console.log(
          "🔢 [TOTAL CALC] Invoice line items:",
          invoicePage.content?.lineItems?.length || 0,
        );
      ...
      if (invoicePage?.content?.lineItems?.length > 0) {
        ...
        console.log(
          "🔢 [TOTAL CALC] Calculated total from invoice:",
          subtotalCents,
        );
      } else {
        console.log(
          "🔢 [TOTAL CALC] ⚠️ No invoice page or line items found - total will be 0",
        );
      }
    }
    ...
    const invoicePageForDeposit = allMergedPages.find(
      (p: any) => p.pageType === "INVOICE",
    );
    const paymentPageForDeposit = allMergedPages.find(
      (p: any) => p.pageType === "PAY" || p.pageType === "PAYMENT",
    );
    const depositPct =
      invoicePageForDeposit?.content?.depositPercent ??
  ...
  const handleAccept = () => {
    ...
    const mergedPages = getMergedPages();
    const hasPackagePages = mergedPages.some(
      (page) => page.pageType === "PACKAGE",
    );
        ...
        <Card className="max-w-md mx-4">
          <CardContent className="pt-8 pb-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Contact {expiredData.photographerContact.businessName} to request
              a new proposal.
            </p>
  ...
  const hasContractPage = !!contractPage;
  const requiresClientSignature =
    hasContractPage && contractPage.content.requireClientSignature !== false;
  const clientHasSigned = !!data.projectSmartFile.clientSignatureUrl;
  const paymentPageIndex = sortedPages.findIndex(
    (p) => p.pageType === "PAYMENT",
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      {isPreviewMode && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-amber-500 text-white text-center py-2 px-4 font-medium shadow-md">
          ...
          Preview Mode - Actions are disabled. This is how clients will see your
          Smart File.
        </div>
      )}

      {/* Expiration Banner - Show when proposal has expiration date and is not yet accepted */}
      {data.projectSmartFile.expiresAt &&
        !isAccepted &&
        (() => {
            ...
            >
              ...
              {daysRemaining <= 0
                ...
                : daysRemaining === 1
                  ...
                  : `This proposal expires on ${expiresAt.toLocaleDateString(
                      "en-US",
                      {
                        weekday: "long",
                        month: "long",
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1
                  ...
                  data-testid="text-photographer-name"
                >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {requiresClientSignature &&
            ...
            data.projectSmartFile.selectedPackages && (
              <div className="mb-6">
                <div
                  className="bg-amber-50 dark:bg-amber-950/20 border-2 border-amber-500 rounded-lg p-4"
                  data-testid="banner-resigning-required"
                >
          ...
          <div>
            {sortedPages.length === 0 ? (
              ...
              <div className="space-y-6">
                <div
                  key={currentPage.id}
                  data-testid={`page-${currentPage.pageType.toLowerCase()}-${pageIndex}`}
                  className="relative"
                >
                  {/* TEXT Page */}
                  {currentPage.pageType === "TEXT" && (
                    <>
                      ...
                      {currentPage.content.hero?.backgroundImage ? (
                        <div className="space-y-6">
                          ...
                          currentPage.content.sections.length > 0 ? (
                            ...
                            <p
                              ...
                              data-testid={`text-page-content-${pageIndex}`}
                            >
                        ...
                        <Card>
                          <CardHeader>
                            <CardTitle
                              data-testid={`text-page-title-${pageIndex}`}
                            >
                        ...
                        <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg mb-4">
                          <div className="flex items-center gap-2 text-amber-800 dark:text-amber-400">
                            ...
                            <p className="font-medium">
                              Selections Locked - Contract Signed
                            </p>
                          </div>
                          <p className="text-sm text-amber-700 dark:text-amber-500 mt-1">
                            Package selections are locked after signing the
                            contract to preserve contract integrity.
                          </p>
                        </div>
                      )}
                      <div className="space-y-4">
                      ...
                      <CardContent className="pt-6 space-y-4">
                        {selectionsLocked && (
                          <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                            ...
                            <p className="text-sm text-amber-700 dark:text-amber-500 mt-1">
                              Add-on selections are locked after signing the
                              contract to preserve contract integrity.
                            </p>
                          </div>
                        )}
                        {currentPage.content.addOns?.map(
                          (addOn: any, addonIdx: number) => {
                            const key = `${currentPage.id}-${addOn.id}`;
                            const isSelected = selectedAddOns.has(key);
                            const quantity =
                              selectedAddOns.get(key)?.quantity || 1;
                            ...
                            `}
                                ...
                                onClick={() =>
                                  !selectionsLocked &&
                                  !isSelected &&
                                  handleAddOnToggle(currentPage, addOn, true)
                                }
                              >
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <ContractRenderer
                              ...
                              variables={{
                                ...
                                photographer_address:
                                  data?.photographer.businessAddress || "",

                                // Project details
                                project_date: data?.project.eventDate
                                  ? new Date(
                              ...
                              selectedPackages={(() => {
                                ...
                                selectedPackages.forEach((pkg, key) => {
                                  ...
                                  const freshPkg = freshPackages?.find(
                                    (fp) => fp.id === pkg.packageId,
                                  );

                                  // Get page snapshot data (features) from merged pages
                                  const packagePage = sortedPages.find(
                                    (p) =>
                                      p.pageType === "PACKAGE" &&
                                      p.id === pkg.pageId,
                                  );
                                  const snapshotPkg =
                                    packagePage?.content?.packages?.find(
                                      (p: any) => p.id === pkg.packageId,
                                    );

                                  enrichedPackages.set(key, {
                                    ...pkg,
                                    description:
                                      freshPkg?.description ||
                                      snapshotPkg?.description ||
                                      pkg.description,
                                    features:
                                      snapshotPkg?.features ||
                                      pkg.features ||
                                      [],
                                    imageUrl:
                                      freshPkg?.imageUrl ||
                                      snapshotPkg?.imageUrl ||
                                      pkg.imageUrl,
                                  });
                                });
                              ...
                              selectedAddOns={(() => {
                                ...
                                selectedAddOns.forEach((addon, key) => {
                                  // Get fresh global data (description, imageUrl)
                                  const freshAddon = freshAddOns?.find(
                                    (fa) => fa.id === addon.addOnId,
                                  );

                                  // Get page snapshot data from merged pages
                                  const addonPage = sortedPages.find(
                                    (p) =>
                                      p.pageType === "ADDON" &&
                                      p.id === addon.pageId,
                                  );
                                  const snapshotAddon =
                                    addonPage?.content?.addOns?.find(
                                  ...
                                  enrichedAddOns.set(key, {
                                    ...addon,
                                    description:
                                      freshAddon?.description ||
                                      snapshotAddon?.description ||
                                      addon.description,
                                    imageUrl:
                                      freshAddon?.imageUrl ||
                                      snapshotAddon?.imageUrl ||
                                      addon.imageUrl,
                                  });
                                });
                          ...
                          {isPreviewMode ? (
                            ...
                            <div className="space-y-6">
                              ...
                              {/* Client Signature */}
                              {currentPage.content.requireClientSignature !==
                                false && (
                                <div className="space-y-3">
                                  ...
                                  {data?.projectSmartFile.clientSignatureUrl ? (
                                    <div className="border rounded-lg p-4 bg-muted/20">
                                      ...
                                      <p className="text-xs text-muted-foreground text-center mt-2">
                                        Signed on{" "}
                                        {new Date(
                                          data.projectSmartFile.clientSignedAt!,
                                        ).toLocaleDateString()}
                                      </p>
                                    </div>
                                  ) : (
                              ...
                              <div className="flex items-start space-x-3 p-4 bg-primary/5 rounded-lg border border-primary/20">
                                ...
                                >
                                  I have read and agree to the terms outlined in
                                  this service agreement. My signature above
                                  confirms my acceptance of all terms and
                                  conditions.
                                </Label>
                              </div>
                            </div>
                      ...
                      {!canAccessPayment ? (
                        <Card className="border-2 border-amber-500/50">
                          <CardContent className="p-8 text-center space-y-4">
                            ...
                            <p className="text-muted-foreground">
                              You must sign the contract before you can proceed
                              with payment.
                            </p>
                            <Button
                              onClick={() => {
                                const contractIndex = sortedPages.findIndex(
                                  (p) => p.pageType === "CONTRACT",
                                );
                                if (contractIndex >= 0) {
                              ...
                              <div className="space-y-3 pb-6 border-b">
                                <details className="group">
                                  ...
                                  <div className="mt-4 space-y-3 text-sm">
                                    {Array.from(selectedPackages.values()).map(
                                      (pkg) => (
                                        <div
                                        ...
                                        >
                                          <span
                                            ...
                                            data-testid="text-selected-package"
                                          >
                                            {pkg.name}
                                          </span>
                                          <span
                                            className="font-medium"
                                            data-testid="text-selected-package-price"
                                          >
                                            {formatPrice(pkg.priceCents)}
                                          </span>
                                        </div>
                                    ...
                                    {Array.from(selectedAddOns.values()).map(
                                      (addOn) => (
                                        <div
                                        ...
                                        >
                                          ...
                                          <span className="font-medium">
                                            {formatPrice(
                                              addOn.priceCents * addOn.quantity,
                                            )}
                                          </span>
                                        </div>
                                      ),
                              ...
                              <div className="space-y-6">
                                  ...
                                  <div className="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                      <h3 className="text-xl font-semibold text-green-800 dark:text-green-400">
                                        ...
                                        data.projectSmartFile.depositCents > 0
                                          ? "Full Balance Paid!"
                                          : "Payment Complete!"}
                                      </h3>
                                ...
                                <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">
                                      {formatPrice(
                                        data.projectSmartFile.amountPaidCents ||
                                          0,
                                      )}
                                    </span>
                                  </div>
                                  {data.projectSmartFile.tipCents &&
                                    data.projectSmartFile.tipCents > 0 && (
                                      <div className="flex justify-between text-sm">
                                        <span className="text-muted-foreground">
                                          Tip
                                        </span>
                                    ...
                                    <DialogContent className="max-w-md">
                                      <DialogHeader>
                                        <DialogTitle>
                                          Pay{" "}
                                          {
                                            installmentPaymentData.installment
                                              .description
                                          }
                                        </DialogTitle>
                                        <DialogDescription>
                                      ...
                                      <EmbeddedPaymentForm
                                        ...
                                        existingClientSecret={
                                          installmentPaymentData.clientSecret
                                        }
                                        onSuccess={() => {
                                          setInstallmentPaymentData(null);
                                          queryClient.invalidateQueries({
                                            queryKey: [
                                              "/api/public/smart-files",
                                              params?.token,
                                              "payment-schedule",
                                            ],
                                          });
                                    ...
                                    <div className="space-y-4">
                                      {(() => {
                                        ...
                                        const multiPaymentOptions =
                                          allowedCounts.filter((n) => n >= 2);

                                        // For generating schedule previews
                                        const previewEventDate =
                                          eventDate ||
                                          new Date(
                                            Date.now() +
                                              180 * 24 * 60 * 60 * 1000,
                                        ...
                                        const onlyFullPaymentAvailable =
                                          multiPaymentOptions.length === 0;
                                        const eventIsSoon =
                                          eventDate && daysUntilFinal <= 30;
                                          ...
                                          <>
                                            <div>
                                              ...
                                              <p className="text-xs text-muted-foreground">
                                                Select how you'd like to split
                                                your payments
                                                {!eventDate &&
                                                  " (dates shown are estimates)"}
                                              </p>
                                              {eventIsSoon &&
                                                !onlyFullPaymentAvailable && (
                                                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                                                    Your event is coming up
                                                    soon, so some payment
                                                    options are limited.
                                                  </p>
                                                )}
                                              {onlyFullPaymentAvailable &&
                                                eventDate && (
                                                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                                                    Your event is very close, so
                                                    only full payment is
                                                    available.
                                                  </p>
                                                )}
                                            </div>
                                            <div className="grid gap-3">
                                              ...
                                              {multiPaymentOptions.length <
                                                maxConfigured - 1 &&
                                                eventDate &&
                                                multiPaymentOptions.length >
                                                  0 && (
                                                  <p className="text-xs text-muted-foreground text-center italic">
                                                    Some payment plans aren't
                                                    available because your event
                                                    is coming up soon.
                                                  </p>
                                                )}
                                    ...
                                    <div className="space-y-4">
                                      ...
                                      <div className="bg-muted/50 rounded-lg p-4 space-y-3">
                                        ...
                                        <p className="text-xs text-muted-foreground italic">
                                          This schedule is flexible - you can
                                          ...
                                          onClick={async () => {
                                            ...
                                            if (
                                              ...
                                              toast({
                                                ...
                                                description:
                                                  "Choose how many payments you'd like to split your total into.",
                                                variant: "destructive",
                                              });
                                              return;
                                            }
                                            ...
                                            try {
                                              ...
                                              if (rawConfig) {
                                                if (
                                                  ...
                                                  try {
                                                    ...
                                                    console.error(
                                                      "Failed to parse paymentScheduleConfig:",
                                                      e,
                                                    );
                                                  }
                                                } else {
                                              ...
                                              const hasValidSchedule =
                                                ...
                                                savedConfig?.selectedInstallments ===
                                                  installmentCount &&
                                                typeof (
                                                  savedSchedule as PaymentInstallment[]
                                                )[0]?.amountCents === "number";
                                                ...
                                                console.error(
                                                  "Schedule validation failed:",
                                                  {
                                                    ...
                                                    expectedInstallments:
                                                      installmentCount,
                                                    savedConfigInstallments:
                                                      savedConfig?.selectedInstallments,
                                                    hasAmountCents:
                                                      typeof savedSchedule?.[0]
                                                        ?.amountCents,
                                                  },
                                                );
                                                throw new Error(
                                                  "Schedule not found in server response after save",
                                                );
                                              }
                                              ...
                                              toast({
                                                title:
                                                  "Error Saving Payment Plan",
                                                description:
                                                  error?.message ||
                                                  "Failed to save payment plan. Please try again.",
                                                variant: "destructive",
                                              });
                                            }
                                          }}
                                    ...
                                    {(!currentPage.content
                                      ...
                                      !selectedPaymentOption && (
                                        <EmbeddedPaymentForm
                                          ...
                                          onSuccess={() => {
                                            setLocation(
                                              `/smart-file/${params?.token}/success`,
                                            );
                                          }}
                                          onError={(error) => {
                                            toast({
                    ...
                    <div className="max-w-4xl mx-auto px-4 md:px-8">
                      ...
                      <div className="bg-white rounded-lg shadow-sm border p-8 mb-4">
                        ...
                        selectedAddOns.size > 0 ? (
                          ...
                          currentPage.content.lineItems.length > 0 && (
                            <table className="w-full mb-8">
                              <thead>
                                <tr className="border-b-2 border-gray-200">
                                  <th className="text-left py-3 text-sm font-semibold uppercase tracking-wide">
                              ...
                              <tbody>
                                {currentPage.content.lineItems.map(
                          ...
                          <div className="w-80 space-y-2">
                              ...
                              <div className="flex justify-between py-2 text-sm">
                                ...
                                <span className="font-medium">
                                  ...
                                  {(
                                    Math.round(
                                      subtotal *
                                        (currentPage.content.taxPercent / 100),
                                    ) / 100
                                  ).toFixed(2)}
                                </span>
                              </div>
                            ...
                            {currentPage.content.discountAmount > 0 && (
                              <div className="flex justify-between py-2 text-sm text-green-600">
                                <span>
                                  Discount{" "}
                                  {currentPage.content.discountType ===
                                  "PERCENT"
                          ...
                          <div className="text-sm text-muted-foreground">
                            ...
                            "CLIENT_CHOICE" ? (
                              <div className="space-y-2">
                                ...
                                <p>
                                  Select up to{" "}
                                  {currentPage.content.paymentScheduleConfig
                                    ?.maxInstallments || 6}{" "}
                                  installments to spread out your payments.
                                </p>
                              </div>
                            ) : (
                              <div className="space-y-2">
                                <div className="flex items-center gap-4">
                                  <div className="flex-1 p-3 bg-muted/50 rounded-lg">
                                    ...
                                    <div className="font-semibold text-foreground">
                                      {currentPage.content.depositPercent || 50}
                                      % upon booking
                                    </div>
                                  </div>
                                  <div className="flex-1 p-3 bg-muted/50 rounded-lg">
                  ...
                  {currentPage.pageType === "PAY" &&
                    (() => {
                      ...
                      const hasProposalPages = sortedPages.some(
                        (p) =>
                          p.pageType === "PACKAGE" || p.pageType === "ADDON",
                      );
                      const hasPackageSelections =
                        selectedPackages.size > 0 || selectedAddOns.size > 0;
                      const invoicePage = sortedPages.find(
                          ...
                          <div className="max-w-lg mx-auto px-4 md:px-0">
                            <Card className="overflow-hidden">
                              <CardContent className="p-8 text-center">
                                ...
                                <p className="text-muted-foreground mb-4">
                                  Please go back and select a package before
                                  proceeding to payment.
                                </p>
                                <Button
                                  variant="outline"
                                  onClick={() => {
                                    const pkgPageIndex = sortedPages.findIndex(
                                      (p) => p.pageType === "PACKAGE",
                                    );
                                    if (pkgPageIndex >= 0)
                                      setCurrentPageIndex(pkgPageIndex);
                                  }}
                                >
                      ...
                      console.log(
                        "💳 [PAY PAGE] hasProposalPages:",
                        hasProposalPages,
                        "hasPackageSelections:",
                        hasPackageSelections,
                      );
                      console.log(
                        "💳 [PAY PAGE] invoicePage found:",
                        !!invoicePage,
                        "invoiceContent:",
                        invoiceContent,
                      );
                      ...
                      // Check payment state
                      const amountPaid =
                        data.projectSmartFile.amountPaidCents || 0;
                      const savedSchedule = data.projectSmartFile
                        .paymentSchedule as PaymentInstallment[] | undefined;
                      console.log(
                        "💳 [PAY PAGE] amountPaid:",
                        amountPaid,
                        "savedSchedule:",
                      ...
                      let totalPayments =
                        paymentScheduleMode === "SIMPLE"
                          ...
                          : invoiceContent?.paymentScheduleConfig
                              ?.maxInstallments || 2;
                      let amountDue = Math.round(
                        effectiveTotal * (depositPct / 100),
                      );
                      let dueDate = new Date();
                      let paymentType: "DEPOSIT" | "BALANCE" = "DEPOSIT";
                      console.log(
                        "💳 [PAY PAGE] Initial amountDue calculated:",
                        amountDue,
                        "(from effectiveTotal:",
                        effectiveTotal,
                        "* depositPct:",
                      ...
                      if (
                        ...
                        const nextUnpaid = savedSchedule.find(
                          (i) =>
                        ...
                        if (nextUnpaid) {
                          ...
                          amountDue =
                            nextUnpaid.amountCents -
                            (nextUnpaid.amountPaidCents || 0);
                          dueDate = new Date(nextUnpaid.dueDate);
                          paymentType =
                            paymentNumber === 1 ? "DEPOSIT" : "BALANCE";
                        } else {
                          // All paid
                        ...
                        amountPaid > 0 &&
                        amountPaid >=
                          Math.round(effectiveTotal * (depositPct / 100))
                      ) {
                      ...
                      if (
                        ...
                        console.log(
                          "💰 [PAYMENT INTENT] ✅ Starting fetch - setting ref guard",
                        );
                        ...
                        (async () => {
                          ...
                          console.log(
                            "💰 [PAYMENT INTENT] Fetching payment intent with:",
                            {
                              paymentType,
                              tipCents,
                          ...
                          try {
                            const response = await fetch(
                              `/api/public/smart-files/${params?.token}/create-payment-intent`,
                              {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                            ...
                            if (!response.ok) {
                              ...
                              if (
                                errorMessage
                                  .toLowerCase()
                                  .includes("must be accepted")
                              ) {
                                console.log(
                                  "💰 [PAYMENT INTENT] ⚠️ Smart file needs acceptance - will not retry",
                                );
                            ...
                            console.log(
                              "💰 [PAYMENT INTENT] ✅ Payment intent created successfully:",
                              result,
                            );
                            setPayPageError(null);
                        ...
                        console.log(
                          "💰 [PAYMENT INTENT] ❌ Conditions not met - skipping fetch",
                          {
                            ...
                            <Card className="overflow-hidden">
                              <CardContent className="p-8 text-center">
                                ...
                                <p className="text-muted-foreground">
                                  ...
                                  {formatCentsAsDollars(effectiveTotal)} has
                                  been received.
                                </p>
                              </CardContent>
                            </Card>
                                ...
                                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                                  <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-xl font-bold text-green-600">
                                      {formatCentsAsDollars(totalWithTip)}
                                    </span>
                                  </div>
                                  {tipCents > 0 && (
                                  ...
                                  {balanceRemaining > 0 && (
                                    <div className="flex justify-between items-center pt-2 border-t">
                                      ...
                                      <span className="font-semibold text-amber-600">
                                        {formatCentsAsDollars(balanceRemaining)}
                                      </span>
                                    </div>
                                  )}
                                </div>
                                  ...
                                  <div className="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-mono text-xs bg-gray-200 px-2 py-0.5 rounded">
                                        {payPageResult?.transactionId?.slice(
                                          0,
                                          12,
                                        ) || "Processing..."}
                                      </span>
                                    ...
                                    {payPageResult?.paymentMethod && (
                                      <div className="flex justify-between">
                                        ...
                                        <span className="flex items-center gap-1">
                                          ...
                                          {capitalizeFirst(
                                            payPageResult.paymentMethod.brand,
                                          )}{" "}
                                          ****
                                          {payPageResult.paymentMethod.last4}
                                        </span>
                                      </div>
                                    )}
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-xs text-blue-700">
                                          Check your inbox for the payment
                                          confirmation
                                        </p>
                                      </div>
                                    </div>
                            ...
                            <CardContent className="p-6 space-y-6">
                              ...
                              <div className="text-center pb-4 border-b">
                                ...
                                <div className="text-4xl font-bold">
                                  {formatCentsAsDollars(amountDue)}
                                </div>
                                {tipCents > 0 && (
                                  <div className="text-sm text-muted-foreground mt-1">
                                    + {formatCentsAsDollars(tipCents)} tip ={" "}
                                    <span className="font-semibold">
                                      {formatCentsAsDollars(totalWithTip)}
                                    </span>
                                  </div>
                                ...
                                {effectiveTotal > 0 &&
                                  ...
                                  !tipCents && (
                                    <div className="text-xs text-muted-foreground mt-1">
                                      Total:{" "}
                                      {formatCentsAsDollars(effectiveTotal)}
                                    </div>
                                  )}
                              </div>
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="flex gap-2 flex-wrap">
                                  ...
                                  <Button
                                    variant={
                                      selectedTip === 18 ? "default" : "outline"
                                    }
                                    size="sm"
                                  ...
                                  <Button
                                    variant={
                                      selectedTip === 20 ? "default" : "outline"
                                    }
                                    size="sm"
                                    className="flex-1 min-w-[50px]"
                                    disabled={isPreviewMode}
                                  ...
                                  <Button
                                    variant={
                                      selectedTip === 25 ? "default" : "outline"
                                    }
                                    size="sm"
                                    className="flex-1 min-w-[50px]"
                                    disabled={isPreviewMode}
                              ...
                              <div className="flex items-start gap-3 p-4 bg-muted/30 rounded-lg">
                                <Checkbox
                                  ...
                                  disabled={isPreviewMode}
                                  onCheckedChange={(checked) =>
                                    setPayPageAutopayEnabled(!!checked)
                                  }
                                />
                                <div className="flex-1">
                                  <label
                                  ...
                                  <p className="text-xs text-muted-foreground mt-1">
                                    Automatically pay future installments with
                                    this payment method
                                  </p>
                                </div>
                              </div>
                  ...
                  {currentPage.pageType === "FORM" && (
                    <Card>
                      <CardHeader>
                        <CardTitle data-testid={`form-page-title-${pageIndex}`}>
                          {currentPage.displayTitle}
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                                                  ...
                                                  <div className="space-y-2">
                                                    ...
                                                    {field.fieldType ===
                                                      ...
                                                      field.options && (
                                                        <div
                                                          className="space-y-2"
                                                          data-testid={`checkbox-group-field-${blockIdx}`}
                                                        >
                                                          {field.options.map(
                                                            (
                                                              option: string,
                      ...
                      {isLoadingBooking ? (
                        ...
                        <Card data-testid="card-booking-success">
                          <CardContent className="pt-6">
                            <div className="text-center space-y-4">
                              ...
                              {existingBookingData?.photographer
                                ?.headshotUrl && (
                              ...
                              <div className="bg-muted/50 dark:bg-muted/20 rounded-lg p-4 space-y-2">
                                <div className="flex items-center justify-center gap-2 text-sm">
                                  ...
                                  <span data-testid="text-booking-date">
                                    {existingBookingData?.booking
                                      ? new Date(
                                          existingBookingData.booking.startAt,
                                        ).toLocaleDateString("en-US", {
                                          weekday: "long",
                                          month: "long",
                                          day: "numeric",
                                ...
                                <div className="flex items-center justify-center gap-2 text-sm">
                                  ...
                                  <span data-testid="text-booking-time">
                                    {existingBookingData?.booking
                                      ...
                                      : bookingDetails &&
                                        new Date(
                                          `2000-01-01T${bookingDetails.time}`,
                                        ).toLocaleTimeString("en-US", {
                                          hour: "numeric",
                                          minute: "2-digit",
                                          hour12: true,
                              ...
                              {bookingSuccess && (
                                <p className="text-sm text-muted-foreground">
                                  A confirmation email has been sent to you with
                                  all the details.
                                </p>
                              )}
                            </div>
                        ...
                        <SchedulingCalendar
                          ...
                          showPhotographerProfile={
                            currentPage.content.showPhotographerProfile ?? true
                          }
                          onBookingConfirm={(date, time) => {
                            ...
                            bookingMutation.mutate({
                              ...
                              time,
                              durationMinutes:
                                currentPage.content.durationMinutes || 60,
                            });
                          }}
                        />
                      )}
                ...
                {["ACCEPTED", "DEPOSIT_PAID"].includes(
                  data.projectSmartFile.status,
                ) &&
                  sortedPages.some(
                    ...
                    <div className="mt-6">
                      ...
                      >
                        {resetSelectionsMutation.isPending
                          ? "Resetting..."
                          : "Edit Selections"}
                      </Button>
                    </div>
                  ...
                  paymentPageIndex !== -1 && (
                    <div className="mt-6">
                      ...
                      >
                        ...
                        {formatPrice(
                          data.projectSmartFile.balanceDueCents || 0,
                        )}
                        )
                      </Button>
                    </div>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    {!canAccessPayment &&
                      currentPageIndex + 1 === paymentPageIndex && (
                        <p
                          className="text-xs text-muted-foreground"
                          data-testid="text-signature-required"
                        >
                    ...
                    <Button
                      onClick={async () => {
                        ...
                        if (isPreviewMode) {
                          setCurrentPageIndex(
                            Math.min(sortedPages.length - 1, nextIndex),
                          );
                          return;
                        }
                        ...
                        if (
                          ...
                          const signatureData =
                            signaturePadRef.current?.getCanvasData();
                          ...
                          if (!signatureData) {
                            toast({
                              ...
                              description:
                                "Please sign the contract before continuing.",
                              variant: "destructive",
                            });
                            return;
                          }
                          ...
                          try {
                            ...
                            await queryClient.refetchQueries({
                              queryKey: [
                                `/api/public/smart-files/${params?.token}`,
                              ],
                            });
                          } catch (err) {
                        ...
                        if (
                          nextIndex === paymentPageIndex &&
                          !canAccessPayment
                        ) {
                          toast({
                            title: "Signature Required",
                            description:
                              "Please sign the contract before proceeding to payment.",
                            variant: "destructive",
                          });
                          return;
                        }
