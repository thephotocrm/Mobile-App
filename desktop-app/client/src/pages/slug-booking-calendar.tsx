...
function convertTimeBetweenTimezones(
  ...
  try {
    ...
    const sourceOffset = getTimezoneOffsetMs(
      new Date(naiveUtcMs),
      fromTimezone,
    );
    ...
    const getPart = (type: string) =>
      targetParts.find((p) => p.type === type)?.value || "00";
    ...
    const dayOffset = Math.round(
      (targetDate.getTime() - sourceDate.getTime()) / msPerDay,
    );
...
export default function SlugBookingCalendar() {
  ...
  const { data: calendarData, isLoading } = useQuery<PublicCalendarData>({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/public/booking/by-slug/${slug}`,
      );
  ...
  >({
    queryKey: [
      `/api/public/booking/by-slug/${slug}/slots`,
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest(
        "GET",
        `/api/public/booking/by-slug/${slug}/slots/${dateStr}`,
      );
  ...
  const bookingMutation = useMutation({
    mutationFn: async (formData: BookingFormData) => {
      ...
      const response = await apiRequest(
        "POST",
        `/api/public/booking/by-slug/${slug}/book/${dateStr}/${selectedSlot.id}`,
        formData,
      );
      return await response.json();
    },
    onSuccess: (data, formData) => {
      ...
      const dateStr = formatDateForAPI(selectedDate!);
      const photographerTz =
        calendarData?.photographer.timezone || "America/New_York";
  ...
  const formatSlotTimeInClientTimezone = (
    slotTime: string,
    showDayOffset = false,
  ) => {
          ...
          <div>
                      ...
                      <div className="space-y-3">
                        {Array.isArray(timeSlots) &&
                          timeSlots.map((slot) => (
                            ...
                            >
                              <div className="text-center">
                                {formatSlotTimeInClientTimezone(
                                  slot.startTime,
                                  true,
                                )}{" "}
                                -{" "}
            ...
            {selectedSlot && (
              <div className="mt-6">
                ...
                <p className="text-center text-sm text-gray-500 mt-3">
                  Selected:{" "}
                  {formatSlotTimeInClientTimezone(selectedSlot.startTime, true)}{" "}
                  - {formatSlotTimeInClientTimezone(selectedSlot.endTime, true)}{" "}
                  ({clientTzAbbr})
                </p>
              </div>
            )}
            ...
            <div className="mt-6 flex items-center justify-center gap-2 text-sm text-gray-600">
              ...
              <Select value={clientTimezone} onValueChange={setClientTimezone}>
                <SelectTrigger
                  className="w-auto h-8 text-sm border-0 bg-transparent hover:bg-gray-100 px-2"
                ...
                <SelectContent>
                  ...
                  {!TIMEZONE_OPTIONS.find(
                    ...
                    <SelectItem value={clientTimezone}>
                      {clientTimezone} (
                      {getTimezoneAbbreviation(clientTimezone)})
                    </SelectItem>
                  )}
                </SelectContent>
              </Select>
