...
import { Calendar as CalendarIcon, Clock, Settings, Trash2, Plus, Coffee, Copy, ExternalLink, Link, Check, CheckCircle, X, Video, User, Mail, Phone, FileText, CalendarX, Loader2, ChevronLeft, ChevronRight, RotateCcw } from "lucide-react";
import { SchedulingCalendar, type DayOverride } from "@/components/SchedulingCalendar";
...
export default function Scheduling() {
  ...
  const [blockDateRange, setBlockDateRange] = useState<DateRange | undefined>(undefined);
  const [blockSelectedDates, setBlockSelectedDates] = useState<Date[] | undefined>(undefined);
  const [dateSelectionMode, setDateSelectionMode] = useState<'range' | 'individual'>('individual');
  const [blockReason, setBlockReason] = useState<string>('');
  const [availabilityType, setAvailabilityType] = useState<'blocked' | 'custom'>('blocked');
  ...
  const [editingOverride, setEditingOverride] = useState<DayOverride | null>(null);
  const [editOverrideType, setEditOverrideType] = useState<'blocked' | 'custom' | 'reset'>('blocked');
  ...
  const { data: bookingsData, isLoading: bookingsLoading } = useQuery<BookingData[]>({
    queryKey: ["/api/bookings"],
    enabled: !!user
  });
  const bookings: BookingData[] = Array.isArray(bookingsData) ? bookingsData : [];

  const { data: photographer, isLoading: photographerLoading } = useQuery({
    queryKey: ["/api/auth/me"],
    enabled: !!user
  }) as { data: { publicToken?: string; businessName?: string; timezone?: string; portalSlug?: string } | undefined; isLoading: boolean };
  ...
  const handleConnectCalendar = async () => {
    try {
      const response = await fetch("/api/auth/google-calendar?returnUrl=/scheduling", {
        credentials: 'include'
      });
  ...
  const bulkBlockMutation = useMutation({
    mutationFn: async (data: { startDate?: string; endDate?: string; dates?: string[]; reason?: string; startTime?: string; endTime?: string }) => {
      const response = await apiRequest("POST", "/api/availability/overrides/bulk", data);
  ...
  const updateOverrideMutation = useMutation({
    mutationFn: async (data: { id: string; startTime?: string | null; endTime?: string | null; reason?: string | null }) => {
      const response = await apiRequest("PUT", `/api/availability/overrides/${data.id}`, {
        startTime: data.startTime,
        endTime: data.endTime,
        reason: data.reason
      });
  ...
  const formatSelectedDates = (dates: Date[]): string => {
    ...
    if (dates.length <= 3) {
      return sortedDates.map(d => d.toLocaleDateString('en-US', options)).join(', ');
    }
    const first = sortedDates[0].toLocaleDateString('en-US', options);
    const last = sortedDates[sortedDates.length - 1].toLocaleDateString('en-US', options);
    return `${first} ... ${last}`;
  };

  // Check if any dates are selected (for either mode)
  const hasSelectedDates = dateSelectionMode === 'individual'
  ...
  const handleSlotPillClick = (slot: TimeSlot) => {
    if (!slot.isAvailable) {
      ...
      const matchingBooking = bookings.find(booking => {
        const bookingDate = new Date(booking.startAt).toLocaleDateString('en-CA', { timeZone: timezone });
        const bookingStartTime = new Date(booking.startAt).toLocaleTimeString('en-US', {
          timeZone: timezone,
          hour: '2-digit',
          minute: '2-digit',
          hour12: false
    ...
    <div className="flex flex-col h-[calc(100vh-1px)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card">
        <div className="flex items-center gap-2">
          <SidebarTrigger data-testid="button-menu-toggle" className="md:hidden mr-2" />
          <Button variant="outline" size="sm" onClick={() => handleCalendarNavigate('today')}>
            Today
          </Button>
          <Button variant="ghost" size="sm" onClick={() => handleCalendarNavigate('prev')}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <h2 className="text-lg font-semibold min-w-[180px] text-center">{calendarHeaderText}</h2>
          <Button variant="ghost" size="sm" onClick={() => handleCalendarNavigate('next')}>
            <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
        <Select value={calendarView} onValueChange={(v) => setCalendarView(v as 'month' | 'week' | 'day')}>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-64 border-r border-border bg-card p-4 overflow-y-auto hidden md:flex md:flex-col">
          ...
          <div className="mb-6">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">ThePhotoCrm</h3>
          ...
          {/* Availability Legend */}
          <div className="mb-6">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Availability</h3>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
          ...
          <div className="mb-6">
            ...
            {isCalendarConnected ? (
              <>
                ...
                {/* Legend */}
                {googleCalendarMetadata && Object.keys(googleCalendarMetadata).length > 0 ? (
                  <div className="space-y-2">
                    {Object.entries(googleCalendarMetadata).map(([calId, cal]) => (
                      <div key={calId} className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded" style={{ backgroundColor: cal.color }} />
                        <span className="text-sm truncate">{cal.name}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">No calendars selected</p>
                )}
              </>
            ) : (
              <>
          ...
          <div className="pt-4 border-t border-border">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={handleNewTemplate}>
            ...
            <div className="grid grid-cols-7 gap-0.5">
              {["M", "T", "W", "T", "F", "S", "S"].map((day, index) => {
                ...
                const template = dailyTemplates.find(t => Number(t.dayOfWeek) === dayMap[index]);
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      const t = dailyTemplates.find(t => Number(t.dayOfWeek) === dayMap[index]);
                      setEditingTemplate(t || null);
                      setShowTemplateModal(true);
                    }}
                    title={hasAvailability && template ? `${formatTime(template.startTime)} - ${formatTime(template.endTime)}` : 'Off'}
                  >
                    {day}
                  </div>
                );
            ...
            <div className="pt-3 mt-3 border-t border-border">
              <div className="flex items-center gap-1">
                <Button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(`https://${photographer.portalSlug}.tpcportal.co/book`);
                    toast({ title: "Copied!", description: "Booking link copied" });
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => window.open(`https://${photographer.portalSlug}.tpcportal.co/book`, '_blank')}
                >
      ...
      {/* Template Modal */}
        ...
        <Dialog open={showOverrideModal} onOpenChange={(open) => {
          ...
          <DialogContent className="max-w-lg p-0 overflow-hidden animate-in zoom-in-95 fade-in duration-200" aria-describedby="override-description">
            {/* Header - changes color based on availability type */}
            <div className={`px-6 pt-6 pb-4 ${availabilityType === 'blocked' ? 'bg-amber-50 dark:bg-amber-950/30' : 'bg-violet-50 dark:bg-violet-950/30'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p id="override-description" className="text-sm text-muted-foreground">
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Select date(s)</p>
                ...
                <div className="border border-border rounded-lg p-2 flex justify-center">
                  {dateSelectionMode === 'individual' ? (
                    <Calendar
                      ...
                      classNames={{
                        day_selected: availabilityType === 'blocked'
              ...
              {hasSelectedDates && (
                <div className={`rounded-lg p-4 border ${
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-sm" data-testid="block-date-range-display">
                        {dateSelectionMode === 'individual' && blockSelectedDates
                          ? formatSelectedDates(blockSelectedDates)
                          : blockDateRange?.from
                            ? formatDateRange(blockDateRange.from, blockDateRange.to || blockDateRange.from)
                            : ''
                        }
                      </p>
                      <p className="text-xs text-muted-foreground" data-testid="block-day-count">
                        {getSelectedDateCount()} {getSelectedDateCount() === 1 ? 'day' : 'days'} selected
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {hasSelectedDates && (
                <div>
                  <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Availability type</p>
                  <div className="space-y-2">
                    <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                      ...
                      />
                      <div>
                        <p className="font-medium text-sm">Completely unavailable</p>
                        <p className="text-xs text-muted-foreground">Block entire day(s) - vacation, holiday, etc.</p>
                      </div>
                    </label>
                    <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Set specific availability hours for selected dates</p>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              {hasSelectedDates && availabilityType === 'custom' && (
                <div className="grid grid-cols-2 gap-4 transition-all duration-200">
                  <div>
                    <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider block mb-2">Start Time</label>
                    <Input
                      type="time"
                  ...
                  <div>
                    <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider block mb-2">End Time</label>
                    <Input
                      type="time"
                      value={customEndTime}
              ...
              {hasSelectedDates && (
                <div>
                  <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Note (optional)</p>
                  <Input
                    placeholder={availabilityType === 'blocked' ? "e.g., Holiday vacation, Wedding trip..." : "e.g., Evening availability only..."}
                    value={blockReason}
                    onChange={(e) => setBlockReason(e.target.value)}
                    className={availabilityType === 'blocked'
            ...
            <div className={`px-6 py-4 border-t flex items-center justify-between ${
              ...
              <Button
                onClick={() => {
                  if (dateSelectionMode === 'individual' && blockSelectedDates && blockSelectedDates.length > 0) {
                    // Individual dates mode - send dates array
                    const sortedDates = [...blockSelectedDates].sort((a, b) => a.getTime() - b.getTime());
                    bulkBlockMutation.mutate({
                      dates: sortedDates.map(d => d.toISOString().split('T')[0]),
                      reason: blockReason || undefined,
                      startTime: availabilityType === 'custom' ? customStartTime : undefined,
                      endTime: availabilityType === 'custom' ? customEndTime : undefined
                    });
                  } else if (dateSelectionMode === 'range' && blockDateRange?.from) {
                    // Range mode - send start/end dates
                    bulkBlockMutation.mutate({
                      startDate: blockDateRange.from.toISOString().split('T')[0],
                      endDate: (blockDateRange.to || blockDateRange.from).toISOString().split('T')[0],
                      reason: blockReason || undefined,
                      startTime: availabilityType === 'custom' ? customStartTime : undefined,
                      endTime: availabilityType === 'custom' ? customEndTime : undefined
                    });
                ...
                disabled={!hasSelectedDates || bulkBlockMutation.isPending || (availabilityType === 'custom' && customEndTime <= customStartTime)}
                className={availabilityType === 'blocked'
              ...
              <>
                ...
                <div className="p-6 pb-4">
                  <div className="flex items-start gap-4">
                    {/* Gradient Avatar */}
                    <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${getClientGradient(selectedBooking.clientName || 'Client')} flex items-center justify-center text-white font-semibold text-lg shadow-lg`}>
                      {(selectedBooking.clientName || 'C').charAt(0).toUpperCase()}
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold text-lg truncate" data-testid="modal-booking-title">
                          {selectedBooking.clientName || formatBookingTitle(selectedBooking.title)}
                        </h3>
                        <Badge
                ...
                <div className="px-6 pb-4 space-y-4">
                  {/* Scheduled Time */}
                  <div>
                    <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Scheduled Time</p>
                    <div className="flex items-center gap-3 text-sm">
                      <Clock className="w-4 h-4 text-muted-foreground" />
                      <div>
                        <p className="font-medium" data-testid="modal-booking-date">
                          {(() => {
                        ...
                        <p className="text-muted-foreground" data-testid="modal-booking-time">
                          {(() => {
                            const startDateTime = formatBookingDateTime(selectedBooking.startAt);
                  ...
                  {selectedBooking.googleMeetLink && (
                    <div>
                      <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Meeting Link</p>
                      <a
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-sm">Join Meeting</span>
                        </div>
                      ...
                      <div className="space-y-2">
                        {selectedBooking.clientEmail && (
                          <div className="flex items-center gap-3 text-sm">
                            ...
                            <span data-testid="modal-client-email">{selectedBooking.clientEmail}</span>
                          </div>
                        )}
                        {selectedBooking.clientPhone && (
                          <div className="flex items-center gap-3 text-sm">
                            <Phone className="w-4 h-4 text-muted-foreground" />
                            <span data-testid="modal-client-phone">{selectedBooking.clientPhone}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {selectedBooking.description && (
                    <div>
                      ...
                      <div className="p-3 rounded-lg bg-muted/50 border border-border">
                        <p className="text-sm text-muted-foreground" data-testid="modal-booking-description">
                          {selectedBooking.description}
                        </p>
                      </div>
                    </div>
        ...
        {/* Edit Override Modal */}
        <Dialog open={!!editingOverride} onOpenChange={(open) => {
          if (!open) {
            setEditingOverride(null);
          }
        }}>
          <DialogContent className="max-w-md p-0 overflow-hidden animate-in zoom-in-95 fade-in duration-200" aria-describedby="edit-override-description">
            {editingOverride && (
              <>
                {/* Header */}
                <div className={`px-6 pt-6 pb-4 ${
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p id="edit-override-description" className="text-sm text-muted-foreground">
                ...
                <div className="px-6 py-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                        ...
                        <div>
                          <p className="font-medium text-sm">Completely unavailable</p>
                          <p className="text-xs text-muted-foreground">Block entire day - vacation, holiday, etc.</p>
                        </div>
                      </label>
                      <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                        editOverrideType === 'custom'
                        ...
                        <div>
                          <p className="font-medium text-sm">Custom hours</p>
                          <p className="text-xs text-muted-foreground">Set specific availability hours</p>
                        </div>
                      </label>
                      <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                        ...
                        />
                        <div>
                          <p className="font-medium text-sm">Reset to default hours</p>
                          <p className="text-xs text-muted-foreground">Use your regular weekly schedule</p>
                        </div>
                      </label>
                    </div>
                  </div>
                  ...
                  {editOverrideType !== 'reset' && (
                    <div>
                      <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Note (optional)</p>
                      <Input
                        placeholder={editOverrideType === 'blocked' ? "e.g., Holiday vacation, Wedding trip..." : "e.g., Evening availability only..."}
                        value={editReason}
                        onChange={(e) => setEditReason(e.target.value)}
                        className={editOverrideType === 'blocked'
                          ? "border-amber-200 dark:border-amber-800 focus:border-amber-500 focus:ring-amber-500"
                          : "border-violet-200 dark:border-violet-800 focus:border-violet-500 focus:ring-violet-500"
                        }
                ...
                <div className={`px-6 py-4 border-t flex items-center justify-end gap-2 ${
                  ...
                  <Button
                    onClick={() => {
                      if (editOverrideType === 'reset') {
                        ...
                        updateOverrideMutation.mutate({
                          ...
                          startTime: editOverrideType === 'custom' ? editStartTime : null,
                          endTime: editOverrideType === 'custom' ? editEndTime : null,
                    ...
                    disabled={
                      ...
                      (editOverrideType === 'custom' && editEndTime <= editStartTime)
                    }
                    className={
                      editOverrideType === 'blocked'
                        ? "bg-amber-500 hover:bg-amber-600 text-white shadow-sm"
                  ...
                  >
                    ...
                    {editOverrideType === 'reset' ? 'Reset to Default' : 'Save Changes'}
                  </Button>
                </div>
              </>
            )}
