...
export default function Earnings() {
  ...
  const openStripeDashboard = async () => {
    ...
    if (!newWindow) {
      ...
      toast({
        title: "Popup Blocked",
        description: "Please allow popups for this site to open the Stripe Dashboard.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Unable to Open Dashboard",
        description: error.message || "Failed to generate dashboard link. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsOpeningStripeDashboard(false);
        ...
        <div className="p-3 sm:p-6 space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  View full transaction history, manage payouts, and update bank details
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <Tabs defaultValue="outstanding" className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="outstanding" data-testid="tab-outstanding" className="relative">
                Outstanding
            ...
            <TabsContent value="outstanding" className="space-y-4">
              ...
              {outstanding?.summary && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <div className="text-2xl font-bold text-orange-600" data-testid="text-total-outstanding">
                        {formatCurrency(outstanding.summary.totalOutstandingCents)}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {outstanding.summary.invoiceCount} invoice{outstanding.summary.invoiceCount !== 1 ? 's' : ''}
                      </div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="p-4">
                      <div className="text-sm text-muted-foreground">Already Collected</div>
                  ...
                  <Card className={outstanding.summary.hasOverdue ? 'border-red-200 bg-red-50 dark:bg-red-950/20' : ''}>
                    <CardContent className="p-4">
                      <div className="text-sm text-muted-foreground">Overdue Invoices</div>
                      <div className={`text-2xl font-bold ${outstanding.summary.hasOverdue ? 'text-red-600' : 'text-muted-foreground'}`} data-testid="text-overdue-count">
                        {outstanding.summary.overdueCount}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {outstanding.summary.hasOverdue ? 'Requires attention' : 'All invoices on track'}
                      </div>
                    </CardContent>
                  </Card>
                </div>
              ...
              <Card>
                ...
                <CardContent>
                  {outstandingLoading ? (
                    <div className="text-center py-8">Loading outstanding invoices...</div>
                  ) : !outstanding?.invoices || outstanding.invoices.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      <CheckCircle className="w-12 h-12 mx-auto mb-4 text-green-500 opacity-50" />
                      <p className="font-medium">All caught up!</p>
                      <p className="text-sm">No outstanding invoices at the moment.</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {(outstanding.invoices as any[]).map((invoice: any) => {
                        const dueDate = invoice.nextInstallment?.dueDate ? new Date(invoice.nextInstallment.dueDate) : null;
                          ...
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <h4 className="font-semibold">{invoice.title}</h4>
                                  {invoice.hasOverdue && (
                              ...
                              <div className="text-right">
                                <div className="text-lg font-bold">{formatCurrency(invoice.remainingCents)}</div>
                            ...
                            <div className="mb-3">
                              <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                                <span>{formatCurrency(invoice.paidCents)} paid</span>
                                <span>{invoice.percentComplete}% complete</span>
                              </div>
                              <Progress value={invoice.percentComplete} className="h-2" />
                            </div>
                            ...
                            {invoice.nextInstallment && (
                              <div className="flex items-center justify-between pt-2 border-t">
                                <div className="text-sm">
                                  ...
                                  <span className="font-medium">{invoice.nextInstallment.label}</span>
                                  ...
                                  <span className={`${
                                    ...
                                    {daysUntil && daysUntil > 0 && daysUntil <= 7 && ` (${daysUntil}d)`}
                                  </span>
                                </div>
                                <Button 
                                  variant="outline" 
                                  size="sm"
                                  onClick={() => setLocation(`/smart-file/${invoice.token}`)}
