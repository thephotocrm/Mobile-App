...
export default function DripCampaigns() {
  ...
  const [selectedEmailForPreview, setSelectedEmailForPreview] =
    useState<any>(null);
  ...
  const [previewTab, setPreviewTab] = useState<"builder" | "preview">(
    "builder",
  );
  ...
  const { data: campaignSettings } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/static-campaign-settings/${selectedProjectType}`,
      );
  ...
  const saveSettingsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["/api/static-campaign-settings"],
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const updateEmailMutation = useMutation({
    mutationFn: async (data: {
      ...
      const res = await apiRequest(
        "PATCH",
        `/api/drip-campaigns/${data.campaignId}/emails/${data.emailId}`,
        {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["/api/drip-campaigns/static-templates"],
      });
  ...
  const { data: staticCampaign, isLoading: campaignsLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/drip-campaigns/static-templates?projectType=${selectedProjectType}`,
      );
      return await res.json();
    },
    enabled: !!user?.photographerId,
  ...
  const { data: draftCampaigns = [] } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/drip-campaigns?projectType=${selectedProjectType}`,
      );
      return await res.json();
    },
    enabled: !!user?.photographerId,
  ...
  const mergedEmails = useMemo(() => {
    ...
    const draftCampaign = draftCampaigns?.find(
      (c: any) => c.projectType === selectedProjectType && c.isStaticTemplate,
    );
    if (!draftCampaign?.emails || draftCampaign.emails.length === 0)
      return staticCampaign.emails;
    ...
    if (draftCampaign.emails.length < staticCampaign.emails.length) {
      return draftCampaign.emails.map((draftEmail: any) => {
        const staticEmail = staticCampaign.emails.find(
          (s: any) => s.sequenceIndex === draftEmail.sequenceIndex,
        );
        return {
          ...staticEmail,
          ...draftEmail,
    ...
    return staticCampaign.emails.map((staticEmail: any) => {
      const draftEmail = draftCampaign.emails.find(
        (d: any) => d.sequenceIndex === staticEmail.sequenceIndex,
      );
      if (draftEmail) {
  ...
  const handleSaveEmail = async () => {
    ...
    try {
      ...
      if (!firstStage) {
        toast({
          title: "Error",
          description: "No stages found. Please create a stage first.",
          variant: "destructive",
        });
        return;
      }
      ...
      console.log(
        "Looking for email with sequenceIndex:",
        emailBeingEdited.sequenceIndex,
      );
      ...
      console.log(
        ...
        draftCampaign.emails?.map((e: any) => e.sequenceIndex),
      );

      // Find the corresponding email in the draft campaign BY SEQUENCE INDEX
      const draftEmail = draftCampaign.emails?.find(
        (e: any) => e.sequenceIndex === emailBeingEdited.sequenceIndex,
      );
      ...
      if (!draftEmail) {
        ...
        toast({
          title: "Error",
          description: "Email not found in draft campaign",
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <div
      ...
      data-testid="page-drip-campaigns"
    >
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1
            ...
            data-testid="heading-drip-campaigns"
          >
            Drip Campaigns
          </h1>
        ...
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="text-sm">
            {mergedEmails.length || staticCampaign?.emails?.length || 24}{" "}
            Templates
          </Badge>
          <Button
            variant="outline"
      ...
      <div>
        ...
        <div className="md:hidden">
          <Label htmlFor="mobile-project-type" className="text-sm font-medium">
            Project Type
          </Label>
          <Select
            value={selectedProjectType}
          ...
          >
            <SelectTrigger
              ...
              data-testid="select-mobile-project-type"
            >
      ...
      <Tabs value={selectedProjectType} onValueChange={setSelectedProjectType}>
        <TabsContent value={selectedProjectType} className="mt-6">
          {/* Campaigns Section */}
          {(() => {
            const customCampaigns = (draftCampaigns || []).filter(
              (c: any) => !c.isStaticTemplate,
            );
                ...
                <Card>
                  <CardContent className="text-center py-12">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Create your first email campaign to nurture{" "}
                      {selectedProjectType.toLowerCase()} leads automatically.
                    </p>
                    <Button
                      onClick={() => setWizardOpen(true)}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Email sequences for {selectedProjectType.toLowerCase()}{" "}
                      projects
                    </p>
                  </div>
                </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                              ...
                              <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground flex-wrap">
                                ...
                                {campaign.isGlobal ? (
                                  ...
                                  <span className="flex items-center gap-1 text-green-600">
                                    {campaign.targetStageIds.length} stage
                                    {campaign.targetStageIds.length !== 1
                                      ? "s"
                                      : ""}{" "}
                                    selected
                                  </span>
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              onClick={() =>
                                setLocation(
                                  `/drip-campaigns/${campaign.id}/edit`,
                                )
                              }
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    await apiRequest(
                                      "POST",
                                      `/api/drip-campaigns/${campaign.id}/activate`,
                                    );
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    await apiRequest(
                                      "POST",
                                      `/api/drip-campaigns/${campaign.id}/pause`,
                                    );
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  await apiRequest(
                                    "DELETE",
                                    `/api/drip-campaigns/${campaign.id}`,
                                  );
            ...
            <DialogHeader>
              <DialogTitle className="flex flex-col sm:flex-row sm:items-center gap-2 text-base sm:text-lg">
                ...
                <Badge variant="outline" className="text-xs w-fit">
                  Day {selectedEmailForPreview.daysAfterStart}
                </Badge>
              </DialogTitle>
                      ...
                      if (!bodyContent && selectedEmailForPreview.emailBlocks) {
                        try {
                          const blocks = JSON.parse(
                            selectedEmailForPreview.emailBlocks,
                          );
                      ...
                      if (bodyContent) {
                        return (
                          <EmailPreview
                            ...
                            includeHeader={
                              selectedEmailForPreview.includeHeader || false
                            }
                            headerStyle={
                              selectedEmailForPreview.headerStyle ||
                              "professional"
                            }
                            includeSignature={
                              selectedEmailForPreview.includeSignature || false
                            }
                            signatureStyle={
                              selectedEmailForPreview.signatureStyle ||
                              "professional"
                            }
                      ...
                      return (
                        <div
                          dangerouslySetInnerHTML={{
                            __html: selectedEmailForPreview.htmlBody || "",
                          }}
            ...
            <div className="lg:hidden">
              <div className="space-y-3 mb-4">
                ...
                <div className="pb-2 border-b">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Customize this email with the visual builder, set timing,
                    and preview changes
                  </p>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="border rounded-lg p-3 bg-card">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="text-sm font-medium">
                          Day {editDaysAfterStart}
                          {editSendAtHour !== null
                            ? ` at ${editSendAtHour === 0 ? "12" : editSendAtHour > 12 ? editSendAtHour - 12 : editSendAtHour}:00 ${editSendAtHour >= 12 ? "PM" : "AM"}`
                            : ""}
                        </div>
                        <Dialog
                        ...
                        >
                          ...
                          <DialogContent className="max-w-md">
                            ...
                            <div className="space-y-4 mt-4">
                              ...
                              <div>
                                <Label htmlFor="timing-hour">
                                  Send At Time
                                </Label>
                                <Select
                                  value={
                                    editSendAtHour !== null
                                      ? editSendAtHour.toString()
                                      : "none"
                                  }
                                  onValueChange={(value) =>
                                    setEditSendAtHour(
                    ...
                    <div className="border rounded-lg p-3 bg-card">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="text-sm font-medium">
                          {editIncludeHeader
                            ? `${editHeaderStyle.charAt(0).toUpperCase() + editHeaderStyle.slice(1)}`
                            : "None"}
                        </div>
                        <Button
                          type="button"
                    ...
                    <div className="border rounded-lg p-3 bg-card">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="text-sm font-medium">
                          {editIncludeSignature
                            ? `${editSignatureStyle.charAt(0).toUpperCase() + editSignatureStyle.slice(1)}`
                            : "None"}
                        </div>
                        <Button
                          type="button"
              ...
              <div className="bg-blue-200 dark:bg-slate-800/60 rounded-lg p-4 border border-blue-400 dark:border-slate-600">
                <Tabs
                  value={previewTab}
                  onValueChange={(v) =>
                    setPreviewTab(v as "builder" | "preview")
                  }
                >
              ...
              <div className="overflow-auto pr-2">
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-200 dark:bg-slate-800/60 rounded-lg p-4 space-y-3 border border-slate-400 dark:border-slate-500">
                    {/* Email Subject */}
                    <div>
                      <Label htmlFor="edit-subject-desktop" className="text-sm">
                        Email Subject
                      </Label>
                      <Input
