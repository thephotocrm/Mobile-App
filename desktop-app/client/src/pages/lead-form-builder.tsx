      ...
      <Card className="shadow-xl max-w-2xl mx-auto">
        ...
        >
          {rows.map((row, rowIndex) => (
            ...
            >
              {row.map((field) => (
                <div key={field.id}>{renderField(field, false)}</div>
              ))}
            </div>
          ))}
          ...
          <div className="flex items-center gap-2">
            <Dialog open={previewModalOpen} onOpenChange={setPreviewModalOpen}>
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="py-6">{renderCleanPreview()}</div>
              </DialogContent>
            </Dialog>
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={(checked) =>
                  setConfig({ ...config, showSmsConsent: checked })
                }
          ...
          <Card className="shadow-xl">
            ...
            >
              {groupFieldsIntoRows(config.customFields).map((row, rowIndex) => {
                const isHalfWidthRow = row.some(
                  (f) => (f.width || "full") === "half",
                );
                return (
                  <div
                    key={rowIndex}
                    className={isHalfWidthRow ? "grid grid-cols-2 gap-3" : ""}
                  >
                    {row.map((field) => {
                          ...
                          {!field.isSystem && (
                            <div className="absolute -top-3 -right-3 flex gap-1 bg-card rounded-md border border-border p-1">
                              ...
                              <Button
                                ...
                                disabled={
                                  index === config.customFields.length - 1
                                }
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex bg-muted rounded-md p-0.5">
                                    <button
                                      type="button"
                                      onClick={() =>
                                        updateField(field.id, { width: "half" })
                                      }
