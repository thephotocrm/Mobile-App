...
**Status:** ✅ Login endpoint now returns token. ⚠️ Portal token endpoint still needs modification.

**File:** `server/routes.ts`

**Login Endpoint:** ✅ Already implemented - returns `token` in response.

**Portal Token Endpoint (still needs modification around line 2159):**
```typescript
...
**Note:** Registration does not automatically log in the user. After successful registration, the mobile app should redirect to the login screen where the user can authenticate and receive their JWT token.
...
**Note:** The JWT token is returned in the response AND set as an httpOnly cookie. Mobile apps should store the token for subsequent requests.

#### GET /api/auth/me
Get current authenticated user with role-specific metadata.
...
#### POST /api/client-portal/projects/:id/send-message
Send a message from client to photographer (appears in project activity feed).
...
```json
{
  ...
  "body": "Hi! I wanted to ask about the wedding day timeline..."
}
```
...
#### DELETE /api/client-portal/payment-methods/:methodId
...
#### POST /api/client-portal/set-default-payment-method
...
#### GET /api/client-portal/projects/:id/payment-summary
Get payment summary for a project (total owed, paid, remaining).
...
```json
{
  ...
  "nextPaymentDue": "2026-02-15T00:00:00.000Z",
...
[
  {
    ...
    "lastReadAt": "2025-11-19T10:25:00Z",
    "unreadCount": 2,
    "channel": "SMS"
  }
]
...
| imageUrl | string? | URL of MMS image (if type is MMS) |
...
| imageUrl | string | No | URL for MMS image. Supports https:// URLs or base64 data URLs (data:image/...) |

> **Note:** When imageUrl is provided, the message is sent as MMS. Base64 data URLs are supported for sending images directly from the device.

#### POST /api/inbox/mark-read/:contactId
...
#### POST /api/projects/:id/send-sms
Send SMS to project client.
...
```json
{
  "message": "Hi! Just checking in about your upcoming session.",
  "imageUrl": "https://example.com/image.jpg"
}
```
...
|-------|------|----------|-------------|
| message | string | Yes | Message text content |
| imageUrl | string | No | URL for MMS image |
...
```json
{
  "message": "Here's your contract to review and sign!"
}
```
...
> **Note:** The Smart File link is automatically appended to the message.
...
interface Photographer {
  ...
  emailSignatureStyle?: string;
  headshotUrl?: string;
  website?: string;
  businessAddress?: string;
  socialLinksJson?: object;
  // Gallery
  galleryPlatform?: string;
  galleryStorageBytes: string;
  galleryPlanId?: string;
  galleryExpirationMonths: number;
  // Onboarding
...
interface ProjectSmartFile {
  ...
  expirationMode?: "TIME_ONLY" | "UNLESS_PAYMENT" | "UNLESS_BOOKING" | "UNLESS_SIGNED";
...
interface Stage {
  id: string;
  photographerId: string;
  projectType?: string;       // Deprecated
  projectTypeId?: string;     // New FK reference to projectTypes
  name: string;
  orderIndex: number;
